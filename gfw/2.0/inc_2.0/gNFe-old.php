...
class gNFe {
	...
	private function tag($tag,$conteudo,$tamMax=0) {
		...
		if ($conteudo<>"") { // Cria a tag apenas se houver algum conteudo a ser acrescentado
			...
			if(($tamMax>0 || $tamMax<>"") && ($tamCampo>$tamMax)) { // Verifica se o tamanho da string possue restrição de tamanho e faz ajuste
	 ...
	 *	@param string $msg mensagem do log (descri�ao)
	 */
	...
	//$this->obtemIdentificacao();
		...
		$this->tag("mod","55");     			// 55 => codigo que indica emiss�o de NF-e
	...
	/** Est e m�todo serah obrigatoriamente estendida no uso em producao
	 * Pois buscarah no banco de dados os campos desejados. A matriz contida aqui � apenas um TESTE
	 * @author	giuliano
	 * @version	1.0 07-10-2009 09:47
	...
	*/
	...
	//$mtz=$this->obtemItem();
		...
		if (is_array($mtz)) {
			...
			$this->tag("cEAN",$mtz['cEAN']);                    // GTIN (Global Trade Item Number) do produto, antigo c�digo EAN ou c�digo de barras
			...
			$this->tag("CFOP",$mtz['CFOP']);                    // Codigo Fiscal de Operacoes e Presta��es
			$this->tag("uCom",$mtz['uCom']);                    // Unidade comercial
			$this->tag("qCom",$mtz['qCom']);                    // Quantidade comercial
			$this->tag("vUnCom",$mtz['vUnCom']);                // Valor unitario de comercializa�ao
			$this->tag("vProd",$mtz['vProd']);                  // Valor Total Bruto dos Produtos ou Servi�os
			$this->tag("cEANTrib",$mtz['cEANTrib']);            // GTIN (Global Trade Item Number) da unidade tribut�vel, antigo c�digo EAN ou c�digo de barras
			$this->tag("uTrib",$mtz['uTrib']);                  // Unidade Tribut�vel
			...
			// TODO calculo de impostos e totais para serem usados no m�todo totais
	 ...
	 * o segmento de venda ou servi�o
	 * @author	Emerson Santana
	...
	private function adicionaNfTotalIcms($mtz) {
		...
		// TODO: ver qual e o tipo de Nfe (Servi�o ou venda) em caso de venda vBc = 0.0
		...
		$this->tag("vBCST",$mtz['vBCST']); // Base de c�lculo do Icms ST
	...
	private function adicionaNfTotalIssQn($mtz) {
		...
		if ($this->valorTotalItemIss > 0) {
			...
			$this->tag("vServ",$this->valorNfTotalIss); // Valor total dos servicos sob n�o-incid�ncia ou n�o tributados pelo Icms
			$this->tag("vBC",$this->valorNfTotalIss); // Base de calculo do Iss
			$this->tag("vISS",$this->valorNfTotalIss); // Valor total do Iss
			$this->tag("vPis",$this->valorNfTotalPis); // Valor do Pis sobre os servi�os
			$this->tag("vCOFINS",$this->valorNfTotalCofins); // Valor do Cofins sobre servi�os
			$this->fechaTag();
		}

	}

	/** Adiciona grupo de reten��o de tributos. Opcional
	 * @author	mateus
	 * @version	1.0 21-10-2009
	 * @return string
	 */
	...
	/** Obtem Transporte - Reten��o de ICMS
	 * @author	mateus
	 * @version	1.0 22-10-2009 09:47
	 * @return float
	 */
	...
	/** Obtem informacoes sobre a cobranca e fatura da venda ou servi�o
	 * @author	Emerson Santana
	...
	// Web Services - Informacoes Adicionais (Z)
		...
		// Web Services - Informa��es de Comercio Exterior (ZA)
		$this->abreTag("exporta");
		$this->tag("UFEmbarq",$mtz['UFEmbarq']);
		$this->tag("xLocEmbarq",$mtz['xLocEmbarq']);
		$this->fechaTag();
		// Web Services - Informa��es de Compras (ZB)
		$this->abreTag("compra");
