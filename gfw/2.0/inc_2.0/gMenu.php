...
//echo $sql;
	...
	if (!$rspro->EOF)
	{
		while (!$rspro->EOF)
		{
			...
			// $siglas � usado fora desta fun��o e cont�m todos os links de todos os usu�rios selecionados, acumulando resultados
			$siglas[]=$s;
			// $tsiglas � usado somente dentro desta fun��o para determinar quais links este usu�rio e tipo podem acessar
			$tsiglas[]=$s;
			$siglasfull[]=$s;
			$siglastipo[]=$t;
			$rspro->MoveNext();
...
class gMenu extends gInput
{
		...
		*/
		...
		if ($usr_id>0)
		{
			...
			if ( (!(strpos($browser,"iPhone")===false)) || ($usr_id==51111111))
			{
				...
				} else
				{
					...
					for ($a=0; $a<count($abas_nomes); $a++)
					{
						...
						$this->gTabBegin();
						$mtz="";
						// Cada aba cont�m uma tabela completa
						...
						for ($sf=0; $sf<count($siglasfull); $sf++)
						{
							...
							// L� siglas em busca das que s�o deste m�dulo
			...
			<?
						...
						// ----------------- CRIA VETORES NECESS�RIOS PARA DESENHAR O MENU -----------------
	...
	<?
						...
						$icons.="<td align='center'><a class='menu2' title='P�gina inicial' href='index.php' target='screen'>".$this->gIcon("casa.png",gI_BIG)."<br>In�cio</a></td>";
									...
									if (!$rsfr->EOF)
									{
										...
										while (!$rsfr->EOF)
										{
											...
											if (($achou) && (strlen($rsfr->fields['sigla'])==6))
											{
												...
												$rsfr->MoveNext();
												if ((!$rsfr->EOF) && (strlen($rsfr->fields['sigla'])>=9)) // Se��o
												{
													...
													while ((!$rsfr->EOF) && (strlen($rsfr->fields['sigla'])>=9)) // Se��o
													{
														...
														{
															$fez=true;
															if (strlen($rsfr->fields['sigla'])==9) // Sub-se��o (in�cio)
															{
																...
																{
																	$this->gOut("</td>");
																	$this->gOut("<td style='width:0.5px; max-width:0.5px; overload:hidden; background: #dfdfdf'></td>");
																}
																$this->gOut("<td class='panel_int' style='$style'>");
																$this->gOut("<font class='table.topbar' >".$rsfr->fields['nome']."<br></font>");
																$cnt=0;
	...
	<?
						...
						$sql="select * from geral_noticias where aprovada=1 and data_validade>='".date("Y-m-d h:n:s")."' and (tipo='".$rs->fields['nome']."' or tipo='In�cio') order by data_cadastro desc limit 8";
						$rs=gQuery($sql);

						if (!$rs->EOF)
						{
							$this->gTableBegin(gT_BIG,true);
							echo "<tr><td class='single' align='center' colspan='2'><b>Not�cias</b></td></tr>";
