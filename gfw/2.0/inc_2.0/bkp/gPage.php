...
* @param string $pagetype Tipo de página
*/
	function g_ToolBar($start,$step,$ttl,$move_previous,$move_next,$move_last,$reply)
	{
		$bar="";
		$step=$start+$step;
		...
		$s.="<-<input class='navig' type='button' value='<<' onClick=\"gGo('1','$go')\"><input class='navig' type='button' value='<' onClick=\"gGo('".$move_previous."','$go')\">";
		$s.="&nbsp;<input class='navig' type='button' value='>' onClick=\"gGo('".$move_next."','$go')\"><input class='navig' type='button' value='>>' onClick=\"gGo('".$move_last."','$go')\">";
		$barra[]=$s;
		$s="<-<input class='tool' type='text' name='start' value='".$start."' maxlength='8' size='6'> a $step de $ttl <input class='tool' type='button' value='".gLng("register_refresh.short")."' onClick=\"gGo('refresh','$go')\">&nbsp;";
	...
	function gShowPage($rs,$name,$perm,$fields="",$link="",$filters="",$autoend=true)
	{
		...
		else if (($action=="list") && (strpos($perm,"S")>0))
		{
			...
			if ($faz)
			{
				...
				$move_last=$ttl-$step; if($move_last<1) $move_last=1;
				...
				$this->g_ToolBar($start,$step,$ttl,$move_previous,$move_next,$move_last,$reply);
		...
		else if (($action=="edit") && (strpos($perm,"S")>0))
		{
			...
			if ($filters<>"")
			{
				...
				//if (!(($_REQUEST["gFilterCtrl"]=="onfilter") || (strlen($export)==3)))
				{
					...
					*/
					...
					foreach($_REQUEST as $chave => $valor)
					{
						if (($valor<>"") && ($chave!="PHPSESSID") && ($chave!="gAction"))
						{
							...
							for ($t_f=0; $t_f<count($filters); $t_f++)
							{
								...
								$this->chkField();
								if (($this->campo==$chave) || 
								((($filters[$t_f][0]==200) || ($filters[$t_f][0]==201)) && ($this->campo==substr($chave,0,strlen($chave)-1))))
								{
					...
					if (strlen($export)!=3)
						$this->gShowForm(array($label." - ".gLng("register_edit.long"),$name),"",$filters,$thispage_parm);
				} 
			...
			if ($faz)
			{
				...
				$this->g_ToolBar($start,$step,$ttl,$move_previous,$move_next,$move_last,$replay);
				/*
				$dict="";
				if (is_array($fields))
				{
		...
		else if (($action=="delete_tnl") && (strpos($perm,"I")>0))
		{
			...
			if ($gError=="")
			{
				$this->gMsgBox($label." - ".gLng("register_copy.long"),"Registro removido com sucesso.",array("Voltar a seleção sem atualizar|-1"));
			} else
			{
				$this->gMsgBox($label." - ".gLng("register_copy.long"),"<font style='color:red'>$gError</font>",array("Voltar|-1"));
			}
			if ($autoend) $this->gEnd();
		}
	}
