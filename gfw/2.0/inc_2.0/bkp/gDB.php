...
<?
		...
		$out->gOut("<img border='0' src='$http://".gVar("global.url").gBASE."/img_".gVER."/schema/".gVar("global.schema")."/mini_logo.jpg'><br><br>");
	...
	if (!$rs->EOF)
	{
		...
		} else
		{
			...
			if (($usr_id<>1) && ($cnt>4))
			{
				...
				while (!$rsts->EOF)
				{
					$sigla=$rsts->fields['sigla'];
					$sigla=substr($sigla,0,9);
					$sql="Select geral_links_permissoes.* from geral_links,geral_links_permissoes where ";
					$sql.=" geral_links.sigla=geral_links_permissoes.sigla_geral_links and sigla like '$sigla%'";
