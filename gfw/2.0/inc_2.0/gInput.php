...
*/
	function gMemo($name,$value="",$cols=70,$rows=5,$param="",$exit_on_page=true)
	{
		...
		if ($label<>"") $out=$label."&nbsp;";
		if ($cols>=60)
			$out.="<textarea id='$name' name='$name' rows='$rows' style='width: 100%'>".$value."</textarea>";
		else
			$out.="<textarea id='$name' name='$name' rows='$rows' cols='$cols'>".$value."</textarea>";
		if ($exit_on_page)
		{
			$this->gOut($out);
		}
...
function gLogoPrint($msg="",$bd=false)
{
	...
	if ($_REQUEST["special"]=="")
	{
		...
		if (is_array($keys))
		{
			foreach ($keys as $rpl)
            {
                if(is_array($_REQUEST[$rpl]))
                {
                    $go.="&" . urlencode($rpl)."=".urlencode(implode(",",$_REQUEST[$rpl]));
                }
                else
                    $go.="&" . urlencode($rpl)."=".urlencode($_REQUEST[$rpl]);
            }
		...
		if (gVar("export.xls")=="true")
			$this->gImage("xls.png","onclick=\"javascript:document.getElementById('gLogoPrintIcons').innerHTML='';gExportGo('xls')\"","",true,"Exportar para planilha");
		if (gVar("export.doc")=="true")
			$this->gImage("doc.png","onclick=\"javascript:document.getElementById('gLogoPrintIcons').innerHTML='';gExportGo('doc')\"","",true,"Exportar para documento");
