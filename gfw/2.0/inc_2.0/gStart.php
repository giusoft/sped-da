...
define('gE_DATABASEQUERY','Erro na express„o de acesso ao banco de dados.');
...
if (gVar("global.url")=="")
	gVar("global.url",$_SERVER['HTTP_HOST']);
...
/** Classe responsÍvel por funÍÍes acessÍrias para variÍveis de sessÍo e aplicaÍÍo
*@package gStart
...
/** Abre um arquivo e inicia uma aplicaÍÍo
*@author Giuliano Nascimento
*@version 2.0
...
/**Retorna um dado especÍfico do arquivo
*@author Giuliano Nascimento
*@version 2.0
*@param mixed $var Valor a ser procurado no arquivo
*@return mixed $ret Se $var nÍo for um array, o valor de retorno serÍ o conteÍdo de um array com a chave $var,
ou se $var for um array, este serÍ retornado.
*/
	function get($var) {
		$vars=$this->getall();
...
/**Retorna todo o conteÍdo do arquivo
*@author Giuliano Nascimento
*@version 2.0
*@return mixed $vars Todo o conteÍdo do arquivo, ou FALSE caso ocorra algum erro durante a leitura do arquivo
*/
function getall() {
		if (!$this->fp) return FALSE;
		$ilocked=$this->_trylock();
...
*@author Giuliano Nascimento
*@version 2.0
*@return bool $ret TRUE se o arquivo foi travado com sucesso, FALSE caso contrÍrio
*/
...
/**Trava o arquivo de forma exclusiva (para gravaÍÍo)
*@author Giuliano Nascimento
*@version 2.0
*@return bool $ret TRUE se o arquivo foi travado com sucesso, FALSE caso contrÍrio
*/
...
*@author Giuliano Nascimento
*@version 2.0
*@return bool $ret TRUE se o arquivo foi destravado com sucesso, FALSE caso contrÍrio
*/
	function unlock() {
		if (!$this->fp) return FALSE;
...
*@author Giuliano Nascimento
*@version 2.0
*@return NULL se o arquivo nÍo for vÍlido, FALSE se o arquivo jÍ se estiver travado
...
*@author Giuliano Nascimento
*@version 2.0
*@param mixed $vars Dados que serÍo serializados e escritos em um arquivo
*/
...
/**LÍ dados de um arquivo
*@author Giuliano Nascimento
*@version 2.0
*@return string $buff O conteÍdo do arquivo
*/
function _read() {
		rewind($this->fp);
		$buff="";
...
*ou false, caso o arquivo nÍo tenha sido aberto corretamente
*/
	function _openfile() {
...
// Fim de funÍÍes acessÍrias para variÍveis de sessÍo e aplicaÍÍo

/**LÍ os dados de um arquivo
*@author Giuliano Nascimento
*@version 2.0
*@param string $t_filename O arquivo a ser lido
*@return $s O conteÍdo do arquivo lido
*/
...
/**LÍ um arquivo .XML e converte-o em um array de dois elementos
*sendo o 1Í = nome e 2Í = valor
*@author Giuliano Nascimento
*@version 2.0
			...
			if (substr($txt,$pnt+1,1)<>"?")
			{
				...
				if (!($pntEnd===false))
				{
					...
					} else
					{
						...
						if ($pntNext>0)
						{
							...
							if ($pntClose>0)
							{
								...
								if ($TagClose=='/'.$Tag)
								{
									...
									if ($cntfld>$maxfld)
									{
										//*** Erro: MÍximo de campos atingido
										$pnt=strlen($txt)+1;
									}
								} else
								{
						...
						{
							$erro=true;
							//*** Erro: NÍo existe o fechamento da TAG
						}
					}
				}
				else
				{
					$erro=true;
					//*** Erro: Iniciou a TAG mas nÍo concluiu
				}
			}
			else
			{
...
/**Registra uma nova sessÍo
*@author Giuliano Nascimento
*@version 2.0
*@param string $t_name O nome da sessÍo
*@param string $t_value Valor atribuÍdo Í variÍvel de sessÍo
*
*/
function gSessionSave($t_name,$t_value)
{
	//@session_register($t_name); ===> removido por conta do php 5.4
	$_SESSION[$t_name] = $t_value;
}

/**Recupera o valor de uma variÍvel de sessÍo se ela existir, caso nÍo exista, redireciona para pÍgina de erro
*@author Giuliano Nascimento
*@version 2.0
*@param string $t_name
*@return mixed O valor da variÍvel de sessÍo
*/
...
/**Salva valor em variÍvel global de aplicaÍÍo
*@author Giuliano Nascimento
*@version 2.0
*@param string $t_name
...
/**Recupera o valor de variÍvel de aplicaÍÍo
*@author Giuliano Nascimento
*@version 2.0
*@param string $t_name O nome da variÍvel a ser recuperada
*@return mixed O valor de variÍvel de aplicaÍÍo
*/
...
function gReadSchema($t_name)
{
	global $gCfg;
	global $gPathDefault;
	// * LÍ Tema e DicionÍrio para a sessÍo atual
	$gxml="xml/gSchema_" . $t_name . ".xml";
  // Carrega o arquivo XML de linguagem e seta as variÍveis
...
{
	// * Retorna o valor da variÍvel "parameter", da matriz "gcfg" (variÍvel de aplicaÍÍo)
	global $gDebug;
	global $gCfg;
	if ($gCfg["global.site"]<>"")
	{
...
{
	global $gTags;
	// * Retorna o TAG constante na variÍvel "parameter", da matriz "gschema" (variÍvel de aplicaÍÍo)
	/* Desativado...
	global $gDebug;
	gRead();
...
function gRead()
{
	//* LÍ o arquivo de configuraÍÍes do site e armazena em uma variÍvel global (ou cookie)
	global $gPathDefault;
	global $gTransFile;
...
*/
	...
	if (($gLanguageFileDate<$gtmpFileDate2) || (!isset($_SESSION['usr_id'])))
	{
		// Carrega o arquivo XML de linguagem e seta as variÍveis
		if ($gTransFile=="")
...
function gDateDiff($from, $to, $negativo = 0) {
	...
	*/
	...
	/*Adicionado o ceil($days) para garantir que o resultado seja sempre um nÍmero inteiro */
...
function gFieldRpl()
{
	...
	$rpl[]=array("cao","Á„o");
	$rpl[]=array("sao","s„o");
	$rpl[]=array("ssoes","ssıes");
	$rpl[]=array("nao","n„o");
	$rpl[]=array("mao","m„o");
	$rpl[]=array("coes","Áıes");
	$rpl[]=array("tao","t„o");
	$rpl[]=array("toes","tıes");
	$rpl[]=array("odigo","Ûdigo");
	$rpl[]=array("id_","");
	$rpl[]=array("umero","˙mero");
	$rpl[]=array("ervico","erviÁo");
	return($rpl);
}
...
function html2str($texto)
{
	$t=trim($texto);
	$t=str_replace('&nbsp;',' ',$t);
	$t=str_replace('N<sup>o</sup>','NÍ',$t);
...
"sete", "oito", "nove");
	...
	// $fim identifica onde que deve se dar junÍÍo de centenas por "e" ou por "," ;)
...
{
//-----------------------------------------
// Mostra o conteÍdo de uma variÍvel na janela cliente
...
/** Efetua c·lculos passando a fÛrmula e um array contendo vari·veis
*@author Giuliano Nascimento
*@version 2.0
*@param string $formula FÛrmula (usa a mesma sintaxe do bc do shell, ou seja, aceita IF e outros comandos)
*@param array $matriz Matriz contendo vari·veis
*/
...
/** Converte um valor de um campo checkbox para 0 ou 1 (ao invÈs de '' ou 'on')
*@author Giuliano Nascimento
*@version 2.0
...
/** Aciona download de arquivo ou conte˙do da vari·vel
 * @author	giuliano
 * @version	1.0 29-04-2009 13:55
 * @param mixed $file Caminho do arquivo ou conte˙do a ser baixado
...
{
    $t_t=strtr($t_st,"·ÈÌÛ˙‡ËÏÚ˘‚ÍÓÙ˚„ı¡…Õ”⁄¿»Ã“Ÿ¬ Œ‘€√’Á«","aeiouaeiouaeiouaoAEIOUAEIOUAEIOUAOcC");
    return $t_t;
}
