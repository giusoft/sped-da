...
class gUICore
{
	 ...
	 * @return bool
	 */
 	function onAfterAdd($params = ""): bool
	{
		return true;
	}
	 ...
	 * @return bool
	 */
 	function onAfterUpdate($params = ""): bool
	{
		return true;
	}
	 ...
	 * @return bool
	 */
 	function onBeforeDelete($params = ""): bool
	{
		return true;
	}
	 ...
	 * @return bool
	 */
 	function onAfterDelete($params = ""): bool
	{
		return true;
	}
	 ...
	 * @version	4.0 29/07/2014 10:16
	 */
	function addRowButton($json): void
	{
		$jarr = cssDecode($json);
	 ...
	 */
	function getQueryData($name, $sql): void
	{
		global $sp;
	...
	function addLimitOnQuery($sql)
	{
		...
		if (stripos((string) $sql,"LIMIT ") !== false) {
			$sql = substr((string) $sql,0,stripos((string) $sql,"LIMIT "));
		}
	 ...
	 */
	function addDictionary($json): void
	{
		global $gDB;
		if (is_object($json)) {
			// Compatibilidade com gFW3
				...
				if (isset($this->tableFields[$jarr['name']])) {
					foreach ($jarr as $el => $value) {
						if ($el === "type") {
							...
							$arr = match ($value) {
								"combo", "comboMultiSelection" => $this->getQueryData($jarr['name'], $jarr['items']),
							};
						}
					}
				}
			...
			if (isset($this->tableFields[$jarr['name']])) {
				...
				foreach ($jarr as $el => $value) {
					if ($el=="type") {
						...
						$gDB[$this->tableAlias]['fields'][$jarr['name']]['align'] = $align;
						$arr = match ($value) {
							"combo", "comboMultiSelection" => $this->getQueryData($jarr['name'], $gDB[$this->tableAlias]['fields'][$jarr['name']]['items']),
						};
					}
				}
			}
	...
	function parseDictionaries(): void
	{
...
class gUI extends gUICore
{
	...
	function __construct( $json = "" )
	{
		...
		foreach ($_REQUEST as $key => $value) {
			if (
				...
				$this->extraParm .= "&".$key.'='.urlencode((string) $value);
			}
		}
		...
		if (stripos((string) $this->jarr['permissions'],'S') !== false) {
			$this->select = true;
		}

		if (stripos((string) $this->jarr['permissions'],'I') !== false) {
			$this->insert = true;
		}

		if (stripos((string) $this->jarr['permissions'],'U') !== false) {
			$this->update = true;
		}

		if (stripos((string) $this->jarr['permissions'],'D') !== false) {
			$this->delete = true;
		}

		if (stripos((string) $this->jarr['permissions'],'A') !== false) {
			$this->active = true;
		}
	 ...
	 */
	function setTableDefs($name): void
	{
		global $gDB;
		$this->tableName = $name;
		...
		foreach ($this->tables as $table) {
			if ($table['name'] == $this->tableAlias) {
				...
				if (stripos((string) $table['permissions'],'S') !== false) {
					$this->select = true;
				}

				if (stripos((string) $table['permissions'],'I') !== false) {
					$this->insert = true;
				}

				if (stripos((string) $table['permissions'],'U') !== false) {
					$this->update = true;
				}

				if (stripos((string) $table['permissions'],'D') !== false) {
					$this->delete = true;
				}

				if (stripos((string) $table['permissions'],'A') !== false) {
					$this->active = true;
				}
			}
		}
	 ...
	 * @param string $name Nome da tabela
	 */
	function setGridQuery($sql): void
	{
		$this->gridQuery = $sql;
	}
	 ...
	 */
	function addQuery($sql, $json = ""): void
	{
		$name = "";
		if (!$this->queries) {
	 ...
	 * @param string $sql Query
	 */
	function addConditional($json): void
	{
	 ...
	 */
	function addFormMessage(string $txt): void
	{
	...
	function parseQuery($name, $sql)
	{
		...
		if (str_contains((string) $name,' ')) {
			$name = trim(substr((string) $name,0,strpos((string) $name,' ')));
		}
	 ...
	 * @version	4.0 29/07/2014 10:16
	 */
	function addFilterOnQuery($filter = ''): string
	{
		global $usrIdd;
		$w = [];
		...
		if ($filter == '') {
			...
			foreach ($this->filters as $fltKey => $fltValue) {
				...
				if (
					...
					if (
						$this->filters[$fltKey]['operator'] == 'range'
						|| (
							(str_starts_with((string) $fltKey, 'from_'))
							|| (str_starts_with((string) $fltKey, 'to__'))
						)
					) {
						if (
							stripos((string) $fltKey,'__FROM') !== false
							|| str_starts_with((string) $fltKey, 'from__')
						) {
							$op = '>=';
						}

						if (
							stripos((string) $fltKey,'__TO') !== false
							|| str_starts_with((string) $fltKey, 'to__')
						) {
							$op = '<=';
						}
		...
		if (
			(isset($this->tableSections['limit']))
			&& (intval($this->tableSections['limit']) > 0)
		) {
	...
	function getGridTable(&$o, $sql, array $show, $gTable='',string $extra=''): string
	{
		...
		if ($rs) {
			...
			foreach ($this->tableFields as $key => $value) {
				if ($this->tableFields[$key]['type'] != 'hidden') {
					if ($show[$key] === true) {
						if (
							$this->tableFields[$key]['format'] == 'number'
							|| $this->tableFields[$key]['format'] == 'numeric'
							|| $this->tableFields[$key]['format'] == 'integer'
						) {
			...
			foreach ($rs as $row) {
				...
				foreach ($row as $key => $value) {
					if (
						(!is_numeric($key))
						&& ($this->tableFields[$key]['type'] != 'hidden')
					) {
						...
						if ($show[$key] === true) {
							if (
								($this->tableFields[$key]['type'] == 'textarea')
								|| ($this->tableFields[$key]['type'] == 'memo')
								|| ($this->tableFields[$key]['type'] == 'code')
							) {
								if (strlen($value) > 60) {
								...
								if ($fileLink != "") {
									if (stripos($fileLink,'mpeg') !== false) {
										...
										stripos($fileLink,'jpg') !== false
										|| stripos($fileLink,'gif') !== false
										|| stripos($fileLink,'png') !== false
									) {
	 ...
	 */
	function showGrid(&$o, $sql, $show, $gTable='', string $gFkName='id', $gFk=0, ?string $flatTable=''): string
	{
			...
			if ($this->filter) {
				...
				foreach ($this->filters as $fltKey=>$fltValue) {
					...
					if (
						...
						&& (
							(str_contains((string) $this->tableFields[$fltKey]['type'],'text'))
							|| ($this->tableFields[$fltKey]['type'] == 'combo')
							|| ($this->tableFields[$fltKey]['type'] == 'comboMultiSelection')
						)
					) {
						$filtra = false;
					}
				...
				if (is_array($fltMsg) && $this->gCmd != 'ajax' && method_exists($o, 'p')) {
	...
	function showForm(&$o): string
	{
		...
		if ($id != 0) {
			...
			foreach ($this->tableFields as $key => $arr) {
				$value = $rs[0][$key];
				if (
					($this->tableFields[$key]['type'] != 'checkbox')
					&& ($this->tableFields[$key]['type'] != 'combo')
				) {
					$value = formatValueByType($key, $this->tableFields[$key]['type'], $value, $this->combos);
				}
				...
				if ($this->tableFields[$key]['type'] == 'file') {
					...
					if ($fileLink != "") {
						...
						if (
							$this->tableFields[$key]['value'] == 'audio/mpeg'
							|| $this->tableFields[$key]['value'] == 'audio/wav'
						) {
							...
							$this->tableFields[$key]['value'] == 'image/jpeg'
							|| $this->tableFields[$key]['value'] == 'image/png'
							|| $this->tableFields[$key]['value'] == 'image/gif'
						) {
		...
		if ($gTable == '' && is_array($this->tables)) {
			foreach ($this->tables as $table) {
		...
		foreach ($fields as $key => $value) {
			if (
				...
				|| (
					$value['type'] == 'html'
					&& str_ends_with((string) $value['name'], 'view')
				)
			) {
	 ...
	 *
	 */
	function showAddButton($page, $master, &$o, string $gTable='', string $gFk=''): string
	{
		$fez=false;
	...
	function run( &$o, &$html ): void
	{
		...
		if ( $this->gCmd == "ajax" ) {
			...
			$this->gParam = json_decode((string) $_REQUEST['gParam']);
			...
			*/
			case ($this->gUI_FILTER):
				...
				foreach ($this->filters as $key => $fltvalue) {
					...
					$origKey = '';
					if (stripos((string) $key,"__FROM") !== false) {
						$origKey = $key;
						$key = str_ireplace('__FROM','',$key);
						$fieldLabel=$fieldLabel.' '.gT('de');
					}

					if (stripos((string) $key,"__TO") !== false) {
						$origKey = $key;
					...
					if (str_starts_with((string) $key, 'from__')) {
						$origKey = $key;
						$key = substr((string) $key, 6);
					...
					if (str_starts_with((string) $key, 'to__')) {
						$origKey = $key;
						$key = substr((string) $key,4);
						$fieldLabel = $this->filters['fieldLabel'] != "" ? $this->filters['fieldLabel'] : $fieldLabel.' '.gT('de');
					}
					...
					if (str_contains((string) $key, '__')){
						...
						$key = substr((string) $key, strpos((string) $key, '__')+2);
						$fieldLabel = $fltvalue['fieldLabel'];
					}

					$value = $this->tableFields[$key];
					if ($gFWMute) {
						$tmpFilter = $origKey != "" ? $this->filters[$origKey] : $this->filters[$key];
			...
			case ($this->gUI_SAVE):
				...
				foreach ($this->tableFields as $key=>$arr) {
					if ($gTable == '' || $arr['name'] != $gFkName) {
						$value = $arr['type'] != 'code' ? gCleanField($_REQUEST[$key]) : $_REQUEST[$key];
						...
						if (
							...
							if ($arr['type'] == 'file') {
								$t = $_FILES[$key]['type'];
								if ($t == '') {
									$t = strtolower(substr((string) $_FILES[$key]['name'],strpos((string) $_FILES[$key]['name'],".")));
								}
						...
						} elseif (($arr['type']=='dateTime') || ($arr['type']=='date') || ($arr['type']=='show')) {
							unset($fields[$key]);
						} elseif (($arr['type']=="integer") || ($arr['type']=="number")) {
							$fields[$key]=0;
						} else {
							$fields[$key]='';
						}
				...
				foreach ($anexos as $nomeDoAnexo => $tipo) {
					if (trim((string) $tipo) !== "") {
						$anexosNomes[$nomeDoAnexo] = trim($usrId.'-'.random_int(10000,99999).'.'.substr((string) $tipo,strpos((string) $tipo,'/')+1));
				...
				if ($uploadOk) {
						...
						foreach ($this->tables as $table) {
							if ($table['relationship'] == 'one-to-one') {
								...
								foreach ($this->tableFields as $key=>$arr) {
									...
									$value = $arr['type'] != 'code' ? gCleanField($_REQUEST[$nkey]) : $_REQUEST[$nkey];
									...
									if ((!empty($_REQUEST[$nkey])) || ($arr['type']=='checkbox') || ($arr['type']=='password')) {
										...
										unset($fields[$key]);
									} elseif ((!isset($fields[$key])) && (!is_numeric($key))) {
										if (($arr['type'] == "integer") || ($arr['type'] == "number")) {
											...
											if (($arr['type'] == 'file') && (trim((string) $_FILES[$nkey]['type']) === '')) {
												unset($fields[$key]);
											}
										}
									}
								...
								foreach ($anexos as $nomeDoAnexo => $tipo) {
									if (trim((string) $tipo) !== "") {
										$novoNome = trim($nId.'.'.substr((string) $tipo,strpos((string) $tipo,'/')+1));
					...
					if ($gTable == '') {
						...
						} elseif ((!$novoRegistro) && ($gLastPage > 0)) {
							redirect($o->page."&gPage=".$gLastPage."&gId=".$id);
						} elseif (!$novoRegistro) {
							redirect($o->page."&gPage=".$this->gUI_FORM."&gId=".$id);
						} elseif (is_array($this->tables)) {
...
class gUIMD extends gUICore
{
	 ...
	 */
	function frame($key, $value): string
	{
		global $o;
		if ($value == "") {
			$value='&nbsp;';
					...
					foreach ($_REQUEST as $key => $value) {
						if (
							...
							switch ($type)
							{
								...
								default:
									if (!str_starts_with($key, 'id_') || $value != '0') {
										$flt[$key]=$name."='".formatDBValueByType($type,$value)."'";
									}
									break;
							}
				...
				case 2:
					...
					if (is_array($this->filters)) {
						foreach ($this->filters as $flt) {
							$extra = '';
							$name = $flt['name'];
							$fieldLabel = $flt['fieldLabel'] ?? gField2String($name);
							$type = $flt['type'];
							$value = $flt['value'];
							if ($type == 'combo') {
