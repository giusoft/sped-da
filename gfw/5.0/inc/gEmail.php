 ...
 */
function gSendEmail($remetente, $para, $cc, $assunto, $conteudo, $anexo = "", $site = "", $nome_remetente = '')
{
    $sai = true;
	$sep = ",";
    if (strpos($para, ";") > 0) {
    ...
    if (
        (gVar("global.smtpserver") . gVar("smtp.server") === "")
        || (gVar("global.smtpserver") == "localhost")
        || (gVar("smtp.server") == "localhost")
    ) {
        if ($remetente == "") {
        ...
        if ($para != "") {
            for ($j = 0; $j < $Qemailpara; $j++) {
                ...
                $sai = mail($VETemailpara[$j], $assunto, $conteudo, "From: $remetente\nReply-To: $remetente\nX-Mailer: PHP/" . phpversion(), "-f" . $remetente);
            }
        }
        ...
        if ($cc != "") {
            for ($j = 0; $j < $Qemailcopia; $j++) {
                ...
                $sai = mail($VETemailcopia[$j], $assunto, $conteudo, "From: $remetente\nReply-To: $remetente\nX-Mailer: PHP/" . phpversion(), "-f" . $remetente);
            }
        }
    } else {
        $mail = new PHPMailer();
        ...
        if (gVar("global.smtpserver") != "") {
            ...
            $mail->FromName = $nome_remetente ?: gVar("global.smtpfromname");
        ...
        if ($para != "") {
            for ($j = 0; $j < $Qemailpara; $j++) {
                $faz = true;
                for ($k = 0; $k < $j; $k++) {
                    if ($VETemailpara[$k] === $VETemailpara[$j]) {
                        $faz = false;
                    }
                }
        ...
        if (($cc != "") && (!is_array($cc))) {
            for ($j = 0; $j < $Qemailcopia; $j++) {
                ...
                for ($k = 0; $k < $Qemailpara; $k++) {
                    if ($VETemailpara[$k] === $VETemailcopia[$j]) {
                        $faz = false;
                    }
                }

                for ($k = 0; $k < $j; $k++) {
                    if ($VETemailcopia[$k] === $VETemailcopia[$j]) {
                        $faz = false;
                    }
                }
        ...
        if (is_array($cc)) {
        	if (is_array($cc['cc'])) {
        		foreach ($cc['cc'] as $ccMail ) {
        			if (strpos((string) $ccMail,"@") > 0) {
        				$mail->AddCC($ccMail);
        				gLog("Enviando cópia de e-mail para " . $ccMail . ": '$assunto'");
        			}
        		}
        	}

        	if (is_array($cc['cco'])) {
        		foreach ($cc['cco'] as $ccoMail ) {
        			if (strpos((string) $ccoMail,"@") > 0) {
        				$mail->AddBCC($ccoMail);
        				gLog("Enviando cópia oculta de e-mail para " . $ccoMail . ": '$assunto'");
        			}
        		}
        	}
