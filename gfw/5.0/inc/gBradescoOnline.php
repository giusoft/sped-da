...
class BradescoOnline
{
	 ...
	 */
	public $BD = ["host" => "127.0.0.1", "usuario" => "web", "senha" => "web", "bd" => "gadmin"];
	 ...
	 */
	private array $url = ["homologacao" => "https://cobranca.bradesconetempresa.b.br/ibpjregistrotitulows/registrotitulohomologacao", "producao" => "https://cobranca.bradesconetempresa.b.br/ibpjregistrotitulows/registrotitulo"];

	/**
	 * Informações do certificado digital A1
	 */
	private array $dadosCertificado = ["pathPfx" => "", "password"=>"", "priKey" => "", "pubKey"=>"", "certKey"=>""];
	 ...
	 */
	public function validaCertificado($dadosCertificado): void
	{
		...
		$pathToFile = dirname((string) $dadosCertificado['pathPfx']);
		$nameFile   = basename((string) $dadosCertificado['pathPfx']);
		$flagNovo   = false;
	 ...
	 */
	public function dadosBoleto(array $dados): void
	{
		$dadosBoleto                                                           = [];
		$dadosBoleto['nuCPFCNPJ']                                              = substr((string) $this->dadosBeneficiario['cnpj'],0,8);
		$dadosBoleto['filialCPFCNPJ']                                          = substr((string) $this->dadosBeneficiario['cnpj'],8,4);
		// Filial CNPJ (4) *
		$dadosBoleto['ctrlCPFCNPJ']                                            = substr((string) $this->dadosBeneficiario['cnpj'],12,2); // Dígito de controle CNPJ (2) *
		 ...
		 */
		$dadosBoleto['nuNegociacao']                                           = str_pad((string) $this->dadosBeneficiario['agencia'],2,"0",STR_PAD_LEFT).'0000000'.str_pad((string) $this->dadosBeneficiario['conta'],7,"0",STR_PAD_LEFT);// Número da Negociação Formato: Agencia: 4 posições(Sem digito) Zeros: 7 posições Conta: 7 posições (Sem digito) (18) *
		...
		$dadosBoleto['controleParticipante']                                   = $this->limpaString(substr((string) $dados['controleParticipante'],0,25));
		 ...
		 */
		$dadosBoleto['nomePagador']                                            = $this->limpaString(substr((string) $dados['nomePagador'],0,70));
		$dadosBoleto['logradouroPagador']                                      = $this->limpaString(substr((string) $dados['logradouroPagador'],0,40));
		$dadosBoleto['nuLogradouroPagador']                                    = $this->limpaString(substr((string) $dados['nuLogradouroPagador'],0,10));
		/**
		 * Expresão regular para remover caracters invalidos
		 */
		$dadosBoleto['complementoLogradouroPagador']                           = preg_match("/^[a-zA-Z\d]+$/", $this->limpaString(substr((string) $dados['complementoLogradouroPagador'],0,15)));

		$cep                                                                   = $this->soNumeros($dados['cepPagador']);
		$dadosBoleto['cepPagador']                                             = $this->soNumeros(substr((string) $cep,0,5));
		$dadosBoleto['complementoCepPagador']                                  = $this->soNumeros(substr((string) $cep,5,3));

		$dadosBoleto['bairroPagador']                                          = $this->limpaString(substr((string) $dados['bairroPagador'],0,40));
		$dadosBoleto['municipioPagador']                                       = $this->limpaString(substr((string) $dados['municipioPagador'],0,30));
		$dadosBoleto['ufPagador']                                              = $this->limpaString(substr((string) $dados['ufPagador'],0,2));
		/**
		* cdIndCpfcnpjPagador
		* 1 = CPF
		* 2 = CNPJ
		*/
		if ( !strlen((string) $this->soNumeros($dados['nuCpfcnpjPagador'])) == 14 ) {
			$dadosBoleto['cdIndCpfcnpjPagador']                                = "1";
			$dadosBoleto['nuCpfcnpjPagador']                                   = str_pad((string) $this->soNumeros($dados['nuCpfcnpjPagador']),14,"0",STR_PAD_LEFT);
		} else{
			$dadosBoleto['cdIndCpfcnpjPagador']                                = "2";
			$dadosBoleto['nuCpfcnpjPagador']                                   = str_pad((string) $this->soNumeros($dados['nuCpfcnpjPagador']), 14,"0",STR_PAD_LEFT);
		}

		$dadosBoleto['endEletronicoPagador']                                   = $this->limpaString(substr((string) $dados['endEletronicoPagador'],0,70));
		$dadosBoleto['nomeSacadorAvalista']                                    = $this->limpaString(substr((string) $dados['nomeSacadorAvalista'],0,40));
		$dadosBoleto['logradouroSacadorAvalista']                              = $this->limpaString(substr((string) $dados['logradouroSacadorAvalista'],0,40));
		$dadosBoleto['nuLogradouroSacadorAvalista']                            = $this->limpaString(substr((string) $dados['nuLogradouroSacadorAvalista'],0,10));
		$dadosBoleto['complementoLogradouroSacadorAvalista']                   = $this->limpaString(substr((string) $dados['complementoLogradouroSacadorAvalista'],0,15));

		$cep = $this->soNumeros($dados['cepSacadorAvalista']);
		$dadosBoleto['cepSacadorAvalista']                                     = $this->soNumeros(substr((string) $cep,0,5)) == "" ? "0" : $this->soNumeros(substr((string) $cep,0,5));
		$dadosBoleto['complementoCepSacadorAvalista']                          = $this->soNumeros(substr((string) $cep,5,3))  == "" ? "0" : $this->soNumeros(substr((string) $cep,5,3));

		$dadosBoleto['bairroSacadorAvalista']                                  = $this->limpaString(substr((string) $dados['bairroSacadorAvalista'],0,40));	
		$dadosBoleto['municipioSacadorAvalista']                               = $this->limpaString(substr((string) $dados['municipioSacadorAvalista'],0,40));	
		$dadosBoleto['ufSacadorAvalista']                                      = $this->limpaString(substr((string) $dados['ufSacadorAvalista'],0,2));
		/**
		* nuCpfcnpjSacadorAvalista
		* 1 = CPF
		* 2 = CNPJ
		*/
		if (strlen((string) $this->soNumeros($dadosBoleto['nuCpfcnpjSacadorAvalista']))  == 14) {
		...
		} elseif(strlen((string) $this->soNumeros($dadosBoleto['nuCpfcnpjSacadorAvalista'])) > 0 ){
			$dadosBoleto['cdIndCpfcnpjSacadorAvalista']                        = "1";
			$dadosBoleto['nuCpfcnpjSacadorAvalista']                           = str_pad((string) $this->soNumeros($dados['nuCpfcnpjSacadorAvalista']), 14,0,STR_PAD_LEFT);
		} else {
			$dadosBoleto['cdIndCpfcnpjSacadorAvalista']                        = "0";
			$dadosBoleto['nuCpfcnpjSacadorAvalista']                       	   = "0";
		}

		$dadosBoleto['endEletronicoSacadorAvalista']                           = $this->limpaString(substr((string) $dados['endEletronicoSacadorAvalista'],0,70));

		$this->json = json_encode($dadosBoleto, JSON_UNESCAPED_UNICODE);
	}


	public function assinaBoleto(): void
	{
		$result = "";
		$file   = "/tmp/".uniqid();
		$file2  = "/tmp/".uniqid();
	...
	public function enviaBoleto()
	{
		...
		if ($this->json != "") {
		    ...
		    curl_setopt($ch, CURLOPT_HTTPHEADER, ["Content-Type: application/x-pkcs7-mime;", "Content-Transfer-Encoding: base64"]);

		    $xml = curl_exec($ch);
		   	$xml = str_replace(['<?xml version="1.0" encoding="UTF-8"?>'],"",$xml);
			if ($xml) {
		    	$ret = preg_replace("/\r?\n/","", $xml);
		    	$lenN = strlen((string) $ret);
		    	$posX = stripos((string) $ret, "<return>");
		    	$posY = stripos((string) $ret, "</return>");
		    	$return = json_decode(substr((string) $ret,$posX+8,$posY-$lenN));
	...
	* @param  float $val
	*/
	public function formataPercentual($val): string
	{
		if (!$val > 0) {
			$tmp = 0;
		}
	...
	*/
	public function soNumeros($str): string|array|null {
     ...
     */
	private function limpaString($texto): string
    {
        $aFind = [
					'&', 'á', 'à', 'ã', 'â', 'é', 'ê', 'í', 'ó',
					'ô', 'õ', 'ú', 'ü', 'ç', 'Á', 'À', 'Ã', 'Â',
     ...
     */
    public function listaErro($codigo): string
    {
    	$erros = [];
	...
	private function motivo_ocorrencia($id): string
	{
		$ocorrencias = [];
	...
	public function sendMail($email): bool{
		...
		if (
			!is_array($email)
			|| !filter_var($email['remetente'], FILTER_VALIDATE_EMAIL)
			|| !filter_var($email['destinatario'], FILTER_VALIDATE_EMAIL)
		) {
			return false;
		}
	...
	public function gerarBoleto($id)
	{
		...
		if (count($result) >= 1) {
			...
			$data = json_decode((string) $result['json'],true);
			 ...
			 * 5 - Agencia
			 */
			$this->layoutboleto['agencia'] = substr((string) $data['negociacao'],0,4);
			...
			$this->layoutboleto['conta'] = substr((string) $data['negociacao'],-7);
			...
			if ($data['vlJuros'] != '0' || $data['vlJuros'] != '') {
				...
				if ($data['cdValorJuros'] == '1') {
					 ...
					 * @var string
					 */
					$valor = str_pad(ltrim((string) $data['vlJuros'],'0'),3,'0', STR_PAD_LEFT);

					$valorJuros .= "R$ ".substr((string) $data['vlJuros'],0,-2).",".substr((string) $data['vlJuros'],-2)." ao dia.";
				} elseif ($data['cdValorJuros'] == "2") { // Juros percentuais ao mês
					/**
					 ...
					 */
					if (strlen(substr((string) $valorPrevio, 0, 3)) == 3) {
						$valorInteiro = ltrim(substr((string) $valorPrevio, 0,3),'0');
						/**
						 * Verifica se existe os cinco posteriores algoritmos dos juros, que serão os decimais
						 */
						if (strlen(substr((string) $valorPrevio, 3,5)) == 5) {
							$valorDecimal = substr((string) $valorPrevio, 3, 5);
						} else { // Caso não haja números suficientes, os decimais que serão complementados com zero, e não os inteiros
							$valorDecimal = str_pad(substr((string) $valorPrevio, 3,5), 5, '0', STR_PAD_RIGHT);
						}
						/**
						 ...
						 * @var [string]
						 */
						$valorInteiro = ltrim((string) $valorPrevio,'0') === '' ? '0' : ltrim((string) $valorPrevio,'0');
	 ...
	 */
	public function gerarPDF($out, $path, $visualizar=false): bool
	{
		/**
	...
	public function processarRetorno($path='retorno.txt')
	{
		...
		$transacoes = [];
		foreach ($arquivo_retorno as $value) {
			# code...
			if ($value[0] === '1') {
				$transacoes[] = $value;
			}
	...
	public function obterDadosTransacoes($transacoes = []): array
	{
		...
		foreach ($transacoes as $value) {
			...
			$data = [];
			$data['id_registro'] = substr((string) $value, 0, 1);
			$data['inscricao_empresa'] = substr((string) $value, 1, 2);
			$data['id_empresa'] = substr((string) $value,3, 14);
			$data['id_beneficiario_banco'] = substr((string) $value, 20,17);
			$data['controle_participante'] = substr((string) $value, 37, 25);
			$data['id_titulo_banco'] = substr((string) $value, 70, 12);
			$data['parcial'] = substr((string) $value, 105,2);
			$data['carteira'] = substr((string) $value, 107,1);
			$data['id_ocorrencia'] = substr((string) $value, 108,2); //Fazer o tratamento deste valor
			$data['motivo_ocorrencia'] = $this->motivo_ocorrencia($data['id_ocorrencia']);
			$data['data_ocorencia'] =  DateTime::createFromFormat('dmy', substr((string) $value, 110, 6));
			$data['numero_documento'] = substr((string) $value, 126, 20);
			$data['data_titulo'] = DateTime::createFromFormat('dmy', substr((string) $value, 146, 6));
			$data['valor_titulo'] = number_format(substr((string) $value, 152, 13),2,',','');
			$data['despesas_ocorrencia'] = number_format(substr((string) $value, 175, 13), 2,',','');
			$data['desconto_concedido'] = number_format(substr((string) $value, 240, 13),2,',','');
			$data['valor_pago'] = number_format(substr((string) $value, 253, 13),2,',','.');
			$data['juros_mora'] = number_format(substr((string) $value, 266, 13),2,',','');
			$data['motivo_codigo_ocorrencia'] = substr((string) $value, 294, 1);
			$data['data_credito'] = DateTime::createFromFormat('dmy', substr((string) $value, 295, 6));
			$data['motivo_rejeicao'] = substr((string) $value, 318, 10);
			$data['numero_cartorio'] = substr((string) $value, 368, 2);
			$data['numero_registro'] = substr((string) $value, 394, 6);
...
$dadosCertificado = ["pathPfx" => "/var/www/html/certificados/giusoft.pfx", "password" => "giusoft"];
$dadosBeneficiario = [];
