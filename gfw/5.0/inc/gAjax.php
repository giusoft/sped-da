...
class gAjax
{
	...
	public function requestData($func, $e,$param)
	{
		$args = ['e' => $e] + $param;
	 ...
	 *
	 */
	public function comboDynamicValues($source,$target,$function,$otherParameters): void
	{
        $otherParameters = $otherParameters != "" ? explode(",",(string) $otherParameters) : "" ;
		$this->combo[] = [
							'uid' => uniqid(),
							'source_id' => $source,
							'target_id' => $target,
				...
				$script .= '
					$(document).ready(function() {
						$("#'.$combo['source_id'].'").change(function() {
							...
							var json = "";';
				if (is_array($combo['other_parameters']) && $combo['other_parameters']) {
					$script .= "json ='{'; ";
