...
class gHTMLChart extends gOutput
{
    ...
    public function setDimensions($w, $h)
    {
        ...
        $w = (str_ends_with((string) $w, '%'))? $w : $w.'px';
        $h = (str_ends_with((string) $h, '%'))? $h : $h.'px';
     ...
     * @return boolean true ou false
     */
    public function render(&$o,$print = true)
    {
        global $http_lib;

        if (empty($this->y_label) && empty($this->xy_list)) {
            return false;
        }
        ...
        if ($param) {
            foreach($param as $k => $v){
                $script .= ','.$k.': '.(str_contains((string) $v, 'new Array')? $v : '"'.$v.'"');
            }
        }
        ...
        if ($type != 'Donut') {
            ...
            $script .= "] ,xkey: 'x'";
            $script .= (count($klabel)) ? " , ykeys: ['".implode("','",array_keys($ks))."']" : "";
            $script .= (count($y_label)) ? " ,labels: ['".implode("','",$y_label)."']" : "";
...
class gPDFChart
{
     ...
     * @param string
     */
    public function __construct($type = 'Bar', $width = '100%', $height = '350')
    {
        $this->setType($type);
        $this->width  = $width;
        $this->height = $height;
     ...
     * @return boolean true ou false
     */
    public function setDimensions($w, $h): bool
    {
        if(empty($w) && empty($h)) {
            return false;
        }
     ...
     * @return boolean true ou false
     */
    public function setYlabel(...$args): bool
    {
     ...
     * @return boolean true ou false
     */
    public function setXlabels($txt): bool
    {
        $this->x_labels = $txt;
        return true;
    }
     ...
     * @return boolean true ou false
     */
    public function addAssocXY(): bool
    {
        $argl[]='a';
        $argl[]='b';
        $argl[]='c';
     ...
     * @return boolean true ou false
     */
    public function render(&$o,$print = true): void
    {
        ...
        switch ($this->type) {
            case 'Bar':
                ...
                $o->ColumnChart($w, $h, $data, null, [100, 100, 100],0,4,$this->y_label);
            ...
            case 'Donut':
                ...
                $o->SetX(16);
                $h = intval(50*intval($this->height)/350);
                $o->PieChart($w, $h, $data, '%l (%p)', [100, 100, 100]);
