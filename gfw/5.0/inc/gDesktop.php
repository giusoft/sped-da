...
function addMenuItem($json): void
{
	$mtz = cssDecode($json);
	$this->menuItens[] = [$mtz];
}
...
class gDesktop extends gInput
{
		...
		if ($_GET['t'] != "") {
			...
			if (
				(
					...
					&& $firstTry < (intval(date("H") * 3600 + date("i") * 60 + date("s")) - 60)
				)
				|| $firstTry == ""
			) {
				$cnt = 0;
	...
	function showBlocked($json): void
	{
		global $http_img;
		$jarr = cssDecode($json);
		$error = $jarr['error'];
		...
		$mais="";
		if (
			(gVar("global.site") != ".oO Alitem")
			&& (gVar("global.site") != "Alitem")
		) {
			$mais=",{
					xtype: 'label',
	...
	function showLogin($json=""): void
	{
		global $gDevice,$gOs,$http_img;

		$mtz = cssDecode($json);
		$url = trim((string) $mtz['url']);
		$name = $mtz['name'];
		$title = $mtz['title'];
		$error = $mtz['error'];
		if ($url === "") {
	...
	function loginCheck()
	{
		return ($_POST['login']=='test') && ($_POST['password']=='test');
 	}
	...
	function showDesktop(): void
	{
		...
		if ($_SESSION['gFW_MENU']=='')
		{
			...
			if (gVar("global.site")=="Intermarítima") {
				$extra=" :: ( ".strtoupper((string) $_SESSION['nomeArmazem']).") ";
			}
