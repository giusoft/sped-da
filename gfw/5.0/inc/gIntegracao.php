...
class rj_examesTeoricos {
	...
	function verificaConectividade()
	{
		...
		} elseif (stripos($this->browser->html, 'Agendar exame')!==false) {
			$sai = true;
			$this->errorCode = 0;
			$this->errorMessage = '';
		} else {
	...
	function buscarIdLocalExame($id_geral_tipos_exames, $descricao, $endereco='', $bairro='', $codigo='')
	{
		...
		$sql = "SELECT * FROM geral_locais_exames
				WHERE id_geral_tipos_exames = $id_geral_tipos_exames {$where}";
		$rs = dbQuery($sql);
		if ($rs) {
			$sai = $rs[0]['id'];
...
class rj_examesPraticos {
	...
	function verificaConectividade()
	{
		...
		if ($this->browser->errorCode != '') {
			...
			$this->errorCode = $this->browser->errorCode;
			$this->errorMessage = $this->browser->errorMessage;
		} elseif (stripos($this->browser->html, 'NatLogon.html') !== false) {
			$sai = true;
			$this->errorCode = 0;
			$this->errorMessage = '';
		} else {
...
class interprint
{
			...
			if ($this->verificaConectividade()) {
				switch ($this->estado)
				{
					...
					case 'ce':
						...
						if (str_contains($this->browser->html, 'Usuário')) {
							$sai = true;
							$this->browser->getHtmlObject(true);
		...
		switch ($this->estado)
		{
			case 'rj':
				foreach ($this->agendaBruta as $row) {
					...
					$row2['curso'] = $row[3];
					$row2['data'] = $this->browser->format($row[1],'date').' '.substr((string) $this->browser->format($row[2],'string'),0,5).':00';
					$row2['erro_biometria'] = 0;
					if (
						(str_contains((string) $row[7],'ok.jpg'))
						&& (str_contains((string) $row[9],'ok.jpg'))
					) {
						$row2['presenca'] = $row2['data'];
						$row2['hora_entrada'] = substr((string) $row[2],0,5);
						$row2['hora_saida'] = substr((string) $row[2],8,5);
					} else {
						if (
							(str_contains((string) $row[7],'ok.jpg'))
							|| (str_contains((string) $row[8],'ok.jpg'))
							|| (str_contains((string) $row[9],'ok.jpg'))
						) {
				...
				foreach ($this->agendaBruta as $row) {
					...
					$aulaData = $this->browser->format($row[1],'date').' '.substr((string) $this->browser->format($row[2],'string'),0,5).':00';
					$aulaPresente = false;
					...
					if (
						(str_contains((string) $row[ 7],'ok.jpg'))
						&& (str_contains((string) $row[ 9],'ok.jpg') || $row[8] == '')
						&& (str_contains((string) $row[11],'ok.jpg'))
					) {
						$aulaPresente = true;
						...
						if ($aulaPresente) {
							if ($a == 0) {  // primeira aula, então a presença é o horário real registrado pelo aluno
								...
								if ($this->browser->format($row[8],'date') == 'Presença não registrada') {
									...
									$row2['presenca'] = substr((string) $this->browser->format($row[8],'date'),0,16).':00';
								}
							} else {
								$row2['presenca'] = $aulaData;
							}
							...
							if ($a == $ttlAulas-1) {
								...
								if ($this->browser->format($row[8],'date') == 'Presença não registrada') {
									$row2['hora_saida'] = substr($aulaDataFinal, 11,5);
   									...
   									$row2['hora_saida'] = substr((string) $this->browser->format($row[$pos],'date'),11,5);
	 ...
	 */
	function processaInstrutoresId(): void
	{
	 ...
	 * Processa dados dos instrutores obtidos e retorna array com valores na propriedade da Classe
	 */
	function processaInstrutoresCpf(): void
	{
		jLog('INS Processando instrutores', $this->browser->session, 'aviso');
	 ...
	 */
	function processaServicos(): void
	{
	 ...
	 */
	function processaDisciplinas(): void
	{
				...
				if (is_object($select))
				{
					...
					if (is_object($label))
					{
						for ($i = 0; $i < $label->length; $i++) {
							$for        = explode('_',(string) $label->item($i)->getAttribute('for'));
	...
	function processaCadastroCandidato(): void
	{
	...
	function buscaCadastroCandidato($json)
	{
		...
		if ($sai) {
			...
			// Busca imagem e Rio de Janeiro não tem foto
			if ($id > 0 && $this->estado != 'rj') {
	...
	function converteHora($data, $dataInicial): string|int|float|false|null
	{
		// 0000-00-00 00:00:00
		// rptHorarios_ctl10_div0
		// rptHorarios$ctl20$CheckBox0
		$hora = substr((string) $data,11,5);
	...
	function buscaAgendaAluno($json, $datas)
	{
		...
		$uf = strtoupper((string) $this->estado);
		...
		switch ($this->estado)
		{
			...
			case 'rj':
				...
				if ($this->login()) {
					$dataBR = date('d/m/Y', strtotime((string) $data));
					$hora = substr((string) $data,11,5);
					...
					if ($fez) {
						...
						$sai = true;
						if (!str_contains($this->browser->html, 'Agendamento realizado com sucesso')) {
							$sai = false;
	...
	function agendaDisciplina($json)
	{
		...
		switch ($this->estado)
		{
			...
			case 'rj':
				...
				if ($this->login()) {
					$dataBR = date('d/m/Y', strtotime((string) $dataInicio));
					$horaInicio = substr((string) $dataInicio,11,5);
					$horaIntervalo = '';
					...
					switch ($total)
					{
						case 1:
							$horaIntervalo = '';
							$horaTermino = substr(date('Y-m-d H:i:s', strtotime('+'.calculaDuracao($horaInicio).' minute', strtotime((string) $dataInicio))),11,5);
							break;
						case 2:
							$horaIntervalo = '';
							$horaTermino = substr(date('Y-m-d H:i:s', strtotime('+'.calculaDuracao($horaInicio).' minute', strtotime((string) $dataInicio))),11,5);
					...
					if (str_contains($this->browser->html, 'Ocorreu um erro durante o processamento')) {
						$sai = false;
						$this->errorCode = '605';
						...
						if (str_contains($this->browser->html, 'Ocorreu um erro durante o processamento')) {
							...
							foreach ($matches[0] as $match)
							{
								...
								if (count($campos)>1) {
									...
									// Teórico
									if ($campoTipo === 'T' && $campoHora == $horaInicioInt) {
	...
	function agendaAulaAluno_fase1($dataBR, $periodo, $id_servico, $p_t, $id_curso): void
	{
		$this->browser->get($this->urls[$this->estado]['agenda_grade']);
		$this->browser->getHtmlObject(true);
		$this->browser->post['CFCMaster$cphBody$Acord_AccordionExtender_ClientState'] = '0';
	...
	function agendaAulaAluno($aulas, $p_t='T')
	{
		...
		$uf = strtoupper($this->estado);
		$txtEstado = 'Txt'.strtoupper((string) $this->estado);
...
// 								}
								...
								$horaInicio   = substr((string) $dataInicio,11,5);
								$periodo      = ($horaInicio<'12:00') ? 'M' : ($horaInicio>=HORA_LIMITE_AULAS_50_MINUTOS ? 'N' : 'T');
								$horaTermino  = substr(date('Y-m-d H:i:s', strtotime('+50 minute', strtotime((string) $dataInicio))),11,5);
								...
								if ($this->estado == 'ba') {
									foreach ($aulas as $aulaKey=>$aula) {
										$diferencaEmSegundos = strtotime((string) $aula['data']) - strtotime((string) $dataInicio);
										$dia = floor($diferencaEmSegundos / (60 * 60 * 24));
										$hora = substr((string) $aula['data'],11,5);
										...
										if (isset($checkBoxes[$hora])) {
											...
											$hora = str_pad(intval(substr((string) $aula['data'],11,2)-5),2,'0', STR_PAD_LEFT);
											...
											if (!isset($this->browser->post[$cb])) {
												...
												$aula['hora_para'] = substr((string) $aula['data'],11,5);
												$aula['checkbox']  = $cb;
												$dePara[]          = $aula;
											}
										}
									...
									foreach ($aulas as $aulaKey => $aula) {
										...
										if ($dia < 7) {
											...
											foreach ($minutos[$minInicial] as $minuto) {
												if ($minuto['hora'] === substr((string) $aula['data'],11,5)) {
								...
								if (str_contains($this->browser->html, 'Ocorreu um erro durante o processamento')) {
									...
									if (str_contains((string) $msg,'com sucesso')) {
										$sai = true;
										$this->browser->getHtmlObject(true);
										$this->browser->post['__EVENTTARGET'] = '';
										$this->browser->post['__EVENTARGUMENT'] = '';
						...
						if (str_contains($this->browser->html, 'Ocorreu um erro durante o processamento')) {
							...
							foreach ($matches[0] as $match) {
								...
								if (count($campos) > 1) {
									...
									// Teórico
									if ($campoTipo === $p_t && $campoHora == $horaInicioInt) {
										$idGradeCurso = substr($campoIds[0],1);
										$idInstrutor = $campoIds[1];
										jLog("Checando novamente $idInstrutor = $id_instrutor", $this->browser->session);
										if ($idInstrutor == $id_instrutor) {
							...
							if ($fez)
							{
								...
								$this->browser->post['txtCPF'] = $cpf;
								if ($this->estado == 'rj') {
									$this->browser->post['uf'] = strtoupper((string) $this->estado);
									$this->browser->post[$txtEstado] = $renach;
								}
	...
	function buscaSimuladores($aulas)
	{
					...
					if (str_contains($this->browser->html, 'Ocorreu um erro durante o processamento')) {
						...
						foreach ($matches[0] as $match) {
							...
							if (count($campos) > 1) {
								...
								jLog("Checando T:$p_t H:$horaInicioInt I:$id_instrutor S:$servico C:$id_curso P:$placa = [$campoTipo] [" . $campoHora . "] [" . $campoInst . "] [" . $campoServ . "] [" . $campoCurs . "] [" . $campoPlac . "]", $this->browser->session);
								if ($campoTipo === $p_t && $servico == $campoServ && $campoHora == $horaInicioInt) {
									$idGradeCurso = substr($campoIds[0], 1);
									$idInstrutor = $campoIds[1];
									jLog("Checando novamente $idInstrutor = $id_instrutor", $this->browser->session);
									if ($idInstrutor == $id_instrutor) {
						...
						if ($fez) {
							...
							if ($this->processaSelect('ddlSimulador') && is_array($this->select)) {
								...
								foreach ($this->select as $interprint_veic) {
									...
									if (!$achou && (intval($interprint_veic['id']) && $interprint_veic['html'] != '')) {
										$fields = '';
 	...
 	function agendaAulaSimulador(array $aulas)
	{
		$sai = false;
		...
		switch ($this->estado)
		{
			...
			case 'rj':
				...
				if ($this->login()) {
					$dataBR = date('d/m/Y', strtotime((string) $dataInicio));
					$horaInicio = substr((string) $dataInicio,11,5);
					$horaTermino = substr(date('Y-m-d gi:s', strtotime('+50 minute', strtotime((string) $dataTermino))),11,5);
					$periodo = ($horaInicio<'12:00') ? 'M' : ($horaInicio>=HORA_LIMITE_AULAS_50_MINUTOS ? 'N' : 'T');
					// Fase 1
							...
							if ($campoTipo === $p_t) {
								if ($p_t === 'T') {
									...
									if (
										...
										&& $campoServ == $servico
										/*&& $campoCurs == $id_curso*/
									) { // Removendo, temporariamente, a verificacao da disciplina devido a inconsistencias na Valid
										$idGradeCurso = substr($campoIds[0],1);
										$idInstrutor = $campoIds[1];
										jLog("Checando novamente $idInstrutor = $id_instrutor", $this->browser->session);
					...
					if ($fez) {
						...
						if ($p_t === 'P') {
							...
							if ($elSel == 0) {
								...
								$fim = strpos($this->browser->html, (string) $cpf);
								if ($fim !== false) {
	...
	function processaAulasPorPeriodo($id, $placa = "")
	{
		...
		switch ($this->estado)
		{
			...
			case 'ce':
				...
				$estaAula['data_hora'] = $estaAula['data'].' '.substr((string) $estaAula['hora'],0,5).':00';
				...
				$horarios = explode(' - ', (string) $horario);
						...
						if (is_object($span)) {
							...
							if (is_array($aluno)) {
								...
								if (is_object($table2)) {
									...
									if ($this->estado == 'ba') {
										$p1 = substr((string) $this->browser->format($this->browser->htmlObject->getElementById('rptCandidatos_ctl'.str_pad($cnt, 2, '0',STR_PAD_LEFT).'_imgInicioCandidato')->getAttribute('title')),11,5);
										if ($this->tipo == 'T') {
											$ptmp = $this->browser->htmlObject->getElementById('rptCandidatos_ctl'.str_pad($cnt, 2, '0',STR_PAD_LEFT).'_imgIntervaloCandidato');
											if (is_object($ptmp)) {
												$p2 = substr((string) $this->browser->format($ptmp->getAttribute('title')),11,5);
											}
										}

										$p3 = substr((string) $this->browser->format($this->browser->htmlObject->getElementById('rptCandidatos_ctl'.str_pad($cnt, 2, '0',STR_PAD_LEFT).'_imgSaidaCandidato')->getAttribute('title')),11,5);
									} else {
									...
									if (!is_numeric(substr((string) $p1,0,2))) {
										$p1 = '';
									}
									...
									if (!is_numeric(substr((string) $p3,0,2))) {
										$p3 = '';
									}
		...
		switch ($this->estado)
		{
			...
			case 'rj':
				...
				if ($this->login()) {
					...
					$dataInicioBR = date('d/m/Y', strtotime((string) $dataInicio));
					$dataFinalBR = date('d/m/Y', strtotime((string) $dataFinal));
			...
			case 'rj':
				...
				if (is_object($table)) {
					...
					for ($i = 1; $i < $tr->length; $i++) {
						...
						$estaAula['horario'] = substr((string) $this->browser->format($td->item(1)->nodeValue),5,13);
			...
			case 'rj':
				...
				if ($this->login()) {
					...
					if ($fez) {
						...
						$url = $para_hora != '' && $this->estado != 'rj' ? $url1 : $url2;
						$this->browser->referer = $url;
						$this->browser->get($url);
						$this->browser->getHtmlObject(true);
						...
						if ($p_t === 'P') {
							...
							if ($para_hora != '') {
								...
								$this->browser->post['txtHoraTermino'] = date('H:i', strtotime('+'.calculaDuracao($para_hora).' minute', strtotime((string) $para_hora)));
							...
							if (
								!str_contains($this->browser->html,'Ocorreu um erro durante o processamento')
								&& (str_contains($this->browser->html, 'Grade foi alterada com sucesso')
								|| $lblMensagem.$lblErroVeiculo.$lblErroInstrutor === '')
							) {
								$sai = true;
							} else {
								$sai = false;
