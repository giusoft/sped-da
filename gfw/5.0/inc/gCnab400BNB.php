...
class gCnab400
{
    ...
    //      'dataDescontoAte'=> 0
    // ];
    public function addTransacoes(array $dados): void
    {
        $transacao=[];
        ...
        $transacao[] = $this->pad($this->modulo_11($this->apenasNumeros($dados['numeroControle'])),1,"N"); // DV do Nosso Número calculado pelo módulo 11. (vide nota n. 1).
    ...
    public function geraArquivo(): string{
        $this->header();
        return implode("", $this->header);
    }


    public function pad ($valor, $tamanho, $tipo): string
    {
        $sai = '';
        switch ($tipo) {
            case 'A':
                $stringLimpa = strtoupper((string) $this->tiraAcentos($valor));
            ...
            case 'N':
                $stringLimpa = strtoupper((string) $this->apenasNumeros($valor));
                $sai = substr($stringLimpa,0,$tamanho);
    ...
    public static function tiraAcentos($string): ?string
    {
        $normalizeChars = [
    ...
    public static function apenasNumeros($string): string|array|null
    {
        return preg_replace('/[^[:digit:]]/', '', (string) $string);
    }


    public function modulo_11($num, $base=9, $r=0): ?int
    {
        $soma = 0;
        $fator = 2;
