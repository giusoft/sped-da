...
class gPage extends gUI
{
	...
	function showPage($json=""): void
	{
		...
		if (
			stripos((string) $this->jarr['permissions'],'U') !== false
			|| stripos((string) $this->jarr['permissions'],'F') !== false
			|| stripos((string) $this->jarr['permissions'],'W') !== false
		) {
			$this->update=true;
		}
		...
		$o = new gInput();
		$html .= '<div id="bodyContent" data-ng-view="" style="margin: 8px">'."\n";
		parent::run($o,$html);
		$html .= '</div>'."\n";
