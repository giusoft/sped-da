...
class g_Form extends g_Stdout
{
	...
	public function render(&$output = '')
	{
		...
		$parFrm['class']  = match ($style) {
			'2column' => 'form-horizontal',
			'inline' => 'form-inline',
			default => '',
		};
		...
		for ($n = 0; $n < $contador; $n++) {
			...
			if ($fld['type'] == 'hidden') {
				...
				// Campo
				$id = trim((string) $fld['name']);
				$name = $id;
				$value = trim((string) $fld['value']);
				...
				switch($fld['type']) {
					...
					// 		$btn_title = $fld['value'] ?: gT('Selecionar Fuso Horário');
				...
				$field = $style == '2column' ? tagMe('div', $frmFld, $par) : $frmFld;
		...
		if (!empty($this->jarr['confirm'])) {
			$confirm_title = (empty($this->jarr['titleConfirm']))? 'Confirmar operação' : $this->jarr['titleConfirm'];
			$confirm_text  = (empty($this->jarr['textConfirm']))? 'Tem certeza que deseja executar este formulário?' : $this->jarr['textConfirm'];
			$confirm_button_confirm = (empty($this->jarr['btnConfirmYes']))? 'Sim' : $confirm_dialog['btnConfirmYes'];
			$confirm_button_cancel  = (empty($this->jarr['btnConfirmNo']))? 'Não' : $confirm_dialog['btnConfirmNo'];
