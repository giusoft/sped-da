...
class gMultiPage extends gInput
{
	...
	function render($fields): void
	{
		...
		if (!$this->breadcrumb && $jsn['subTitle'] != '') {
			...
			if ($this->breadcrumb) {
				...
				if ($pag > 0 && $nomeBotaoVoltar != 'no') {
					...
					if ($fields['gPageBack'] != '' && is_numeric($voltar)) {
						$fvoltar.=$o->button("{caption: '$nomeBotaoVoltar'; tag: button;}", "window.location.href=\"" . $this->page . "&gPage=" . ($pag - 1) . "&" . base64_decode((string) $fields['gPageBack']) . "\";");
					} elseif (is_numeric($voltar)) {
	 ...
	 * @version	4.0 17-12-2013 10:50
	 */
	function showMultiPage($json): void
	{
		...
		$fields = $this->onBeforeShowPage($fields);
		$this->render($fields);
	}
}
