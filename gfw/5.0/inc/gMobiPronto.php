...
class gMobiPronto
{
	...
	public $erros;
	private string $h_wsdl = '';
	private string $p_wsdl = 'http://www.mpgateway.com/v_3_00/sms/service.asmx?wsdl';
	private array|string $errors = [];
	private array $params = [];
	private string $operation_mode = 'H';


	private array $template_MPG_Send_LMS = [
		'CREDENCIAL' => ['required' => true, 'min' => 1, 'max' => 40],
		'TOKEN' => ['required' => true, 'min' => 1, 'max' => 6],
		'PRINCIPAL_USER' => ['required' => false, 'min' => 1, 'max' => 50], // Segundo a documentacao é o campo (tag) é obrigatório, mas pode uma string vazia... Logo (o conteúdo) não é obrigatório.
		'AUX_USER' => ['required' => true, 'min' => 1, 'max' => 20],
		'MOBILE' => ['required' => true, 'min' => 1, 'max' => 16],
		'MESSAGE' => ['required' => true, 'min' => 1, 'max' => 160],
 ];

	private array $template_MPG_Credits = [
		'CREDENCIAL' => ['required' => true, 'min' => 1, 'max' => 40],
		'TOKEN' => ['required' => true, 'min' => 1, 'max' => 6],
		'v_st_Status' => ['required' => false, 'min' => 1, 'max' => 3]
	];

	private array $template_MPG_Calculate_Message_Length_UTF8_or_UTF16 = [
		'CREDENCIAL' => ['required' => true, 'min' => 1, 'max' => 40],
		'TOKEN' => ['required' => true, 'min' => 1, 'max' => 6],
		'MESSAGE' => ['required' => true, 'min' => 1, 'max' => 999999999]
	];

	private array $template_MPG_Query01 = [
		'CREDENCIAL' => ['required' => true, 'min' => 1, 'max' => 40],
		'TOKEN' => ['required' => true, 'min' => 1, 'max' => 6],
		'START_DATE' => ['required' => true, 'min' => 10, 'max' => 10],
		'END_DATE' => ['required' => true, 'min' => 10, 'max' => 10],
		'AUX_USER' => ['required' => false, 'min' => 1, 'max' => 20],
		'MOBILE' => ['required' => false, 'min' => 15, 'max' => 15],
		'STATUS_CODE' => ['required' => true, 'min' => 1, 'max' => 1]
	];
	 ...
	 * @param array $data
	 */
 	private function MPG_Send_LMS(array $data)
	{
		$sai = false;
	 ...
	 * @return unknown
	 */
	private function MPG_Calculate_Message_Length_UTF8_or_UTF16($msg): int|false
	{
		$data['credencial'] = $this->params['credencial'];
		$data['token'] = $this->params['token'];
		$data['message'] = $msg;
		...
		if ($this->validateData($data, 'template_MPG_Calculate_Message_Length_UTF8_or_UTF16')) {
			...
			if ($ret !== false) {
				...
				if ($qtd < 0) {
					$this->errors[] = match ($qtd) {
						-1 => 'Erro ' . $qtd . ': Credencial inválida',
						-2 => 'Erro ' . $qtd . ': Token inválido',
						-9 => 'Erro ' . $qtd . ': Erro interno, favor entrar em contato com o suporte',
						default => 'Erro ' . $qtd . ': código não identificado',
					};
				} else {
					$sai = $qtd;
				}
	 ...
	 * @param array $data
	 */
	private function MPG_Query01(array $data): array|false
	{
		$sai = false;
		$data['credencial'] = $this->params['credencial'];
		$data['token'] = $this->params['token'];
	 ...
	 * @return boolean
	 */
 	private function validateData(&$data, string $template)
	{
		$sai = true;
		$this->errors = '';
		foreach ($this->$template as $key => $value) {
			...
			if (isset($data[strtolower((string) $key)])) {
				// Verifica se o tamanho está correto
				$length = strlen((string) $data[strtolower((string) $key)]);
				...
				$data[strtolower((string) $key)] = ''; // Adiciona a chave pois precisa existir, ao menos, a tag vazia.
			}
		}
	...
	private function sendSOAP(string $content, string $soapAction)
	{
		...
		$url = $this->operation_mode == 'P' ? $this->p_wsdl : $this->h_wsdl;
