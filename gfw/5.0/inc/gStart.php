...
function register_global_array( $sg ): void
{
  	Static $superGlobals = [
		'e' => '_ENV',
		'g' => '_GET',
...
function register_globals( $order = 'gpcsn' ): void
{
	// $_SERVER;
	// $_ENV;
	// $_REQUEST;

	$order  = str_split( strtolower( (string) $order ) );
	array_map( 'register_global_array' , $order );
}


function gWrkCrypt($txt): string
{
	$rnd = substr(base64_encode('GiuSoft Tecnologia - Key - '.date("Y-m-d H:i:s")),0,17);
	$txt = base64_encode((string) $txt);
	return($rnd.$txt);
}
function gWrkDecrypt($txt): string
{
	return(base64_decode(substr((string) $txt,17)));
}
...
if (mb_substr((string) $docRoot,-1) === '/') {
    $docRoot=mb_substr((string) $docRoot,0,mb_strlen((string) $docRoot)-1);
}
...
if (
	(str_ends_with((string) $_SERVER['PHP_SELF'], 'login.php'))
	&& (!str_starts_with($test1, $test2))
) {
...
if (!str_starts_with($tp, $docRoot."/".$gBASE)) {
    unset($_SESSION['gSetup']);
}

$gSetup = (isset($_SESSION['gSetup'])) && ($_SESSION['usrId']>0) ? $_SESSION['gSetup'] : $gSETUP;
...
if ($_POST['gWrk'] != '') {
	...
	if (str_contains((string) $cmd,"dbinfo")) {
		sleep(30);
	} else {
		sleep(5);
	}
...
$gPathUsrFiles = $gPath . "files/".str_pad((string) $_SESSION['usrIdd'],9,"0",STR_PAD_LEFT);
...
$smartphones = ["opera mini", "lynx", "windows ce", "symbian", "msie 6.0", "opr", "OPR/15", "Trident/3.1", "IEMobile", "AUTOID", "SM-G611MT"];
$tablets = ["ipad", "xoom", "android 4"];
$gOs = 'windows';
$gDevice = "web";
...
//$gDevice="web";
if (str_contains($browser,'linux')) {
    $gOs="linux";
}

if (str_contains($browser,'mac os')) {
    $gOs="mac";
}

if ((str_contains($browser,'iphone'))||(str_contains($browser,'ipad'))||(str_contains($browser,'ipod'))) {
    $gOs="ios";
}

if (str_contains($browser,'android')) {
    $gOs="android";
}

if (str_contains($browser,'symbian')) {
    $gOs="symbian";
}
...
$gLangR=trim((string) $_REQUEST['gLang']);
$gLangS=trim((string) $_SESSION['gLang']);
...
if (($gLangR === "") && ($gLangS === "")) {
    ...
    $gLang = match ($lang) {
        ...
        'en' => "en",
        'fr' => "fr",
        default => gVar("global.language"),
    };
    $gLangR = $gLang;
    $gLangS = $gLang;
}

$gLang = $gLangR != "" ? $gLangR : $gLangS;
if (!str_contains((string) gVar("global.languages"),(string) $gLang)) {
...
$gurl = trim((string) $setup->get("global.url"));

$gNetwork="internet";
if (str_starts_with((string) $_SERVER['REMOTE_ADDR'], "192.168.")) {
    $gNetwork="local";
}

if (
	(str_starts_with((string) $_SERVER['SERVER_ADDR'], "127"))
	|| (
		(str_starts_with((string) $_SERVER['SERVER_ADDR'], "192"))
		&& (substr((string) $_SERVER['SERVER_ADDR'],0,10) === substr((string) $_SERVER['REMOTE_ADDR'],0,10))
	)
) {
	$gNetwork="local";
}

if (substr((string) $_SERVER['REMOTE_ADDR'],0,10) === "189.89.157.") {
    $gNetwork="giusoft";
}

if (str_starts_with((string) $_SERVER['REMOTE_ADDR'], "200.254.228.")) {
...
$http_usr_files = $http_base ."files/".str_pad((string) $_SESSION['usrIdd'],9,"0",STR_PAD_LEFT);
 ...
 * @return mixed $sai
 */
function gVar(mixed $param,$new=":null:")
{
	global $setup;
	$sai='';
	if (is_object($setup)) {
...
function gLangIndex($text): string
{
	$txt = substr((string) $text,0,100);
	if ($txt != $text) {
		$txt = substr($txt,0,strrpos($txt," "));
	}
	$txt = gCleanField($txt);
	return(trim(strtolower(autoencode($txt))));
}
...
function gLang($t_word)
{
	...
	if (array_key_exists($l_word,$gLngs)) {
		$agLng=$gLngs[$t_word];
		$gLng = is_array($agLng) ? $agLng[0] : $agLng;
		...
		if ($gLng=="") {
			...
			if (stripos((string) $t_word,".long") !== false) {
				$t_words=explode(".",(string) $t_word);
				if (count($t_words)>2) {
					// Se tiver 3 parametros separados por ponto, ignora o primeiro
					$t_words = [$t_words[1], $t_words[2]];
				}
				if ($t_words[1] === "") {
					$t_words[1] == "short";
				...
				if (!is_array($agLng)) {
					$gLng = $agLng;
				} elseif ($t_words[1] === "long") {
					$gLng = count($t_words) > 1 ? $agLng[1] : $agLng[0];
				} else {
					$gLng=$agLng[0];
				}
			} else {
...
//======================================= funcoes CSS =======================================
	 ...
	 */
	function cssMerge($array,$default=""): array
	{
		$quitarray=true;
		if (!is_array($array)) {
			$quitarray=false;
	...
	function putQuotation(array $mtz): array
	{
	 ...
	 * @return string $sai
	 */
	function cssEncode($mtz, $default=""): string
	{
		$sai = "";
...
if ($_REQUEST['gCmd'] == "createTranslationFiles" && gVar("global.languages") != "") {
...
function faceRecognition($json): void
{
...
function biocheck360($pathImage, array $imageData, $convertBase64 = true) {

	if ($convertBase64) {
		$img = file_get_contents($pathImage);
		$base64Image = base64_encode($img);
	}

	$url = "http://167.234.233.9/v1";
	...
	if ($convertBase64) {
		$FaceToMatch["FaceToScan"] = $base64Image;
	} else {
		$FaceToMatch["FaceToScan"] = $pathImage;
		$FaceToMatch["ImageSourceIsPath"] = true;
	}

	$request = ["FaceToMatch" => $FaceToMatch];
...
if ($cmd != "" && $cmd === 'saveImage') {
    $usrId = intval($_SESSION['usrId']);
    if ($usrId > 0) {
		$data_uri = $_POST['data_uri'];
		$filaBiometria = $_REQUEST['filaBiometria'];
		if (preg_match('/^data:image\/(\w+);base64,/', (string) $data_uri, $type)) {
			$ano = date('Y');
			$fotoCadastrada = $gPathFiles."/geral_pessoas/".$usrId.'.jpg';
			$data = substr((string) $data_uri, strpos((string) $data_uri, ',') + 1);
			$data = base64_decode($data);
			//$mediaPath = $gPathFiles."/facial/$ano/";
			//mkdir($mediaPath, 0775, true);
			$mediaPath = file_exists("/tmp/ramdisk/") ? "/tmp/ramdisk/" : "/tmp";
			 ...
			 *
			 */
			$jarr = gFacialService("{task: compare, img1:$fotoObtida, img2:$fotoCadastrada}");
			if ($jarr) {
