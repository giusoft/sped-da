...
class gNetLayout
{
	...
	{
		return (explode("|",(string) $this->name));
	}
...
class gNet
{
	 ...
	 */
	function addLayout($layout): void
	{
	...
	function htmlTextClean($txt): string
	{
	...
	function setCookie($ckfile): void
	{
	...
	function getCaptcha($url,$style=0)
	{
		...
		$ch = curl_init ();
		if (str_contains((string) $url[1],"https")) {
	 ...
	 */
	function siteOpen($url,$post='',$cookie=false,$captcha=false, $json=false): bool|string
	{
		global $usrId;
		$ok = true;
		...
		if ($this->html == '') {
			if ($captcha) {
				...
				if (is_array($url)) {
					...
					$ch = curl_init ($url[1]);
					if ($json) {
						$bodyData = ['json' => json_encode($post)];
						...
						curl_setopt($ch,CURLOPT_HTTPHEADER,[
							...
							"Content-Length: ".strlen((string) $post)
						]);
			...
			if (stripos($this->html,"ISO-8859") !== false) {
				...
				// Mudando ISO pra UTF
				$this->html=mb_convert_encoding($this->html, 'UTF-8', 'ISO-8859-1');
	...
	function searchLayout($debug=0)
	{
		...
		foreach ($tmp as $index => $layout) {
			if ($acheiEm == 100000) {
				...
				foreach ($keys as $key) {
					$b = stripos((string) $this->html,(string) $key);
					if (($b !== false) && ($b < $acheiEm)) {
						...
						$b = stripos(html_entity_decode((string) $this->html),(string) $key);
						if (($b !== false) && ($b < $acheiEm)) {
							if ($debug > 0) {
								echo "<h1>Achei layout [$key] em $b</h1>";
							}
	...
	function attachExtract($txt): string
	{
		$sai = '';
		$p1 = stripos((string) $txt,"<a ");
		...
		if ($p1 !== false) {
			...
			$p1 = stripos((string) $txt,"<img ");
			if ($p1 !== false) {
				$txt = substr((string) $txt,$p1);
				$txt = explode("'",$txt);
				$sai = $txt[1];
			}
		}
	...
	function getTags(string $tag,$start,string $end="",$delimiter="",$debug=0): void
	{
		$html=$this->html;
		$tagStart="<".$tag;
		$tagEnd="</".$tag;

		$p = stripos((string) $html,(string) $start);
		if ($p === false) {
			$start=html_entity_decode((string) $start);
			$p=stripos((string) $html,$start);
		}

		if ($p !== false) {
			$txt = substr((string) $html,($p+strlen((string) $start)));
			if ($end !== "") {
				$txt=substr($txt,0,stripos($txt,$end));
			}

			$txts = $delimiter != "" ? explode($delimiter,$txt) : [$txt];
			...
			foreach ($txts as $txt) {
				...
				while((stripos($txt,$tagStart) !== false) && ($txt !== ''))
				{
					...
					$txt = substr($txt,$p2);
					$this->htmlTags[$tag][($start."~".$end)][] = [$limpo, $sujo, $primeiro];
					$primeiro = false;
				}
			}
			...
			if ($param['type'] == "array") {
				...
				if ($delimiter != '') {
					...
					while ($i < $ttl)
					{
						...
						if (trim((string) $id) !== '') {
							$campos[$campoBd][] = [$id, trim(str_ireplace("<br>"," ",implode($separator,$content))), trim(str_ireplace("<br>"," ",implode(chr(10),$attach)))];
						}
					}
					...
					$posContents = explode(",",(string) $posContent);
					$posStep = $param['step'];
					$ttl = count($this->htmlTags[$tag][$start."~".$end]);
				...
				foreach ($indexes as $indx) {
					if ($this->htmlTags[$tag][$start."~".$end][$indx][0] != '') {
						$faz = true;
						if ($param['find'] != '') {
							$faz = (stripos((string) $this->htmlTags[$tag][$start."~".$end][$indx][1],(string) $param['find']) !== false);
						}
				...
				foreach ($this->htmlTags[$tag][$start."~".$end] as $el) {
					foreach ($indexes as $indx) {
						if (stripos($el[1], $indx) !== false && $el[0] !== '') {
	...
	function run($url,$post,$cookie): int
	{
		$error = 0;
		// Abre a URL
