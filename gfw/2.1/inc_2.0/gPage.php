			...
			} else
			{
				if (strpos($perm,"V")>0)
				{
					...
					for ($t=0; $t<$cntcmps; $t++)
					{
						...
						if (($cmp->name<>'id') && ($cmp->name<>'idd'))
						{
							$cmps[$cmp->name]=$cmp->name;
						}
					}
					$w="";
					foreach ($flds as $field)
					{
						...
						{
							if ($f[0]==gI_DATE) // data?
								$w[]=$f[1]."='".gDBDate($_REQUEST[$f[1]])."'";
							elseif ($f[0]==gI_DATETIME)
								$w[]=$f[1]."='".gDBDateTime($_REQUEST[$f[1]])."'";
							else
								$w[]=$f[1]."='".$_REQUEST[$f[1]]."'";
						}
					}
					if (is_array($w))
					{
						$sql="select id from $table where ".implode(" and ",$w);
						$rsv=gQuery($sql);
						if (!$rsv->EOF)
							$gError="Operação cancelada!<br><br>Registro semelhante já existe!";
					}
				}
				if ($gError=="")
				{
					...
					if (strpos(strtolower($rs)," where ")>0)
					{
						...
						for ($t_f=0; $t_f<count($filters); $t_f++)
						{
							...
							if ($this->activeFilter($filters[$t_f][0],$_REQUEST[$this->campo].$_REQUEST[$this->campo."1"].$_REQUEST[$this->campo."2"]))
							{
								...
								elseif ($filters[$t_f][0]==gI_CHECK) // tipos checkbox
								{
									$flt[]=gField($this->campotxt)." '".$_REQUEST[$this->campo]."'";
									if (strpos($this->campo,"__")>0)
										$where.=str_replace("__",".",$this->campo)."=1 and ";
									else
										$where.="$table.$this->campo = 1 and ";
								}
					...
					} else
					{
						...
						for ($t_f=0; $t_f<count($filters); $t_f++)
						{
							...
							if ($this->activeFilter($filters[$t_f][0],$_REQUEST[$this->campo].$_REQUEST[$this->campo."1"].$_REQUEST[$this->campo."2"]))
							{
								...
								} elseif ($filters[$t_f][0]==50) // tipos checkbox
								{
									$flt[]=gField($this->campotxt)." '".$_REQUEST[$this->campo]."'";
									if (strpos($this->campo,"__")>0)
										$where.=str_replace("__",".",$this->campo)."=1 and ";
				...
				if (true)
				{
					...
					for ($t_f=0; $t_f<count($filters); $t_f++)
					{
						...
						if ($this->activeFilter($filters[$t_f][0],$this->campovalor))
						{
							...
							elseif ($filters[$t_f][0]==50) // tipos checkbox
							{
								//echo "<pre>"; var_dump($_REQUEST[$this->campo]); exit;
								if($_REQUEST[$this->campo] == 'on')
								{
									$flt[]=gField($this->campotxt)." '".gCheck($_REQUEST[$this->campo])."'";
									if (strpos($this->campo,"__")>0)
										$where.=str_replace("__",".",$this->campo)."=".gDBCheck($_REQUEST[$this->campo])." and ";
									else
										$where.="$table.$this->campo = ".gDBCheck($_REQUEST[$this->campo])." and ";
								}
							} 
							elseif (($filters[$t_f][0]==gI_DATE) || ($filters[$t_f][0]==gI_DATENULL))
							{
