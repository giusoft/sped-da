		...
		foreach ($csv as $linha)
		{
			...
			} else
			{
				...
				if ($linha[3]==5)
				{
					...
					if ($padrao=="")
					{
						if ((strpos($linha[0],"fim")!==false) || (strpos($linha[0],"final")!==false) || (strpos($linha[0],"até")!==false))
						{
							$padrao=gDate(date("Y-m-d"));
						}
						if ((strpos($linha[0],"início")!==false) || (strpos($linha[0],"inicio")!==false) || (strpos($linha[0],"_de")!==false))
						{
							$padrao=gDate(date("Y-m-d"));
						}
					}
					$mtz[]="<-".$obj->gText($prefixo.$linha[0],$padrao,$linha[3],$linha[4],$linha[5],false);
				} elseif ($linha[3]==9)
				{
					$padrao=$linha[2];
					if ($padrao=="")
					{
						if ((strpos($linha[0],"fim")!==false) || (strpos($linha[0],"final")!==false) || (strpos($linha[0],"até")!==false))
						{
							$padrao=gDateTime(date("Y-m-d 23:59:59"));
						}
						if ((strpos($linha[0],"início")!==false) || (strpos($linha[0],"inicio")!==false) || (strpos($linha[0],"_de")!==false))
						{
							$padrao=gDateTime(date("Y-m-d 00:00:00"));
						}
					}
					$mtz[]="<-".$obj->gText($prefixo.$linha[0],$padrao,$linha[3],$linha[4],$linha[5],false);
				} elseif ($linha[3]<20)
