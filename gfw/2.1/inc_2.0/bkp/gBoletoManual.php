...
*/
	...
	function gPDFBegin($logomarca=false)
	{
		$orientation="P";
		$this->boleto_atual=0;
		//$this->boleto_atual=$vBoletoAtual;
	...
	function gPDFEnd()
	{
		$this->Output("boleto.pdf",true);
	}
	function gPDFAddPage()
	{
		$this->AddPage();
	}
	function gPDFOpen()
	...
	function gPreparaBoleto($banco=gB_ITAU,$id_empresa=0,$id_pessoa=0,$valor=0,$nosso_numero=0,$numero_documento='',$tipo_boleto='',$data_vencimento,$fin_parc_lanc=0)
...
// Zera variáveis
		...
		// Acesso ao banco
		//$_rs=gQuery("select * from fin_bancos where codigo='$banco'");
		//12 é o código do banco Itaú manual
		$_rs=gQuery("select * from fin_bancos where id=12");
		...
		if ($id_pessoa>0)
		{
			...
			//$this->dados['sacado_cnpj']="CPF: ".$rsc->fields['cpf'];
		...
		//$this->dados['vencimento']=$dia."/".$mes."/".$ano;
		//echo $this->dados['emissao']."<---->".$_rs->fields['vencimento_em_dias_apos_emissao'];
		//$this->dados['emissao']=gDate(date("Y-m-d"));
		$this->dados['vencimento']=gDate($data_vencimento);
		$this->dados['valor']=$valor;
		$this->dados['id_fin_parcelas']=$fin_parc_lanc;
	...
	function gBoletoVazio($boleto_atual)
	{
	// Gera saída da grade do boleto (formulário vazio)
		...
		$this->Text($mesq,$msupb+10.7,"Local de Pagamento");$this->Text($mesq+137,$msupb+10.7,"Vencimento");
		$this->Text($mesq,$msupb+19.8,"Cedente");$this->Text($mesq+137,$msupb+19.8,"Agência/Código Cedente");
		...
		$this->Text($mesq,$msupb+32.7,"Uso do Banco");$this->Text($mesq+34.5,$msupb+32.7,"Carteira");$this->Text($mesq+49.5,$msupb+32.7,"Espécie");$this->Text($mesq+63.5,$msupb+32.7,"Quantidade");$this->Text($mesq+102.2,$msupb+33.9,"x");$this->Text($mesq+103.5,$msupb+32.7,"Valor");$this->Text($mesq+137,$msupb+32.7,"(=) Valor do Documento");
		...
		$this->Text($mesq,$msupb+80.2,"Sacador/Avalista");$this->Text($mesq+149.5,$msupb+80.2,"Código de Baixa");
	...
	function gBoletoDados()
	{
		...
		$id_fin_tipos =$this->dados['tipo_boleto'];
		...
		if ($ValorDoc_banco <> 0)
		{
			...
			$id_parcelas =$this->dados['id_fin_parcelas'];
		...
		//Fim do lancamento financeiro
			...
			$this->Text($mesq+10,$msupb+29.5,$dados['emissao']);$this->Text($mesq+34.5,$msupb+29.5,$numero_documento_local) ;$this->Text($mesq+70.2,$msupb+29.5,"DM");$this->Text($mesq+92,$msupb+29.5,"N");$this->Text($mesq+165,$msupb+29.5,$NossoNum);
		...
		$this->Text($mesq+39,$msupb+35.5,strtoupper($dados['carteira']));$this->Text($mesq+54,$msupb+35.5,"R$");
		$this->SetFont('helvetica','B',8);
...
{
//echo 	$carteira." teste- ".$nosso_numero;
