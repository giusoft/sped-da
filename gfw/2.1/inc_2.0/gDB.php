<?
include $gPathDefault."adodb/adodb.inc.php";
...
function gQuery($arg,$bd=gD_DEFAULT,$fetch=0,$transaction=gD_NOTRANS)
{
  ...
  global $gLastTransaction;
	...
	if ($transaction<=1)
	{
		...
		if ((gVar("database.charset")=="UTF-8") && (substr(gVar("database.engine"),0,5)=="mysql"))
		{
	...
	} else
	{
		...
		$sql=str_replace("--","-",$sql);
		if ( (strpos(strtolower($sql),"show databases")!==false) ||
...
<?
		...
		$out->gBr();
		//$out->gMsg("<a href='http://www.sindauto.com.br'>www.sindauto.com.br</a>");
		$out->gTableColEnd();
		$out->gTableRowEnd();
		$out->gTableEnd();
		$out->gEnd();
...
if (($gPageSecurity) && (strpos($thispage,"index.php")===false) && (strpos($thispage,"menu.php")===false) && (strpos($thispage,gVar("page.index"))===false) && (strpos($thispage,gVar("page.login"))===false) && (strpos($thispage,gVar("page.logout"))===false) && (strpos($thispage, "online")>0) )
{
	...
	$sql="select id from geral_online where id_geral_pessoas=".(int) $usr_id." order by id desc";
	$rs=gQuery($sql);	

	if ( (int) $rs->fields['id'] > 0)
	{
