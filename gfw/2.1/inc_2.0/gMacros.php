...
$sql='select p.data_vencimento,p.valor,m.descricao as forma_de_pagamento from geral_pessoas a left join fin_lancamentos l on a.id=l.id_geral_pessoas
    left join fin_parcelas p on l.id=p.id_fin_lancamentos left join fin_modos_pagamento m on p.id_fin_modos_pagamento=m.id where a.id=15';
$sqlfin=gQuery($sql);
    ...
    if (!$sqlfin->EOF){
       ...
       while(!$sqlfin->EOF)
       {
           $vencimento[]=$sqlfin->fields['data_vencimento'];
           $valor[]=$sqlfin->fields['valor'];
           $forma_pagamento[]=$sqlfin->fields['forma_de_pagamento'];
           $sqlfin->MoveNext();
       }
       $gMacros[]=$vencimento;
       $gMacros[]=$valor;
       $gMacros[]=$forma_pagamento;
    }
...
if (!$rstmp->EOF)
{
	...
	$gMacros[]=array("empresa_endereco_complemento",$rstmp->fields['complemento']);
...
*/
function gMacro($texto,$macros="",$bd="",$t="")
{
//echo "---->".$bd;exit;
	global $gId;
