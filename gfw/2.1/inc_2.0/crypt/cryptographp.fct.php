...
// -----------------------------------------------
 ...
 function dsp_crypt($cfg=0,$reload=1,$exit_on_page=1) {
 // Affiche le cryptogramme
 $s="<table><tr><td><img id='cryptogram' src='".$_SESSION['cryptdir']."/cryptographp.php?cfg=".$cfg."&".SID."'></td>";
 if ($reload) $s.="<td><a title='".($reload==1?'':$reload)."' style=\"cursor:pointer\" onclick=\"javascript:document.images.cryptogram.src='".$_SESSION['cryptdir']."/cryptographp.php?cfg=".$cfg."&".SID."&'+Math.round(Math.random(0)*1000)+1\"><img src=\"".$_SESSION['cryptdir']."/images/reload.png\"></a></td>";
 $s.="</tr></table>";
 if ($exit_on_page==1) echo $s;
 return($s);
 }
