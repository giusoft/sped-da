...
class gDatabase
{
	...
	function __construct($json)
	{
		...
		if ($jarr['mode'] == "pdo")
			...
			$this->mode = "adodb";
		if (($jarr['persistent'] == "true") || (($jarr['persistent'] == "on")))
	...
	function connect()
	{
		try {
			...
			if ($this->persistent)
				$this->con = new PDO(gVar("database.engine") . ":host=" . gVar("database.url") . "$charset;dbname=" . gVar("database.name"), gVar("database.user"), gVar("database.password"), array(PDO::ATTR_PERSISTENT => true));
			else
	...
	function query()
	{
		...
		try {
			...
			$stmt->execute();
			if (stripos($sql, "SELECT ") !== false && strtoupper(substr(trim($sql),0,6)) != "UPDATE" && strtoupper(substr(trim($sql),0,6)) != "DELETE" && strtoupper(substr(trim($sql),0,6)) != "INSERT") {
				if ($this->mode == "adodb") {
					// Emula o PHP ADODB
					$sai = $stmt->fetchAll();
					$obj = new gAdodb($sai);
					...
					return $stmt->fetchAll((int) gVar("database.fetchMode"));
				}
			}
		} catch (PDOException $e) {
			
			$this->rollBack();
			if ((strpos($e->getMessage(),"General error")===false) || true) {
	...
	function describe($sql)
	{
		...
		try {
			...
			for ($i = 0; $i < $rs->columnCount(); $i++) {
				...
				if (($col['flags'][0]=='not_null') || ($col['flags'][1]=='not_null') || ($col['flags'][2]=='not_null'))
...
class gAdodb
{
	...
	{
		if (($this->pointer >= $this->ttlFields) || ($this->pointer < 0)) {
			$this->fields = null;
		} else
			$this->fields = $this->rs[$this->pointer];
	}
...
function dbQuery()
{
	...
	switch($aCnt) {
		case 1:
			return $dbConn[$connName]->query($aList[0]);
			break;
		case 2:
			return $dbConn[$connName]->query($aList[0], $aList[1]);
			break;
		case 3:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2]);
			break;
		case 4:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3]);
			break;
		case 5:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4]);
			break;
		case 6:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5]);
			break;
		case 7:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6]);
			break;
		case 8:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6], $aList[7]);
			break;
		case 9:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6], $aList[7], $aList[8]);
			break;
		case 10:
			return $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6], $aList[7], $aList[8], $aList[9]);
			break;
	}
...
function dbQueryValue()
{
	...
	switch($aCnt) {
		case 1:
			$rs = $dbConn[$connName]->query($aList[0]);
			break;
		case 2:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1]);
			break;
		case 3:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2]);
			break;
		case 4:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3]);
			break;
		case 5:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4]);
			break;
		case 6:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5]);
			break;
		case 7:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6]);
			break;
		case 8:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6], $aList[7]);
			break;
		case 9:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6], $aList[7], $aList[8]);
			break;
		case 10:
			$rs = $dbConn[$connName]->query($aList[0], $aList[1], $aList[2], $aList[3], $aList[4], $aList[5], $aList[6], $aList[7], $aList[8], $aList[9]);
			break;
	}
	$sai = $rs[0][0];
	return($sai);
 ...
 * @return int Retorna o id do novo registro criado
 */
function dbFastInsert($table, $fields, $returnId = false)
{
	global $AESKEY;
	$sai = 0;
	$nfields = '';
 ...
 * @param type $idField
 */
function dbFastUpdate($table, $fields, $where, $idField = 'id')
{
	global $AESKEY;
	$nfields = '';
	foreach ($fields as $key=>$value)
	...
	foreach ($t as $tdata) {
		...
		} else
		{
			...
			if (strtolower($tmp[$prox]) == "as") {
				...
				if ($tmp[$prox] <> "" && strtoupper($tmp[$prox])<>'LIMIT') {
					$table['alias'] = str_replace("\n", "", trim($tmp[$prox]));
					$table['fieldLabel'] = $tmp[$prox];
				} else {
					$table['alias'] = str_replace("\n", "", trim($table['name']));
...
{
	if (stripos($sql,'idd')===false)
	{
		$id = intval($_SESSION['usrId']);
		$idd = intval($_SESSION['usrIdd']);
		if ((is_developer) && ($id == 1)) {
			$id = intval($_SESSION['usrAppId']);
			$idd = intval($_SESSION['usrAppId']);
		}
...
class gDB
{
	...
	function fieldType($fld, $default)
	{
		...
		if (substr($fld->name, 0, 9) == "container")
			$fldType = $tmp['container'];
		if (substr($fld->name, 0, 5) == "placa")
			$fldType = $tmp['placa'];
		//gLog("====> ".$fld->name." === ".$fld->max_length." === $fldType === ".$fld->type);
		return $fldType;
	}
...
//echo "Campos<pre>";print_r($this->gDB);echo "</pre>";
	}
	...
	function load($force = false)
	{
		...
		if ($this->enabled)
		{
			...
			if (file_exists($this->fileName) && count($gDB)==0 && $force==false)
			...
			} else
			{
				...
				else
				{
					foreach ($sp as $tableName=>$sql)
					{
						if (($sql<>$gDB[$tableName]['sql']) && (strpos($sql,'gfw_')===false))
						{
...
function gCheckAlignByType($type)
{
	$align="center";
	$aligns='';
	$aligns['text']='left';
...
function gLoadDBCache($force = false, $enabled = false)
{
