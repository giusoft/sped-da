...
class gBrowser
{
	...
	{return false;
		...
		$this->gzip = ($this->jarr['gzip']<>'true' ? false : true);
		$this->captcha = ($this->jarr['captcha']<>'true' ? false : true);
		$this->debug = ($this->jarr['debug']<>'true' ? false : true);
	...
	{
		return($this->session.' '.str_pad($this->sendCount,2,'0', STR_PAD_LEFT));
	}

	function beginSession()
	{
		$this->session = date("ymd").str_pad(rand(1,999999), 6, "0", STR_PAD_LEFT);
		jLog("Início de sessão", $this->getSessionNumber(), "infor");
	}

	function endSession($killCookieOnDestruct = true)
	{
	...
	function setCookie()
	{
		...
		$ckfile = '/tmp/gbrowser-'.$_SESSION['db']."-". $_SESSION['usrId']."-".date("Hmi") . "-cookie.txt";;
	...
	function getHtmlObject($convertToUtf8 = false, $validate = false)
	{
		//$this->html = str_replace('&#160;',' ', str_replace('&nbsp;',' ', $html));
		if ($this->info['header_size']>0)
		{
			$this->header		= substr($this->html, 0, $this->info['header_size']);
			$this->html			= trim(substr($this->html, $this->info['header_size']));
			$this->html 		= str_replace('&nbsp;','', $this->html);
		} else {
			$p = strpos($this->html, "\n\n" );
			if ($p!==false)
			{
				$this->header = substr($this->html, 0, $p);
				$this->html = substr($this->html, $p+2);
			}
		}
		if ($convertToUtf8)
		{
	...
	function pushHtml()
	{
		$html = trim(substr($this->html, $this->info['header_size']));
	...
	function prepare($url = "")
	{
		...
		if ($this->returnHeader)
			curl_setopt($this->ch, CURLOPT_HEADER, true); // retorna o cabeçalho HTTP junto ao HTML
		else
			curl_setopt($this->ch, CURLOPT_HEADER, false); // retorna o cabeçalho HTTP junto ao HTML
	...
	function close()
	{
		...
		if ($this->debug)
		{
			...
			$html = "<!--\n\n".strtoupper($this->method).' '.$this->url."\n\n=> Headers:\n".implode("\n",$this->headers)."\n\n";
			if ($this->method=='post')
			{
			...
			$html.=substr($this->html, 0, $this->info['header_size']);
			$html.="\n\n-->\n\n";
			$html.= substr($this->html, $this->info['header_size']);
	...
	function setUrl($url="")
	{
		if ($url <> "")
		{
			...
			$parse = parse_url($url);
			$this->domain = parse_url($url, PHP_URL_SCHEME).'://'.parse_url($url, PHP_URL_HOST);
			$port = parse_url($url, PHP_URL_PORT);
			if ($port<>'80' && $port<>'')
			{
				$this->domain.=':'.parse_url($url, PHP_URL_PORT);
			}
		}
	}
	 ...
	 * @version	1.0 29-03-2016 12:21
	 */
	function get($url = "", $label = "", $autoRedirect = false)
	{
		$ok = true;
		$this->method = "get";
		$this->sendCount++;
		...
		// file_put_contents("/tmp/ramdisk/gbrowser.log", $this->html, FILE_APPEND);
		$this->pushHtml();
		...
		foreach ($codes as $code)
		{
			...
			if (substr($code,0,4)=='200 ' && $this->http_code=='')
			{
				$this->http_code=200;
				//$ok = false;
			}
	 ...
	 * @version	1.0 29-03-2016 12:21
	 */
	function post($url="", $label = "", $autoRedirect = false)
	{
		$ok = true;
		$this->method = "post";
		$this->sendCount++;
	 ...
	 */
	function getElement($tag, $sign, $attr = "")
	{
		$sai = '';
		$matches = '';
		preg_match_all('/<'.$tag.'[^>]+>/i', $this->html, $matches);
		if ($attr == '')
		{
			$sai = $matches[0][0];
		} else {
	 ...
	 */
	function getCaptcha($sign="", $type="jpeg", $optimize = false, $tag="img")
	{
	 ...
	 */
	function ocr($img_file_name = "", $force = "digits" )
	{
		/*
	...
	function optimize($img_data,$img_file_name)
	{
	...
	function runJavascript($script,$mostrar="")
	{
		global $usrId;
		$sai = '';
	...
	function javascriptRedirect($html,$baseUrl='', $getCookie = false, $usePost = true)
	{
		global $usrId;
	...
	function format($txt, $format = 'string')
	{
		if ($txt <> '')
		{
			$txt = superTrim(strip_tags($txt));
			switch ($format)
			{
	...
	function getField($method, $id, $format = "string", $parm = '', $size = 1)
	{
		$sai = "";
		switch ($method)
		{
			...
			case 'id-inc':
				...
				*/
				...
				$cnt = substr($id, $parm, $size);
				$faz = true;
				while ($faz)
				{
					$idCnt = substr($id,0, $parm).$cnt.substr($id,$parm+$size);
					//echo $idCnt."<br>";
			...
			case 'java-value-class':
				...
				if (is_object($tab))
				{
					...
					for ($i = 0; $i < $and->length; $i++) {
						if (trim($and->item($i)->getAttribute('class'))=='value')
						{
...
class gBrowserJuridico extends gBrowser
{
	...
	function step_setCookie()
	{
		// Primeiro chama a página só pra salvar o cookie...
		return ($this->get('','Obtendo Cookie'));
	}

	function step_getCaptcha($optimize = true, $captchaName = 'captchaImg')
	{
	...
	function step_post($usarDetalhe = true)
	{
		if ($this->viewState<>'')
		...
		if (strpos($this->html, $this->jarr['captchaError'])!==false)
		{
			$ok = false;
			$this->errorCode = "602";
			...
			if (strpos($this->html, $this->jarr['notFound'])!==false)
			{
				$ok = false;
				$this->errorCode = "601";
 ...
 * @version	1.0 27-03-2016
 */
function jLog($texto, $prefixo = '', $style = '')
{
	global $debug;
	// if ($debug!==false)
	{
		...
		{
			case 'error':
				$txt = "\033[1;00;31m" . "[Erro ]\t" . $txt . "\033[1;00;37m";
				break;
			case 'aviso':
				$txt = "\033[1;00;32m" . "[Aviso]\t" . $txt . "\033[1;00;37m";
				break;
			case 'infor':
				$txt = "\033[1;00;34m" . "[Infor]\t" . $txt . "\033[1;00;37m";
				break;
			case 'query':
				$txt = "\033[1;00;33m" . "[Query]\t" . $txt . "\033[1;00;37m";
				break;
			default:
				$txt = "[-----]\t" . $txt ;
		}
