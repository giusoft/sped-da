...
if ($ambienteDesenvolvimento = false) {
	function mssql_connect(){return true;}
	function mssql_select_db(){return true;}
	function mssql_query(){return true;}
	function mssql_fetch_assoc(){return true;}
}
...
class gNFe extends ConvertNFePHP
{
	...
	function __construct($tipo = "nfe", $verificaStatus = true,$versao='4.00')
	{
		...
		if ($tipo == "nfe") {
			...
			$this->c['sincrono'] = (int) gVar('nfe.sincrono');
		} else {
		...
		$verificaStatus=true;
		if ($verificaStatus && $tipo == "nfe" && intval($_SESSION["armazemAtualId"])<>8) {
	...
	function processaNfe()
	{
		...
		$this->assina();
		$this->valida();
		// echo "<textarea rows='6' cols='60'>{$this->nfexml[0]}</textarea>";
		// exit;
	...
	*/
	...
	function config()
	{
		...
		$cfg['sincrono'] = (int) $this->c['sincrono'];
	...
	*/
	function geraTxt()
	{
		...
		$m[] = gCleanField(substr($this->c['dataEmissao'], 0, 10)."T".date("H:i:s").$this->gTimeZone);
		$m[] = gCleanField(substr($this->c['dataSaida'], 0, 10)."T".date("H:i:s").$this->gTimeZone);
		...
		$s.=implode("|", $m) . NL;
		if ($this->c['refNFe'] <> "") {
			//if (count(explode($this->c['refNFe'])) == 1) {
 				$this->c['refNFe'] = $this->c['refNFe'];
 			//}
		...
		if($this->c['idDest']<> 3)
		{// Não é uma operação com exterior
		...
		$m[] = $this->c['enderecoPais'] 
			? gCleanField(strtoupper($this->tiraEstranhos($this->c['enderecoPais']))) 
			: 'BRASIL';
		...
		foreach ($this->i as $item) 
		{
			...
			*/
			...
			if ($item["cProdANP"]<>"" || $item["cProdANP_item"])
			{
				...
				{
					$uf=$this->c['enderecoUf'];
				}

				if ($item["cProdANP"]<>"")
				{
					$cProdANP=$item["cProdANP"];
				} else
				{
					$cProdANP=$item["cProdANP_item"];
				}

				if ($item["descANP"]<>"")
				{
					$descANP=$item["descANP"];
				} else
				{
					$descANP=$item["descANP_item"];
				}
		...
		fwrite($arq, $s);
		fclose($arq);
		//echo "<textarea>".$this->nfetxt2xml($s)[0]."</textarea>";
		//exit;
		return($s);
	}
	...
	*/
	function geraXml()
	{
		...
		$ret=file_put_contents($this->arqxml, $arq);
	...
	function envia($xml = "", $numero = "")
	{
		...
		*/
		...
		if (is_array($ret))
		{
			...
			{
				$this->erros[]=$ret['cStat']." - ".($ret['protNFe']['infProt']['xMotivo'] ?: $ret['xMotivo']);
...
function cria($tipo = "nfe")
{
	...
	if ($tipo == "nfe") 
	{
		...
		if($EMPRESA=="inter" && $_SERVER['REMOTE_ADDR']!="127.0.0.1")
		{
			...
			if ($id_empresa<>14 && $this->id_empresa<> 8 && $this->id_empresa <>7 && $this->id_empresa <> 12 && $this->id_empresa <> 16)
			{
...
function salva($situacao = 'Assinada', $tipo = "nfe")
{
	...
	if($EMPRESA=="inter" && $_SERVER['REMOTE_ADDR']!="127.0.0.1")
    {
		...
		if ($id_empresa<>14 && $this->id_empresa<> 8 && $this->id_empresa <>7 && $this->id_empresa <> 12 && $this->id_empresa <> 16)
		{
			...
			$sql = "select * from {$tipo}_numeros where id_armazens=".$id_empresa." and serie='" . $this->c['serie'] . "' ";
			gLog("MSSQL :: ".$sql);
	...
	if ($tipo == "nfe") {
		...
		$mtz['cancelada'] = 0;
		$mtz['enviada'] = 0;
		$mtz['numero'] = str_pad($this->c['numero'], 9, "0", STR_PAD_LEFT);
	...
	dbQuery($sql);
	$ultimoId=dbQuery("SELECT id FROM nfe ORDER BY id DESC")[0]["id"];
...
function danfe($gId, $dest = "",$logo="")
{
	...
	if (count($rs)>0) {
		...
		if ($dest == "") {
			...
			$sairPara = "F";
			if(is_dir($dest))
			$nomeArq = $dest."/". $rs['chave'] . '-nfe.pdf';
			else
			$nomeArq = $this->tmp . $rs['chave'] . '-nfe.pdf';
		}
...
function leCertificado()
{
	...
	if (file_exists($pCert) || $_FILES['arquivoCertificado']) {
		if (!is_object($this->nfeTools)) {
			$this->nfeTools = new gNFeTools($this->config());
		}
		// Certificado
		if (!$_FILES['arquivoCertificado']) {
			$key = file_get_contents($pCert);
		} else {
			$key = file_get_contents($_FILES['arquivoCertificado']['tmp_name']);
		}
