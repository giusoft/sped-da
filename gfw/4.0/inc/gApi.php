...
if($gPathLib=="")$gPathLib="/var/www/html/gfw/4.0/inc/lib";
require $gPathLib."/amazon/aws-autoloader.php";
use Aws\Rekognition\RekognitionClient;


/**
* Classe de conexão com o Amazon
*/
class gAmazon
{
   ...
   */
   function __construct($json = "")
   {
      ...
      if ($mtz['key'] <> '') {
         ...
         $this->api_key = gVar('twitter.key');
      }
      if ($mtz['secret'] <> '') {
         $this->api_secret = $mtz['secret'];
      } else {
         $this->api_secret = gVar('twitter.secret');
      }
      
      
      // $this->credentials = new Aws\Credentials\Credentials('AKIA2MK27B3GVPAHSRBT', 'jCqnncKHJvCMNzdtEeBmg3nrjCB3xlEsNnt6+Z6/');
      $this->credentials = new Aws\Credentials\Credentials('AKIA2MK27B3G46LH3HUM', 'Lg+p/X7tKejYYuCli46CpDm59loYbD1wRH2H2tas');
      $this->options = [
          'version' => 'latest',
          'region' => 'us-east-1',
          'credentials' => $this->credentials,
          'httpOptions' => array('timeout' => 10000)
      ];

   }

   function rekognition($photo)
   {
...
class gTwitter
{
   ...
   */
   function __construct($json = "")
   {
      ...
      if ($mtz['oauth_calback'] <> '') {
         $this->oauth_calback = $mtz['oauth_calback'];
      } else {
         $this->oauth_calback = gVar('twitter.oauth_calback');
      }
...
class gFacebook
{
   ...
   */
   function __construct($json = "")
   {
      ...
      if ($mtz['key'] <> '') {
         ...
         $this->api_key = gVar('facebook.key');
      }
      if ($mtz['secret'] <> '') {
         $this->api_secret = $mtz['secret'];
      } else {
         $this->api_secret = gVar('facebook.secret');
      }
   ...
   */
   function getLoginUrl()
   {
      if (gVar("lib.facebook")=='facebook-php-sdk-master/src/facebook.php')
      {
         return($this->fb->getLoginUrl(array('scope' => 'user_birthday, email')));
      } else
      {
   ...
   * @return boolean
   */
   function saveProfilePicture($type = 'normal')
   {
      if (gVar("lib.facebook")=='facebook-php-sdk-master/src/facebook.php')
      {
         $this->fbUserId = $this->fb->getUser();
...
class gApi
{
   ...
   */
   function googleTranslate($text, $source = "pt_BR", $target = "en")
   {
      ...
      if (strpos($response, "translatedText") !== false) {
         ...
         $r=$resp->data->translations[0]->translatedText;
         $response=$r;
      } else {
         $response = "";
      }
   ...
   */
   ...
   /**
   * Identificação de objetos e faces em uma foto (Google Vision API)
   *
   * @param string $pathImage Caminho da imagem
   * @return json Json com as identificações
   */
   function googleVision($pathImagem)
   {

         $apiKey = gVar("google.key");
         ...
         $data = '
            {
               "requests": [
               {
                  ...
                  "image": {
                     "content": "'.base64_encode(file_get_contents($pathImagem)).'"
                  },
               }
               ]
            }
...
function setIdBing($id = ''){
   $this->api_key = (!empty($id))? $id : false;
   return $this->api_key;
}
function bingTranslation($text = '', $source = '', $target = ''){

   if(empty($this->api_key) && empty($text) && empty($source) && empty($target)) return;

   $ch = curl_init('https://api.datamarket.azure.com/Bing/MicrosoftTranslator/v1/Translate?Text=%27'.rawurlencode($text).'%27&From=%27'.$source.'%27&To=%27'.$target.'%27');
