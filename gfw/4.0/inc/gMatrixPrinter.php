...
{
	protected $n_colunas; 						// padrão: 40 colunas por linha
	protected $txt_cabecalho = array();
	protected $txt_corpo = array();
	protected $txt_corpo_config = array(); 		// Qtd de colunas para cada informação
	protected $txt_rodape = array();
	 ...
	 * @param string $string
	 */
	function addTexto($s, $cabecalho = true)
	{
		while ($s<>"")
		{
			$s_tmp = substr($s,0,$this->n_colunas);
			$p = strrpos($s_tmp,' ');
			if ($p===false || strlen($s)<$this->n_colunas)
			{
				if ($cabecalho)
				{
					$this->addCabecalho($s);
				...
				if ($cabecalho)
				{
					$this->addCabecalho(substr($s,0,$p));
				} else {
					$this->addRodape(substr($s,0,$p));
				}
				$s = substr($s,$p+1);
			}
		}
	 ...
	 * @return string
	 */
	function linhaSeparadora($caracter = '=')
	{
		$sai = '';
	 ...
	 * @param array $config
	 */
	function configColunasCorpo($config)
	{
