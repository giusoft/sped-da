...
class g_Menu extends g_Stdout
{
	...
	function render(&$output = '', $extra='')
	{
		...
		foreach ($this->items as $key=>$item)
		{
			...
			{
				case 'html':
					if ((count($ddsLinks)>0) && (($ddsTitle<>'') || ($ddsIcon<>'')))
					{
				...
				case 'dropdown':
					...
					if ((count($ddLinks)>0) && (($ddTitle<>'') || ($ddIcon<>'')))
					{
		...
		if ($output->bootstrapVersao==3)
		{
			if ($jarr['style'] == '' || $jarr['style'] == 'default')
				$jarr['style']= 'dark';
...
class gCalendar
{
	...
	function add($evento)
	{
		...
		$this->events[$json['dateStart']] = array($json['title'], $json['details'], $img, $json['dateEnd'], $url);
	}
		...
		if (!empty($mtz['startTime'])) {
			for ($h = $hIni; $h <= $hFim; $h++) {
				...
				for ($s = 0; $s < 60; $s+=$hSal) {
					...
					$this->dayEvents[$horaMinuto] = array('', '', '', '', $url);
				}
			}
		}
	...
	function calendar($m, $checaHoje = true)
	{
		...
		for ($l = 0; $l < 6; $l++) {
			...
			if ($checaHoje) {
				...
				$sai.="<td class=''>";
				$sai.="<a href='$lnk$dataTmp' class='btn btn-default btn-xs' style='width: 80px'>" . strtoupper(substr($meses[intval(substr($dataTmp, 5, 2))], 0, 3)) . "</a>";
				$sai.="</td>";
			}
			$outroMes++;
			for ($ds = 0; $ds < 7; $ds++) {
...
//$sai.="<h1>".$this->today."</h1>";
			...
			foreach ($this->events as $hora => $evento) {
				if (substr($hora, 0, 7) == substr($this->today, 0, 7)) {
					...
					$sai.="</td>";
					$horaFim = substr($evento[3], 11, 5);
					if ((substr($hora, 11, 5) == $horaFim) || ($horaFim == "00:00") || $horaFim == '')
						$horaFim = '';
					else
						$horaFim = ' '.gT("às").' ' . $horaFim;
					$sai.="<td>" . substr($hora, 11, 5) . $horaFim . "</td>";
					$sai.="</tr>";
				}
			}
			if (!$fez) {
	...
	function showYear()
	{
		...
		elseif ($_REQUEST["calShow"] == "month")
			...
			$sai.="<tr>";
			$sai.="<td align='center'><a href='{$lnk}year&date=$anoPassado' class='btn btn-lg btn-default'>" . (intval(substr($this->today, 0, 4)) - 1) . "</a></td>";
			$sai.="<td align='center'><button class='btn btn-lg btn-primary'>" . substr($this->today, 0, 4) . "</button></td>";
			$sai.="<td align='center' ><a href='{$lnk}year&date=$anoProximo' class='btn btn-lg btn-default'>" . (intval(substr($this->today, 0, 4)) + 1) . "</a></td>";
			$sai.="</tr>";
			$m = 0;
			$meses = $this->meses;
			$js = "";
			...
			for ($l = 0; $l < 4; $l++) {
				...
				for ($c = 0; $c < 3; $c++) {
					...
					if (substr($today, 5, 2) == substr($this->today, 5, 2)){
						$this->today = $today;
						$sai.="<a class='btn btn-sm btn-primary' href='{$lnk}month&date={$this->today}'>" . ucfirst($meses[$m]) . "</a>";
					} else
					{
						$sai.="<a class='btn btn-sm btn-default' href='{$lnk}month&date={$this->today}'>" . ucfirst($meses[$m]) . "</a>";
...
if ($device == "web")
{
	...
	switch ($tipo)
	{
		...
		case 6:
			include_once $gPath . "gfw/inc/gExportTxt.php";
			class gInput extends gTxt {}
			class gMenu extends g_Menu {}
			class gForm {
				function add(){}
				function row(){}
		...
		default:
			...
			class gForm extends g_Stdout
			{
				...
				private $class_offset = '';
				private $class_label = '';
				private $class_element = '';
				private $rows = array();
				private $groups = array();
				private $size = '';
				private $parsley = true;
				private $has_file_multiple = false;
				...
				public function __construct($json = '', $ajson = '')
				{
					...
					if(!empty($this->jarr['class']) && in_array('form-horizontal',explode(' ',$this->jarr['class'])))
					{
				...
				public function addFormMessage($txt)
				{
					global $http_lib;
					$txt.='<hr>';
				...
				public function element($name, $withLabel=true)
				{
					...
					if(empty($this->fields[trim($name)])) return;
					...
					$attr['name'] = trim($name);
					$attr['id'] = $id;
					$attr['value'] = ($this->lists[$name]<>'')? trim($this->lists[$name]) : $field['value'];
					$attr['class'] = 'form-control'.($small? ' input-sm' : '');
					$attr['type'] = $type;

					$value = $attr['value'];

					$attr['list'] = !empty($this->lists[$name]) && is_array($this->lists[$name])? $this->lists[$name] : array();
					...
					// Campo obrigatorio
					if(!empty($field['allowBlank']) && ($field['allowBlank'] == 'false' || $field['allowBlank'] == 'off'))
					...
					if ($this->firstElementName=='' && $type<>'hidden' && $type<>'show')
					...
					switch($type){
						...
						case 'upperText':
							...
							if (!$GLOBALS['pasteInBarcode']) {
								$attr['paste'] = ($field['paste'] != NULL && $field['paste'] != 'false' && $field['paste'] != '0');
							}
							break;
						case 'lowerText':
						...
						case 'memo':
							...
							$extra = array('indent','style', 'align','image', 'h1', 'h2', 'h3', 'h4', 'code', 'alerts');
						...
						case 'date':
							...
							if (!empty($field['startDate']))
								$attr_js[] = 'minDate:"' . $field['startDate'] . '"';

							if (!empty($field['endDate']))
						...
						case 'timezonepicker':
							...
							$attr['list'] = $attr['list'] + include $gPathLib.'timezonepicker/timezonedata.php';
						break;
						...
						case 'select':
							...
							if(!empty($field['value']))
								$attr['value'] = (is_array($field['value']))? $field['value'] : array($field['value']);
							else
								$attr['value']='';

							if((!$this->hasAttr('required',$attr)) && (!$this->hasAttr('multiple',$attr)))
								$attr['list'][0] = '* '.gT("Indiferente");
					...
					switch($type){
						...
						case 'recaptcha':
							...
							if(!empty($_SERVER['HTTPS'])) $element = str_replace('http','https',$element);
						break;
						case 'recaptchav2':
						...
						case 'timezonepicker':
							...
							{
								$btn_title = $field['value']? $field['value'] : gT('Selecionar Fuso Horário');
						...
						case 'comboMultiSelection':
							$element = $this->elementTag($attr);
							if (gVar("lib.selectize") <> '' && empty($field['disableSelectize']) && $this->bootstrapVersao==3)
							{
						...
						case 'selectcode':
							...
							$js = "function ".$field['name']."getSelectCode(id)
							{
								$(document).ready(function()
								{
									$.ajax(
					...
					{
						if ($this->jarr['inline']<>'true' || $type=="html")
						{
							if ($type=="html")
				...
				function elementTag($attr){
					...
					if(in_array($type,array('select','combo','textarea', 'code','hidden'))){
					...
					switch($type){
							...
							if(count($list)){
								foreach($list as $opt_value => $opt_label){
									...
									else 
									{
                                        ...
                                        {
											// echo "opt_value = ".$opt_value." <BR> opt_label = $opt_label <BR> value = $value";
											// exit;
						...
						case 'barcode':
							...
							$output.= '<input '.$this->renderAttr($attr) . $notPaste . '/>';
				 ...
				 * @return type
				 */
				private function hasAttr($needle,$haystack){
					return is_array($haystack) && array_key_exists($needle, $haystack) ? true : false;
				}
				 ...
				 * @return type
				 */
				private function attr($needle,$haystack){
					...
					if ($this->jarr['enabled']=='false')
					{
						...
						if (is_array($this->buttons)) {
							foreach ($this->buttons as $indx=>$btn) {
								...
								if (is_object($o))
								{
									$buttons.=$o->button("{".$btnIcon." ".$btnConfirm." ".$btnTarget. " " . $btnshowWait. " " . $btnOnClick." type: $btnType; name: " . $btn['name'] . "; title: " . $btn['title'] .
														"; size: $this->size; style: $btnStyle; url: " . $btn['url'] . "; href: " . $btn['href'] ."}", $this->buttonsJavascript[$indx]).'<div class="hidden-lg hidden-md hidden-sm" style="height: 1px"><br/></div>';
								} else {
									$buttons.=$o->button("{".$btnIcon." ".$btnConfirm." ".$btnTarget. " " . $btnshowWait. " " . $btnOnClick." type: $btnType; name: " . $btn['name'] . "; title: " . $btn['title'] .
														"; size: $this->size; style: $btnStyle; url: " . $btn['url'] . "; href: " . $btn['href'] ."}", $this->buttonsJavascript[$indx]).'<div class="hidden-lg hidden-md hidden-sm" style="height: 1px"><br/></div>';
								}
						...
						if ($this->jarr['style']!="inline" || $this->jarr['forceSubmit']=="true")
						{
							if($this->class_element){
				 ...
				 * @return string
				 */
				function renderInputHidden()
				{
					$output = '';
					if(count($this->input_hidden))
					{
				...
				public function openTag()
				{
					...
					if ($this->hasAttr('autoValidate',$form_attr) && $form_attr['autoValidate'] == "false" || $form_attr['autoValidate'] == "off") {
						$this->parsley = false;
					} else
					{
						$this->parsley = true;
				...
				public function render(&$output = '')
				{
					...
					$elem = '';
					// echo "<pre>";var_dump($this->rows,$elements);exit;
					...
					switch ($this->jarr['style'])
					{
								...
								if(count($this->rows)){
									...
									foreach($this->rows as $elements){
										...
										foreach($elements as $element){
											if(($this->fields[$element]['type']<>'hidden') && ($this->fields[$element]['type']<>"") )
						...
						default:
							...
							{
								// Elementos soltos
								if((count($this->rows)==0) && (count($this->fields)>3)){
...
function gAddMarkdownRequirements($conteudo="", $mostraAjuda=true)
{
...
class gWiki {
	...
	function decode($conteudo, $prepararJavascript = true)
	{
		global $LOCALHOST;
		...
		if ($LOCALHOST)
		{
			$sai=str_replace('https://app.giusoft.com.br/','http://localhost/', $sai);
		}
		return($sai);
	}

	function encode($conteudo)
	{
		$sai = str_replace('"', '”', $conteudo);
		$sai = str_replace("'", "‘", $sai);
		$sai = str_replace("{", "«", $sai);
	...
	function label($tag)
	{
		...
		{
			case 1:
				$sai.=$o->button("{title: ".$tag['descricao']."; size: tiny; style: primary; href: index.php?gSearch=%3A".$tag['descricao']."}");
			break;
			case 2:
				$sai.=$o->button("{title: ".$tag['descricao']."; size: tiny; style: success; href: index.php?gSearch=%3A".$tag['descricao']."}");
			break;
			case 3:
				$sai.=$o->button("{title: ".$tag['descricao']."; size: tiny; style: warning; href: index.php?gSearch=%3A".$tag['descricao']."}");
			break;
			case 4:
				$sai.=$o->button("{title: ".$tag['descricao']."; size: tiny; style: info; href: index.php?gSearch=%3A".$tag['descricao']."}");
			break;
			case 5:
				$sai.=$o->button("{title: ".$tag['descricao']."; size: tiny; style: danger; href: index.php?gSearch=%3A".$tag['descricao']."}");
			break;
			default:
				$sai.=$o->button("{title: ".$tag['descricao']."; size: tiny;style; href: index.php?gSearch=%3A".$tag['descricao']."}");
		}
		return($sai);
	}

	function adicionaEstilosPersonalizados()
	{
			$this->html.='
			<style>
				#targetDiv {background-color: #fff; margin-top: 8px; border: 0px; box-shadow: rgba(0, 0, 0, 0.25) 0px 54px 55px, rgba(0, 0, 0, 0.12) 0px -12px 30px, rgba(0, 0, 0, 0.12) 0px 4px 6px, rgba(0, 0, 0, 0.17) 0px 12px 13px, rgba(0, 0, 0, 0.09) 0px -3px 5px;}
				#targetDiv h1 {margin-top: 0; padding-bottom: 16px; padding-top: 16px; text-shadow: 2px 2px 2px #ddd;}
				#targetDiv h2 {margin-top: 0; padding-bottom: 16px; padding-top: 16px; border-top: 1px solid #ddd;}
				#targetDiv h3 {
					margin-bottom: 0px;
					padding-bottom: 16px;
					padding-top: 16px;
	...
	function __construct($msgInicial)
	{
		...
		} else if ($gPage==gWIKI_INDEX && $this->id==0)
		{
		...
		if ($gPage==gWIKI_ATTACHMENTS)
		{
			...
			if (count($rs))
			{
				...
				{
					$mtz = array();
					$fileName = $row['id'].'.'.substr($row['tipo'],strpos($row['tipo'],'/')+1);
					$filePath = $http_usr_files.$fileName;
					$prev = $o->button('{icon: paperclip; style: info; href: '.$filePath);
					if (strpos($row['tipo'], 'jpeg')!==false || strpos($row['tipo'], 'jpg')!==false || strpos($row['tipo'], 'png')!== false || strpos($row['tipo'], 'gif')!== false)
					{
		...
		} elseif ($gPage == gWIKI_ATTACHMENTS_SAVE)
		{
			...
			if ($this->id>0 && $gPage<>gWIKI_EDIT)
			{
			...
			} else if ($gPage<>gWIKI_EDIT){



				$buscar = gCleanField($_REQUEST['gSearch']);
				if (substr($buscar,0,1)==":")
				{
					$filtros = array();
				...
				$sql = "SELECT DISTINCT W.id id_page,W.data_criacao, W.bloqueado, W.aprovado, W.data_mostrar, W.data_ocultar, P.apelido criou, PV.apelido versionou,
									WV.titulo, WV.conteudo, WV.id id_versao
							FROM wiki W
				...
				foreach ($rs as $row)
				{
					...
					$sql = "SELECT WT.*
								...
								LEFT JOIN wiki_tags WT ON WVT.id_wiki_tags=WT.id
								WHERE WVT.id_wiki_versoes=".$row['id_versao']." ORDER BY WT.id";
					$rst = dbQuery($sql);
					$tags = "";
					foreach ($rst as $rt)
					{
					...
					$conteudo = $o->small(strip_tags(str_replace('\n',' • ', str_replace("#","", str_replace("~"," ", substr($this->decode($row['conteudo']),0,200))))) );
	...
	function open()
	{
		...
		$sql = "SELECT W.id id_page,W.data_criacao, W.bloqueado, W.aprovado, W.data_mostrar, W.data_ocultar, P.apelido criou, PV.apelido versionou,
							WV.*
					FROM wiki W
					LEFT JOIN wiki_versoes WV ON W.id=WV.id_wiki
					LEFT JOIN pessoas P ON W.id_pessoas_criou = P.id
		...
		$this->page = $rs[0];
		$this->page['criou'] = ucfirst($this->page['criou']);
		$this->page['versionou'] = ucfirst($this->page['versionou']);
	...
	function editPage()
	{
		...
		if ($titulo<>"")
		{
			if ($this->id==0)
			{
				...
				//echo $o->page."&gPage=".gWIKI_SEARCH."&gId=".$this->id;
			}
