(function(G) {
	...
	if (J) {
		...
		C = function(O) {
			var P = G(
					'<canvas style="width:' + O.width + "px;height:" + O.height
							+ 'px;"></canvas>').get(0);
		...
		C = function(O) {
			return G(
					'<var style="zoom:1;overflow:hidden;display:block;width:'
							+ O.width + "px;height:" + O.height + 'px;"></var>')
					.get(0)
		};
		K = function(P, S, T, W, O) {
			var U, V, Q, R;
			U = '<v:fill color="#' + W.fillColor + '" opacity="'
					+ (W.fill ? W.fillOpacity : 0) + '" />';
			V = (W.stroke ? 'strokeweight="' + W.strokeWidth
					+ '" stroked="t" strokecolor="#' + W.strokeColor + '"'
					: 'stroked="f"');
			...
			if (S == "rect") {
				R = G('<v:rect name="'
						...
						+ T[0] + "px;top:" + T[1] + "px;width:" + (T[2] - T[0])
						+ "px;height:" + (T[3] - T[1]) + 'px;"></v:rect>')
			} else {
				if (S == "poly") {
					R = G('<v:shape name="'
							+ O
					...
					if (S == "circ") {
						R = G('<v:oval name="'
								...
								+ V
								+ ' style="zoom:1;margin:0;padding:0;display:block;position:absolute;left:'
								+ (T[0] - T[2]) + "px;top:" + (T[1] - T[2])
								+ "px;width:" + (T[2] * 2) + "px;height:"
								+ (T[2] * 2) + 'px;"></v:oval>')
					}
				}
			}
	...
	L = function(Q, P) {
		var O = G(Q);
		return G.extend({}, P, G.metadata ? O.metadata() : false, O
				.data("maphilight"))
	};
	A = function(O) {
		if (!O.complete) {
	...
	G.fn.maphilight = function(Q) {
		...
		if (!J && G.browser.msie && !D) {
			...
			var O = [ "shape", "rect", "oval", "circ", "fill", "stroke",
		...
		return this.each(function() {
			...
			a = G.extend({}, Q, G.metadata ? W.metadata() : false, W
					.data("maphilight"));
			...
			if (W.hasClass("maphilighted")) {
				...
				W.insertBefore(R);
				R.remove();
				G(S).unbind(".maphilight").find("area[coords]").unbind(
						".maphilight")
			}
			T = G("<div></div>").css({
				display : "block",
...
//			}
				...
				if (!d.neverOn && !d.alwaysOn) {
					...
					if (d.groupBy) {
						...
						if (/^[a-zA-Z][\-a-zA-Z]+$/.test(d.groupBy)) {
							b = S.find("area[" + d.groupBy + '="'
									+ G(this).attr(d.groupBy) + '"]')
						} else {
							b = S.find(d.groupBy)
						}
						var g = this;
			...
			G(S).trigger("alwaysOn.maphilight").find("area[coords]").bind(
					"mouseover.maphilight", Z).bind("mouseout.maphilight",
					function(b) {
						N(V)
					});
			W.before(V);
