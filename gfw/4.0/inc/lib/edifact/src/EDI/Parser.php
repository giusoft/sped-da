...
class Parser
{
     ...
     */
     ...
     private function removerAcentos($t)
     {
        $t = iconv('ASCII', 'ISO-8859-1//TRANSLIT//IGNORE', $t);
    ...
    public function parse()
    {
        ...
        foreach ($this->getRawSegments() as $line) {
            ...
            if ($line_bytes !== \strlen($lineTrim)) {
                $this->errors[] = 'Caractere desconhecido '.$i.': '.$lineTrim;
            }
     ...
     *
     * @param $line UNA definition line (without UNA tag). Example : :+.? '
     * @return void
     */
    ...
    public function checkEncoding()
    {
        if (empty($this->parsedfile)) {
            throw new \LogicException('Nenhum texto foi analisado ainda');
        }
        if (! isset(self::$charsets[$this->syntaxID])) {
            echo ('Identificador de sintaxe não suportado: ' . $this->syntaxID);
        }
        
        $check = mb_check_encoding($this->parsedfile, self::$charsets[$this->syntaxID]);
        if(!$check)
            $this->errors[] = 'A codificação de caracteres não corresponde à declaração no cabeçalho de intercâmbio UNB';
     ...
     *
     * @param $location Either a local file path or a URL
     */
    public function load($location)
    {
        $contents = \file_get_contents($location);
        if ($contents === false) {
            throw new \RuntimeException("Nao foi possivel recuperar o arquivo {$location}");
        }
     ...
     */
    public function loadString($txt)
    {
        $this->resetUNA();
        $this->resetUNB();
     ...
     */
    public function loadArray($lines, $raw = true)
    {
        if ($raw) {
            $this->resetUNA();
            $this->resetUNB();
    ...
    private function unwrap(&$string)
    {
        ...
        if (preg_match_all("/[A-Z0-9]+(?:\?'|$)[\r\n]+/i", $string, $matches, PREG_OFFSET_CAPTURE) > 0) {
            $this->errors[] = 'Este arquivo contém alguns segmentos sem terminadores';
        }
        ...
        foreach ($matches as &$value) {
            ...
            if (
                ...
                if (\preg_match(self::$DELIMITER.'(?<!'.$this->symbRel.')'.$this->symbEnd.self::$DELIMITER, $value)) {
                    $this->errors[] = "Falhou ".\stripslashes($this->symbEnd).' ao escapar dados; string '.$str;
                }
            }
            ...
            if (
                ...
                if (\preg_match(self::$DELIMITER.'(?<!'.$this->symbRel.')'.$this->symbRel.'(?!'.$this->symbRel.')(?!'.$this->sepData.')(?!'.$this->sepComp.')(?!'.$this->symbEnd.')'.self::$DELIMITER, $value)) {
                    $this->errors[] = "Tem um caracter que nao escapou ".\stripslashes($this->symbRel ?: '').' neste dado; string '.$value;
                }
            }
