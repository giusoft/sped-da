...
class gNfse
{
    ...
    public function __construct($dadosEmpresa, $idCliente, $producao, $ip='bd-webcfc.giusoft.com.br')
    {
        try {
    ...
    public function setDadosNfse($nota)
    {
        $this->nota = $nota;
    }


    public function geraXmlNfse()
    {
        ...
        $infRps->appendChild($dom->createElement('Competencia', "2024-06-10T00:00:00" ));
        ...
        foreach ($this->nota['itens'] as $item) {
            ...
            $ValorInss       = $dom->createElement('ValorInss', number_format($item['valorInss'], 2, '.', ''));
            ...
            $CodigoTributacaoMunicipio = $dom->createElement('CodigoTributacaoMunicipio', trim($item['codigoTributacaoMunicipio']));
        ...
        $Endereco                  = $dom->createElement('Endereco', $this->tiraAcentos(trim($this->dadosCliente['endereco'])));
        $Numero                    = $dom->createElement('Numero', $this->tiraAcentos(trim($this->dadosCliente['enderecoNumero'])));
        $Bairro                    = $dom->createElement('Bairro', $this->tiraAcentos(trim($this->dadosCliente['enderecoBairro'])));
        $CodigoMunicipio           = $dom->createElement('CodigoMunicipio', $this->soNumeros($this->dadosCliente['enderecoIbgeMunicipio']));
        $Uf                        = $dom->createElement('Uf', trim($this->dadosCliente['enderecoUf']));
    ...
    public function assina($tagid)
    {
        ...
        } else if($tagid == 'CancelarNfseEnvio') {
        ...
        //echo '$this->URLdsig:  '.$this->URLdsig;
    ...
    public function enviarRps()
    {
        ...
        $data = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:end="http://endpoint.nfse.ws.webservicenfse.edza.com.br/">'
                    ...
                    .'</soapenv:Body>'
                .'</soapenv:Envelope>';
        $retorno = $this->enviarXML($data);

        return $retorno;
    }


    public function gerarNfse($dados) {
        ...
        $idLancamentos    = explode(',', $idLancamentos);

        $valorPIS=number_format((floor(str_replace(',', '.', $dados['valor_pis']) * 100) / 100), 2, '.', '');
        $valorCOFINS=number_format((floor(str_replace(',', '.', $dados['valor_cofins']) * 100) / 100), 2, '.', '');
        $valorINSS=number_format((floor(str_replace(',', '.', $dados['valor_inss']) * 100) / 100), 2, '.', '');
        $valorIRRF=number_format((floor(str_replace(',', '.', $dados['valor_irrf']) * 100) / 100), 2, '.', '');
        $valorCSLL=number_format((floor(str_replace(',', '.', $dados['valor_csll']) * 100) / 100), 2, '.', '');
        $valorOutras=number_format((floor(str_replace(',', '.', $dados['valor_outras']) * 100) / 100), 2, '.', '');

        $valorTotalLiquido=$valorTotal - ($valorPIS+$valorCOFINS+$valorINSS+$valorIRRF+$valorCSLL+$valorOutras);
       

        //trata lista de servico
        $listaServicos   = array('01.03', '01.01', '01.05', '01.07', '01.08');
        $listaAtividades = array('1010158', '6311900', '2651500', '6201501', '6204000', '6209100');
        ...
        $nota['itens'][0]['valorLiquido'] = $valorTotalLiquido;
        $nota['itens'][0]['valorDeducoes'] = "0.00";
        $nota['itens'][0]['valorPis']  = $valorPIS;
        $nota['itens'][0]['valorCsll'] = $valorCSLL;
        $nota['itens'][0]['issRetido'] = "2";
        $nota['itens'][0]['valorIss']  = ($nota['itens'][0]['valor'] * 0.03);
        $nota['itens'][0]['valorIssRetido']  = "0.00";
        $nota['itens'][0]['valorCofins']     =  $valorCOFINS;
        $nota['itens'][0]['valorInss']     =  $valorINSS;
        $nota['itens'][0]['valorIr']         = $valorIRRF;
        $nota['itens'][0]['outrasRetencoes'] = $valorOutras;
        $nota['itens'][0]['baseCalculo'] = $nota['itens'][0]['valor'];
        // $nota['itens'][0]['aliquota']    = '3.00'; // valor antigo
        $nota['itens'][0]['aliquota']    = '0.03';
...
*/
        ...
        if (stristr($dadosRetorno, "EnviarLoteRpsResposta")) {
            ...
            $xmlDadosRetorno = simplexml_load_string($dadosRetorno);
    ...
    public function tratarXmlParaEnvio($xmlEnvio)
    {
    ...
    public function carregaCertificados($testaVal = true)
    {
        try {
            ...
            $certDir = dirname($this->dadosEmpresa['path_certificado']);
            if (!is_dir($certDir)) {
                $msg = "Dados do certificado inválido";
                throw new Exception($msg);
            }
            $this->certsDir = $certDir;
            $this->certName = basename($this->dadosEmpresa['path_certificado']);
    ...
    public function consultarProtocolo($idNfse)
    {
        $query = "SELECT nfse.protocolo
                  FROM nfse
    ...
    public function consultarNfse($protocolo)
    {
        ...
        if (stristr($dadosNfse, 'Nfse')) {
            $dadosNfse = $this->extrairXml('<Nfse>', '</Nfse>', htmlspecialchars_decode($dadosNfse));
    ...
    public function consultarXmlNfse($idNfse)
    {
    ...
    public function cancelarNfse($idNfse, $idPessoa, $numeroNota)
    {
    ...
    public function gerarDanfeComProtocolo($protocolo)
    {
        ...
        $xml               = simplexml_load_string($nfseEnvelopada);
    ...
    public function gerarLinkDanfe($cnpj, $numeroNfse, $codigoVerificacao)
    {
        $linkNotaFiscal = 'http://lftributos.metropolisweb.com.br:8181/'
    ...
    public function extrairXml($tagInicial, $tagFinal, $xml)
    {
        if(trim($xml) !== '') {
            ...
            $xmlExtraido    = $xmlExtraido.$tagFinal;
        }
    ...
    {
        $dadosResultado = htmlspecialchars_decode($resultado);
        if(
            ...
            $dadosMensagemErro = simplexml_load_string($dadosResultado);
            //$resultado = '';
    ...
    public function consultarIdNfse($idNota)
    {
        $consultarIdNfse = "
            SELECT nfse.id
            FROM   nfse
    ...
    public function consultarSituacao($idNfse)
    {
        $consultaSituacao = "
            SELECT CONCAT(
                            nfse.situacao,
    ...
    public function atualizarSituacao($idNfse)
    {
        $protocolo    = $this->consultarProtocolo($idNfse);
        $xmlDadosNfse = $this->consultarNfse($protocolo);

        if(!empty($xmlDadosNfse) && !stristr($xmlDadosNfse, 'Erro:')) {
            ...
            $atualizaSituacao = "
                UPDATE nfse
                SET nfse.situacao   = 'APROVADA',
                    nfse.confirmada = '1'
                WHERE nfse.id = ".$idNfse;
            $this->executarQuery($atualizaSituacao);

            //3 - Consulta proximo lote da NFSe
            $xmlDadosNfse = simplexml_load_string($xmlDadosNfse);
    ...
    public function consultarLoteGerado()
    {
        ...
        $ultimoLote       = $ultimoLote->fetch(PDO::FETCH_ASSOC)['lote'];

        return $ultimoLote;
    }


    public function inserirNfse(
        $cliente,
        $pessoaEmitiu,
        $dataEmissao,
        $protocolo
        ...
        $insertNfse     = $insertNfse.";";
        $dadosEnvio     = $this->executarQuery($insertNfse);
        $idNfseInserida = $this->conn->lastInsertId();
    ...
    public function cadastrarNota (
        ...
        $insertNota     = $this->executarQuery($insertNota);
        $idNotaInserida = $this->conn->lastInsertId();

        return $idNotaInserida;
    }


    public function atualizarNfLancamentos($idNfseInserida, $idLancamentos, $idNotaInserida)
    {
        $updateLancamento = "
    ...
    public function atualizarXmlNfse($xmlNfse, $idNfseInserida)
    {
        $updateXmlNfse = "
            UPDATE nfse
            SET    nfse.xml = '".$xmlNfse
            ."' WHERE nfse.id = ".$idNfseInserida.";";

        $this->executarQuery($updateXmlNfse);
    }


    public function inserirNfseNumeros(
        $ultimoLote,
        $idNfseInserida
    ) {
        $insertNfseNumeros = "
            UPDATE nfse_numeros
            SET id_nfse = ". (int) $idNfseInserida."
            WHERE lote = {$ultimoLote};";

        $this->executarQuery($insertNfseNumeros);
    }


    public function atualizarNfseNumeros(
        $serie,
        $numero,
        $idNfse
    ) {
    ...
    public function atualizarNumeroNota($idNfse, $numero)
    {
        $atualizaNumeroNota  = "
            UPDATE fin_notas
            JOIN   fin_lancamentos ON fin_lancamentos.id_fin_notas = fin_notas.id
    ...
    public function atualizarNumeroNotaLancamento($idNfse, $numero)
    {
        if($numero == "") {
        ...
        $atualizaNumeroNotaLancamento = "
            UPDATE gadmin.fin_lancamentos
            SET    numero = ".$numero."
            WHERE  fin_lancamentos.id_nfse = ".$idNfse;
    ...
    {
        $insereLote = "
            INSERT INTO gadmin.nfse_numeros (lote)
    ...
    public function gerarXmlNfse($dadosNfse)
    {
        //este echo deve ficar habilitado para
    ...
    public function verificarDadosCliente($dadosCliente)
    {
        $retorno = true;
        $mensagem = "";
    ...
    */
    ...
    public function buscarDadosCliente($idCliente)
    {
        ...
        $dadosCliente = $dadosCliente->fetch(PDO::FETCH_ASSOC);
        return $dadosCliente;
    }
    ...
    public function extrairNomeServico($xmlServico)
    {
        $posicaoNomeServico = stripos($xmlServico, '&#');
        $nomeServico = substr($xmlServico, 0, $posicaoNomeServico);
        return $nomeServico;
    }


    public function pxml($xml, $exit = true)
    {
        $dom = new DOMDocument;
        $dom->preserveWhiteSpace = FALSE;
        $dom->loadXML($xml);
    ...
    public function extrairConteudoTagsXML($xml, $tagInicial, $tagFinal)
    {
        ...
        $xmlSemInicio   = substr($xml, $posicaoInicial);
        $posicaoFinal   = strpos($xmlSemInicio, '</'.$tagFinal.'>');
        $xml            = substr($xmlSemInicio, 0, $posicaoFinal);
