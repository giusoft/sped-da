...
function msgBlockquote(){}
	...
	function tableBegin($size="big", $border=true, $alternateColor=false)
	{
		if (!$this->started)
		{
			$this->started=true;
	...
	function tableRow($row, $type='detail')
	{
		if ($type=='header')
			$this->tableHeaders[]=array($row,$type);
		else
			$this->tableLines[]=array($row,$type);
	}
...
//				$this->Cell($this->cellsWidths[$i],$line[2],$cont,$this->tableBorder,0,$align,true);
	...
	function renderLines()
	{
		...
		$cnt=0;
		$fill = false;
		foreach ($this->tableLines as $nr => $line)
		{
			$cells=$line[0];
			$type=$line[1];
			$ttl = $this->cellsQtd;
		...
		if (!is_array($this->cellsWidths))
		{
			...
			foreach ($this->tableHeaders as $key=>$line)
			{
				...
				if (!is_array($this->cellsWidths))
				{
					...
					{
						$s.=$line[0][$a];
						$this->cellsWidths[$a]=strlen($line[0][$a]);
					}
				}
				$len=strlen($s);
				...
				for ($a=0; $a<$this->cellsQtd; $a++)
				{
					...
					{
						$x = intval( ($maxW * strlen($line[0][$a])) / $len );
						if ($a<$this->cellsQtd-1)
						{
							$this->cellsWidths[$a]=$x;
						} else
						{
							$this->cellsWidths[$a]=$maxW-$ttl;
						}
	...
	function __construct($json)
	{
		...
		$this->header=$mtz['header']=="false" ? false : true;
		$this->footer=$mtz['footer']=="false" ? false : true;
		$this->fullFooter=$mtz['fullfooter']=="false" ? false : true;
		$this->charset=$mtz['charset']=="" ? "utf8" : strtolower($mtz['charset']);
	...
	************************************************************/
    ...
    function setGraphColors($colorTheme)
    {
        $colors = getGraphColors($colorTheme);
        $this->colorArray = '';
        foreach ($colors as $color)
        {
            $this->colorArray[] = array(hexdec(substr($color,1,2)),hexdec(substr($color,3,2)),hexdec(substr($color,5,2)));
        }
    }

    function ColumnChart($w, $h, $data, $format, $color=null, $maxVal=0, $nbDiv=4,$y_label="")
    {
        ...
        if(count($y_label>0)){
	        ...
	        $margin = 1;
	        $hLegend = 5;
	        $radius = min($w - $margin * 4 - $hLegend - count($y_label), $h - $margin * 2);
	        $radius = floor($radius / 2);
	        $XDiag = $XPage + $margin + $radius;
	        $YDiag = $YPage + $margin + $radius;
    ...
    function SetLegends2($data, $format,$y_label)
    {
