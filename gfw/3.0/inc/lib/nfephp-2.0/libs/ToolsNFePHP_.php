...
class ToolsNFePHP {
    ...
    */
    ...
    //private $timeZone= $this->timezone_offset_string(timezone_offset_get( new DateTimeZone( date('e') ), new DateTime() ));
    /**
     * logopath
   ...
   /**
	...
	//$this->timeZone = $this->timezone_offset_string(timezone_offset_get( new DateTimeZone( date('e') ), new DateTime() ));
		...
		if(is_array($aConfig)) {
			...
			$this->enableSCAN = $aConfig['enableSCAN'];
            if ($aConfig['proxyIP'] != ''){
...
//echo "\n\n$docxml2\n\n ";
    ...
    **/
    public function statusServico($UF='',$tpAmb='',$modSOAP='1'){
        //retorno da funçao
		...
		//echo "<textarea style='width:100%; height: 100%'>".($retorno)."</textarea>";
     ...
     **/
    public function consultaCadastro($UF,$CNPJ='',$IE='',$CPF='',$tpAmp='',$modSOAP='1'){
    ...
    **/
    public function sendLot($aNFe,$id,$modSOAP='1') {
        //variavel de retorno do metodo
        ...
        $sNFe = implode('',$aNFe);
		//echo $urlservico;exit;
        //remover <?xml version="1.0" encoding=... das NFe pois somente uma dessas tags pode exitir na mensagem
        $sNFe = str_replace(array('<?xml version="1.0" encoding="utf-8"?>','<?xml version="1.0" encoding="UTF-8"?>','<?xml version="1.0" encoding="UTF-8" standalone="no"?>'),'',$sNFe);
        $sNFe = str_replace(array("\r","\n","\s"),"",$sNFe);
        //montagem do cabeçalho da comunicação SOAP
        $cabec = '<nfeCabecMsg xmlns="'.$namespace.'"><cUF>'.$this->cUF.'</cUF><versaoDados>'.$versao.'</versaoDados></nfeCabecMsg>';
        //montagem dos dados da mensagem SOAP
        $dados = '<nfeDadosMsg><enviNFe xmlns="'.$this->URLPortal.'" versao="'.$versao.'"><idLote>'.$id.'</idLote>'.$sNFe.'</enviNFe></nfeDadosMsg>';
        //envia dados via SOAP
		if ($modSOAP == '2'){
            $retorno = $this->__sendSOAP2($urlservico, $namespace, $cabec, $dados, $metodo, $this->tpAmb);
        } else {
//			echo $urlservico."<BR>".$namespace."<BR>".$cabec."<BR>".$dados."<BR>".$metodo."<BR>".$this->tpAmb."<BR>".$this->UF;
//			exit;
...
//echo "<textarea style='width:80%; height: 80%'>".($this->soapRequest)."</textarea>";
		...
		if ($this->errStatus && $this->errStatus!='NULL' && $this->errStatus!=NULL)
		{
			return;
		}
...
//EXIT;
     ...
     */
    public function sendEvent($aEvento,$tpAmb='',$modSOAP='1'){
        ...
        $dhEvento = date('Y-m-d').'T'.date('H:i:s')."-02:00";
        ...
        foreach ($aEvento as $e){
            ...
            if ($aTEvent[$tpEvento]==''){
                $this->errStatus = true;
                $this->errMsg = "Número do evento indicado não encontrado [$tpEvento]";            
                return false;
            }
 ...
 */
    ...
    **/
    public function getProtocol($recibo='',$chave='',$tpAmb='',$modSOAP='1'){
        //carrega defaults
        $i = 0;
        ...
        if ($recibo != '' && $chave == '') {
            ...
            $cabec = '<nfeCabecMsg xmlns="'.$namespace.'"><cUF>'.$cUF.'</cUF><versaoDados>'.$versao.'</versaoDados></nfeCabecMsg>';
            //montagem dos dados da mensagem SOAP
            $dados = '<nfeDadosMsg><consReciNFe xmlns="'.$this->URLPortal.'" versao="'. $versao.'"><tpAmb>'. $tpAmb.'</tpAmb><nRec>'.$recibo .'</nRec></consReciNFe></nfeDadosMsg>';
            //nome do arquivo
		...
		//echo $modSOAP."<BR>".$urlservico."<BR>".$namespace."<BR><textarea>".$cabec."</textarea><BR><textarea>".$dados."</textarea><BR>".$metodo."<BR>".$tpAmb."<BR>".$UF;
        //exit;
        //envia a solicitação via SOAP
        if ($modSOAP == 2){
            $retorno = $this->__sendSOAP2($urlservico, $namespace, $cabec, $dados, $metodo, $tpAmb);
        ...
        //verifica o retorno
		//echo "<textarea>$retorno</textarea>";exit;
        if ($retorno){
			//tratar dados de retorno
            $doc = new DOMDocument(); //cria objeto DOM
            $doc->formatOutput = false;
    ...
    **/
    public function inutNF($nAno='',$nSerie='1',$nIni='',$nFin='',$xJust='',$modSOAP='1'){
        //variavel de retorno do metodo
     ...
     */
    public function envCCe($chNFe='',$xCorrecao='',$nSeqEvento='1',$tpAmb='',$modSOAP='1'){
        ...
        //envia dados via SOAP
        //echo "<br>".$urlservico."<BR>"; 
        //echo $namespace."<br>";
        //echo "<textarea>$cabec</textarea><br>";
        //echo "<textarea>$dados</textarea><br>";
        //echo $metodo."<br>";
        //echo $tpAmb;
	//exit;
        if ($modSOAP == '2'){
...
//        }
		$ret="";
		$ret['arquivo']="$chNFe-$nSeqEvento-procCCe.xml";
		$ret['xml']=$procXML;
		$ret['cStat']=$cStat;
		$ret['error']=$this->errMsg;
		$ret['motivo']=$xMotivo;
        return $ret;
    }//fim envCCe
    ...
    **/
    public function cancelNF($id,$protId,$xJust,$modSOAP='1'){
        //variável de retorno
        $aRetorno = array('bStat'=>false,'cStat'=>'','xMotivo'=>'','dhRecbto'=>'','nProt'=>'');
     ...
     */
    public function cancelEvent($chNFe='',$nProt='',$xJust='',$tpAmb='',$modSOAP='1'){
        //validação dos dados de entrada
...
//            $this->errMsg = $msg;
            $sai="";
            $sai['cStat']="000";
            $sai['xMotivo']="cStat está em branco, houve erro na comunicação";
            return $sai;
        }
    ...
    */
    public function loadSEFAZ($spathXML,$tpAmb='',$sUF) {
		...
		//echo $spathXML."<BR>".$tpAmb."<BR>".$sUF;exit;
    ...
    protected function __sendSOAP($urlsefaz,$namespace,$cabecalho,$dados,$metodo,$ambiente,$UF=''){
       ...
       if ($this->enableSVAN){
        ...
        if ($this->enableSCAN){
            ...
            $urlsefaz = "http://localhost/gfw/3.0/inc/lib/nfephp-2.0/wsdl/2.00/$ambiente/SCAN$usef";
...
//		echo "<BR><BR><BR><BR>";
//		var_dump($options);
//		echo "<textarea>$dados</textarea>";exit;
//		exit;
        ...
        $oSoapClient->__setSoapHeaders($header);
        //monta o corpo da mensagem soap
		$dados=str_replace(array('<?xml version="1.0" encoding="UTF-8" standalone="no"?>','<?xml version="1.0" encoding="UTF-8"?>'),"",$dados);
		
        $varBody = new SoapVar($dados,XSD_ANYXML);
        //faz a chamada ao metodo do webservices
		//echo $metodo."<BR>".var_dump($varBody);exit;
		$resp = $oSoapClient->__soapCall($metodo, array($varBody) );
...
class NFeSOAP2Client extends SoapClient {
    ...
    function __doRequest($request, $location, $action, $version) {
		...
		//echo "<textarea style='width:100%; height: 100%'>".($request)."</textarea>";
		//echo $location."<BR><BR>".$action."<BR><BR>".$version;
		//exit;
