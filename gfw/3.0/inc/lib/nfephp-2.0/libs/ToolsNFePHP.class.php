 ...
 *
 * Esta classe não mantêm a compatibilidade com a versão 1.10 da SEFAZ !!!
 *
 * @package   NFePHP
 * @name      ToolsNFePHP
 * @version   2.12
 ...
 * @author    Roberto L. Machado <linux.rml at gmail dot com>
 *
 *        CONTRIBUIDORES :
...
class ToolsNFePHP {
    ...
    */
     ...
     * @var integer
     */
    private $certMonthsToExpire=0;
    /**
     * certDaysToExpire
     * Dias que faltam para o certificado expirar
     * @var integer
     */
    private $certDaysToExpire=0;
    /**
     * priKEY
    ...
    /**
     * pubKEY
     * Path completo para a chave private em formato pem
     * @var string 
     */
    private $pubKEY='';
    /**
     ...
     * @var string
     */
    private $danfepaper = 'A4';
    /**
     * danfecanhoto
     * Estabelece se o canhoto será impresso ou não
     * @var boolean
     */
    private $danfecanhoto = true;
    /**
     * danfefont
    ...
    function __construct($aConfig='') {
        ...
        if(is_array($aConfig)) {
            ...
            $this->UF=$aConfig['UF'];
            $this->cMunicipio=$aConfig['cMunicipio'];
            ...
            if ($aConfig['mailFROM'] != ''){
                $this->aMAIL = array('mailFROM'=>$aConfig['mailFROM'],'mailHOST'=>$aConfig['mailHOST'],'mailUSER'=>$aConfig['mailUSER'],'mailPASS'=>$aConfig['mailPASS']);
            }
            ...
            if ( is_file($this->raizDir . 'config' . DIRECTORY_SEPARATOR . 'config.php') ){
                ...
                if ($mailFROM != ''){
                    $this->aMail = array('mailFROM'=>$proxyFROM,'mailHOST'=>$mailHOST,'mailUSER'=>$mailUSER,'mailPASS'=>$mailPASS);
                }
        ...
        $this->entDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'entradas' . DIRECTORY_SEPARATOR;
        $this->assDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'assinadas' . DIRECTORY_SEPARATOR;        
        $this->valDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'validadas' . DIRECTORY_SEPARATOR;
        $this->rejDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'rejeitadas' . DIRECTORY_SEPARATOR;
        $this->envDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'enviadas' . DIRECTORY_SEPARATOR;
        $this->aprDir=$this->envDir . DIRECTORY_SEPARATOR . 'aprovadas' . DIRECTORY_SEPARATOR;
        $this->denDir=$this->envDir . DIRECTORY_SEPARATOR . 'denegadas' . DIRECTORY_SEPARATOR;
        $this->repDir=$this->envDir . DIRECTORY_SEPARATOR . 'reprovadas' . DIRECTORY_SEPARATOR;
        $this->canDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'canceladas' . DIRECTORY_SEPARATOR;
        $this->inuDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'inutilizadas' . DIRECTORY_SEPARATOR;
        $this->temDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'temporarias' . DIRECTORY_SEPARATOR;
        $this->recDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'recebidas' . DIRECTORY_SEPARATOR;
        $this->conDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'consultadas' . DIRECTORY_SEPARATOR;
        $this->pdfDir=$this->arqDir . DIRECTORY_SEPARATOR . $sAmb . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR;
        ...
        $this->mURL = $this->loadSEFAZ($this->raizDir . 'config'. DIRECTORY_SEPARATOR . $this->xmlURLfile,$this->tpAmb,$this->cMunicipio);
        ...
        if ( count($aName ) > 0){
            for ( $x=0; $x < count($aName); $x++ ) {
                ...
                if ( $nfefile = file_get_contents($filename) ) {
                    ...
                    //mantenha desse jeito mesmo com um unico =
                    //a atribuição da variavel e o teste são simultâneos
                    if ( $signn = $this->__signXML($nfefile, 'infNFe') ) {
        ...
        if ( count($aName) > 0 ){
            for ( $x=0; $x < count($aName); $x++ ) {
                ...
                if ( $nfefile = file_get_contents($filename) ) {
                    ...
                    if ( $this->__validXML($nfefile,$filexsd) ) {
    ...
    public function autoPrintSend($para='',$contato='',$printer=''){
        ...
        if ( count($aNApr) > 0 ) {
            ...
            foreach ( $aNApr as $file ) {
                ...
                if ( $printer != '' ) {
                    $command = "lpr -P $printer $this->pdfDir$pdfName";
                    system($command);
                }
                //arquivo da NFe com o protocolo
               ...
               if ($email != '' ) {
                    ...
                    $aEMail = array('emitente'=>$emitente,'para'=>$email,'contato'=>$contato,'razao'=>$razao,'numero'=>$numero,'serie'=>$serie,'vtotal'=>$vtotal);
    ...
    */
     ...
     */
    public function __signXML($docxml, $tagid=''){
            if ( $tagid == '' ){
    ...
    **/
    public function statusServico($UF='',$tpAmb=''){
        ...
        $cabec = '<nfeCabecMsg xmlns="'. $namespace . '"><versaoDados>'.$versao.'</versaoDados><cUF>'.$cUF.'</cUF></nfeCabecMsg>';
        //montagem dos dados da mensagem SOAP
        $dados = '<nfeDadosMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/NfeStatusServico2"><consStatServ xmlns="'.$this->URLPortal.'" versao="'.$versao.'"><tpAmb>'.$tpAmb.'</tpAmb><cUF>'.$cUF.'</cUF><xServ>STATUS</xServ></consStatServ></nfeDadosMsg>';
        $retorno = $this->__sendSOAP($urlservico, $namespace, $cabec, $dados, $metodo, $tpAmb);
    ...
    public function consultaCadastro($UF,$CNPJ='',$IE='',$CPF=''){
        ...
        //montagem dos dados da mensagem SOAP
        $dados = '<nfeDadosMsg><consCad xmlns="' . $this->URLPortal . '" xmlns:xsi="' . $this->URLxsi . '" xmlns:xsd="'. $this->URLxsd . '" versao="' .$versao . '"><infCons><xServ>CONS-CAD</xServ><UF>' . $UF . '</UF>' . $filtro . '</infCons></consCad><nfeDadosMsg>';

        $retorno = $this->__sendSOAP2($urlservico, $namespace, $cabec, $dados, $metodo);
        //se houve retorno
        if ( is_array($retorno) ) {
            //pegar o xml retornado
            $xmlresp = utf8_encode($retorno[$this->aURL[$servico]['method'].'Result']);
            if ( $xmlresp == ''){
    ...
    **/
    public function sendLot($aNFe,$idLote) {
        ...
        $sNFe = str_replace('<?xml version="1.0" encoding="UTF-8" standalone="no"?>','',$sNFe);

        //montagem do cabeçalho da comunicação SOAP
        $cabec = '<nfeCabecMsg xmlns="'. $namespace . '"><versaoDados>' . $versao . '</versaoDados><cUF>'.$this->cUF.'</cUF></nfeCabecMsg>';
        //montagem dos dados da mensagem SOAP
        //$dados = '<nfeDadosMsg><enviNFe xmlns="' . $this->URLPortal . '" xmlns:xsi="' . $this->URLxsi . '" xmlns:xsd="'. $this->URLxsd . '" versao="' . $versao . '"><idLote>' . $idLote . '</idLote>' . $sNFe . '</enviNFe></nfeDadosMsg>';
        $dados = '<nfeDadosMsg><enviNFe xmlns="' . $this->URLPortal . '" versao="' . $versao . '"><idLote>' . $idLote . '</idLote>' . $sNFe . '</enviNFe></nfeDadosMsg>';
        //envia dados via SOAP
        $retorno = $this->__sendSOAP($urlservico, $namespace, $cabec, $dados, $metodo);
        //verifica o retorno do SOAP
		if ($this->errStatus)
		{
			return;
        ...
        if ( isset($retorno) ) {
			...
			$aRet['dhRecbto'] = $doc->getElementsByTagName('dhRecbto')->item(0)->nodeValue;
			$aRet['nRec'] = $doc->getElementsByTagName('nRec')->item(0)->nodeValue;
            //salvar o xml retornado do SEFAZ na pasta de arquivos temporários
...
//echo "servico: $urlservico - metodo: $metodo";
		  $retorno = $this->__sendSOAPNFSe($urlservico, $cabec, $dados, $metodo);

//echo "</pre><br>Resposta<pre>";print_r($retorno);echo "</pre>";
        //verifica o retorno do SOAP
//echo "<br>Erros:<pre>";print_r($this->errMsg);
//echo "</pre><br>\n\nDebug:\n\n<pre>";print_r($this->soapDebug);
//echo "</pre><br>\n\nResposta\n\n<pre>";print_r($retorno);echo "</pre>";
//echo "<textarea style='width:80%; height: 80%'>".($this->soapRequest)."</textarea>";
		if ($this->errStatus)
		{
			return;
		}
...
//echo "</pre><br>Resposta<pre>";print_r($retorno);echo "</pre>";
        //verifica o retorno do SOAP
//echo "servico: $urlservico - metodo: $metodo<br>";
//echo "<textarea style='width:80%; height: 200px'>$dados</textarea>";
//echo "<br>Erros:<pre>";print_r($this->errMsg);
//echo "</pre><br>\n\nDebug:\n\n<pre>";print_r($this->soapDebug);
//echo "</pre><br>\n\nResposta\n\n<pre>";print_r($retorno);echo "</pre>";
//EXIT;
...
//EXIT;
  ...
  /**
     * Solicita o cancelamento de NFS enviada
     *
     * @name cancelNFS
     * @version 2.0
     * @package NFePHP
     ...
     * @return	boolean TRUE se sucesso ou FALSE se falha
    **/
    public function cancelNFS($idNFe,$protId, $xJust, $bSave=TRUE){
    }
     ...
     * @return	Array    array(status = boolean, array(chaves,protocolos))
    **/
    public function getProtocol($recibo='',$chave=''){
        $aProt = array('chave'=>'','protocolo'=>'');
        $aRet = array('status'=>FALSE,'cStat'=>'','xMotivo'=>'',$aProt);

        if ($recibo == '' && $chave == '') {
            $this->errStatus = TRUE;
            $this->errMsg = 'Indicar o numero do recibo ou a chave de acesso da NFe!!';
            return FALSE;
        }
        if ($recibo != '' && $chave != '') {
            $this->errStatus = TRUE;
            $this->errMsg = 'Indicar somente um dos dois dados ou o numero do recibo ou a chave de acesso da NFe!!';
            return FALSE;
        }

        //condulta pelo recibo
        if ($recibo != '' && $chave == '') {
            //buscar o protocolo pelo numero do recibo
            //identificação do serviço
            ...
            $cabec = '<nfeCabecMsg xmlns="'. $namespace . '"><cUF>'.$this->cUF.'</cUF><versaoDados>' . $versao . '</versaoDados></nfeCabecMsg>';
            //montagem dos dados da mensagem SOAP
            //$dados = '<nfeDadosMsg><consReciNFe xmlns="' . $this->URLPortal . '" xmlns:xsi="' . $this->URLxsi . '" xmlns:xsd="'. $this->URLxsd . '" versao="' . $versao . '"><tpAmb>' . $this->tpAmb . '</tpAmb><nRec>' . $recibo . '</nRec></consReciNFe></nfeDadosMsg>';
            $dados = '<nfeDadosMsg><consReciNFe xmlns="' . $this->URLPortal . '" versao="' . $versao . '"><tpAmb>' . $this->tpAmb . '</tpAmb><nRec>' . $recibo . '</nRec></consReciNFe></nfeDadosMsg>';
        ...
        if ($recibo == '' &&  $chave != ''){
            ...
            $cabec = '<nfeCabecMsg xmlns="'. $namespace . '"><cUF>'.$this->cUF.'</cUF><versaoDados>' . $versao . '</versaoDados></nfeCabecMsg>';
            //montagem dos dados da mensagem SOAP
            //$dados = '<nfeDadosMsg><conSitNFe xmlns="' . $this->URLPortal . '" xmlns:xsi="' . $this->URLxsi . '" xmlns:xsd="'. $this->URLxsd . '" versao="' . $versao . '"><tpAmb>' . $this->tpAmb . '</tpAmb><xServ>CONSULTAR</xServ><chNFe>' . $chave . '<chNFe></conSitNFe></nfeDadosMsg>';
            $dados = '<nfeDadosMsg><conSitNFe xmlns="' . $this->URLPortal . '" versao="' . $versao . '"><tpAmb>' . $this->tpAmb . '</tpAmb><xServ>CONSULTAR</xServ><chNFe>' . $chave . '<chNFe></conSitNFe></nfeDadosMsg>';
        ...
        //envia dados via SOAP
//echo "<pre>";print_r(array($urlservico, $namespace, $cabec, $dados, $metodo));exit;
        $retorno = $this->__sendSOAP($urlservico, $namespace, $cabec, $dados, $metodo);
//echo "<pre>";print_r($retorno);exit;
        ...
        if ( isset($retorno) ) {
            ...
            $aRet['status'] = TRUE;
            // codigo da mensagem da SEFAZ
            $aRet['cStat'] = $doc->getElementsByTagName('cStat')->item(0)->nodeValue;
            // motivo 
            $aRet['xMotivo'] = $doc->getElementsByTagName('xMotivo')->item(0)->nodeValue;
            $nome = $this->temDir.$nomeArq.'-prot.xml';
            $nome = $doc->save($nome);
    ...
    **/
    public function inutNF($ano,$serie,$numIni,$numFim,$xJust){
        $modelo="55";
        //identificação do serviço
        ...
        //UF + Ano (2 posições) + CNPJ + modelo + série + nro inicial e nro final precedida do literal “ID”
        $id = 'ID' . $this->cUF . $ano . $this->cnpj . $modelo . str_pad($serie,3,"0",STR_PAD_LEFT) . str_pad($numIni,9,"0",STR_PAD_LEFT) . str_pad($numFim ,9,"0",STR_PAD_LEFT) ;
        //montagem do cabeçalho da comunicação SOAP
        $cabec = '<nfeCabecMsg xmlns="'. $namespace . '"><versaoDados>' . $versao . '</versaoDados><cUF>'.$this->cUF.'</cUF></nfeCabecMsg>';

        //dados da mensagem
        $nfeDadosMsg = '<inutNFe xmlns="'.$this->URLnfe.'" versao="'.$versao.'"><infInut Id="'.$id.'"><tpAmb>'.$this->tpAmb.'</tpAmb><xServ>INUTILIZAR</xServ><cUF>'.$this->cUF.'</cUF><ano>'.$ano.'</ano><CNPJ>'.$this->cnpj.'</CNPJ><mod>'.$modelo.'</mod><serie>'.$serie.'</serie><nNFIni>'.$numIni.'</nNFIni><nNFFin>'.$numFim.'</nNFFin><xJust>'.$xJust.'</xJust></infInut></inutNFe>';
        //assinar a nfeDadosMsg
        $nfeDadosMsg = $this->__signXML($nfeDadosMsg, 'infInut');
        $nfeDadosMsg = str_replace('<?xml version="1.0"?>','',$nfeDadosMsg);
        $nfeDadosMsg = str_replace('<?xml version="1.0" encoding="utf-8"?>','',$nfeDadosMsg);
        $nfeDadosMsg = str_replace('<?xml version="1.0" encoding="UTF-8" standalone="no"?>','',$nfeDadosMsg);
		  
		  $nfeDadosMsg = '<nfeDadosMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/'. $servico . '2">'. $nfeDadosMsg .'</nfeDadosMsg>';

		  $retorno = $this->__sendSOAP($urlservico, $namespace, $cabec, $nfeDadosMsg, $metodo);
        if (isset($retorno)) {
            $xmlresp = utf8_encode($retorno);
            if ($xmlresp == ''){
                //houve uma falha na comunicação SOAP
     ...
     * @return	boolean TRUE se sucesso ou FALSE se falha
    **/
    public function cancelNF($idNFe,$protId, $xJust, $bSave=TRUE){
        //identificação do serviço
        ...
        $cabec = '<nfeCabecMsg xmlns="'. $namespace . '"><versaoDados>' . $versao . '</versaoDados><cUF>'.$this->cUF.'</cUF></nfeCabecMsg>';
        //montagem dos dados da mensagem SOAP
        //$dXML = '<cancNFe xmlns="' . $this->URLPortal . '" xmlns:xsi="' . $this->URLxsi . '" xmlns:xsd="'. $this->URLxsd . '" versao="' . $versao . '">';
        $dXML = '<cancNFe xmlns="' . $this->URLPortal . '" versao="' . $versao . '">';
        $dXML .= '<infCanc Id="ID' . $idNFe . '"><tpAmb>' . $this->tpAmb . '</tpAmb><xServ>CANCELAR</xServ><chNFe>' . $idNFe . '</chNFe><nProt>' . $protId . '</nProt><xJust>' . $xJust . '</xJust></infCanc></cancNFe>';
        //assinar SOAP
        $dSOAP = $this->__signXML($dXML, 'infCanc');
        $nfeDadosMsg = '<nfeDadosMsg>' . $dSOAP . '</nfeDadosMsg>';
        $nfeDadosMsg = str_replace('<?xml version="1.0"?>','',$nfeDadosMsg);
        $nfeDadosMsg = str_replace('<?xml version="1.0" encoding="utf-8"?>','',$nfeDadosMsg);
        $nfeDadosMsg = str_replace('<?xml version="1.0" encoding="UTF-8" standalone="no"?>','',$nfeDadosMsg);
        //variavel de retorno
        $bRet = FALSE;
//echo $nfeDadosMsg;exit;
			//echo $cabec;
		  $retorno = $this->__sendSOAP($urlservico, $namespace, $cabec, $nfeDadosMsg, $metodo);
        if (isset($retorno)) {
            $xmlresp = utf8_encode($retorno);
    ...
    */
    public function __validCerts($cert){
        $flagOK = true;
        $errorMsg = "";
     ...
     * @access   private
     **/
    public function __cleanCerts($certFile){
...
//echo "<pre>";print_r($resposta);exit;
     ...
     * @return mixed False se houve falha ou o retorno em xml do SEFAZ
     */
    public function __sendSOAP($urlsefaz,$namespace,$cabecalho,$dados,$metodo,$ambiente){
        ...
        if(substr($namespace,-1) == '2' ) {
            $soapver = SOAP_1_2;
            if(substr($urlsefaz,0,4) != 'http'){
                $urlsefaz = "$this->URLbase/wsdl/2.00/$ambiente/$urlsefaz";
            }
        } else {
            $soapver = SOAP_1_1;
            if(substr($urlsefaz,0,4) != 'http'){
                $urlsefaz = "$this->URLbase/wsdl/1.10/$ambiente/$urlsefaz";
            }
        }
...
//echo "<pre>$URL\n\n";print_r($options);exit;
        ...
        if(substr($namespace,-1) == '2' ) {
            ...
            $aBody = array('nfeCabecMsg'=>$cabecalho,'nfeDadosMsg'=>$dados);
            //faz a chamada ao metodo do webservices
            try{
                //$resp = $oSoapClient->$metodo($aBody);
                $resp = $oSoapClient->__soapCall($metodo, $aBody);
...
//print_r($this->soapDebug);
     ...
     * @return mixed False se houve falha ou o retorno em xml do SEFAZ
     */
    public function __sendSOAP2($urlsefaz,$namespace,$cabecalho,$dados,$metodo,$ambiente){
     ...
     */
    public function __putNumLot($num){
        if ( is_numeric($num) ){
            $lotfile = $this->raizDir . 'config/numloteenvio.xml';
...
//echo "<textarea style='width:100%; height: 100%'>".($request)."</textarea>";exit;
