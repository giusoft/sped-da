 ...
 * @todo Incluir as funções introduzidas na classe code128 numa classe auxiliar
 *       neste arquivo, para manter a classe code128 inalterada.
 */
...
//fim construct
    ...
    public function montaDANFE($orientacao = 'P', $papel = 'A4', $logoAlign = 'C') {
        ...
        while ($i < $this->det->length) {
            ...
            $hUsado += round(($numlinhas * $this->pdf->FontSize) + 2, 0);
            $i += 1;
        } //fim da soma das areas de itens usadas
        //calculo do numero de paginas necessarias
        //tem que somar 5 pois tenho um exemplo de uma NF com exatamente 25
        //produtos que dá problema... Coisas de arredondamento.
        if ($hUsado > $hDispo1) {
            //serão necessárias mais paginas
            $hOutras = $hUsado - $hDispo1;
            $totPag = 1 + ceil($hOutras / $hDispo2);
            if ($totPag == 2 && $hUsado < 134)
                $totPag = 1;
        } else {
...
//fim função printDANFE
     ...
     */
    private function __GeraInformacoesDaTagCompra() {
        $saida = "";
     ...
     * @return string chave
     */
    private function __GeraChaveAdicionalDeContingencia() {
     ...
     */
    private function __GeraInformacoesDasNotasReferenciadas() {
...
//fim da fun��o __faturaDANFE
     ...
     * @param number $h T�tulo do campo
     * @param number $h Valor do imposto
     */
    private function __impostoDanfeHelper($x, $y, $w, $h, $titulo, $valorImposto) {
    ...
    private function __itensDANFE($x, $y, &$nInicio, $hmax, $pag = 0, $totpag = 0) {
        ...
        foreach ($this->det as $d) {
            if ($i >= $nInicio) {
                ...
                $h = round(($linhaDescr * $this->pdf->FontSize) + 1.1, 0);
                $hUsado += $h;
                if ($hUsado >= $hmax) {
                ...
                $x += $w6;
                $texto = number_format($prod->getElementsByTagName("qCom")->item(0)->nodeValue, 3, ",", ".");
                $this->__textBox($x, $y, $w7, $h, $texto, $aFont, 'T', 'R', 0, '');
                $x += $w7;
                $texto = number_format($prod->getElementsByTagName("vUnCom")->item(0)->nodeValue, 6, ",", ".");
...
//fim __dadosAdicionaisDANFE
    ...
    private function __rodapeDANFE() {
        ...
        $texto = "GiuSoft Tecnologia - www.giusoft.com.br";
        $aFont = array('font' => $this->fontePadrao, 'size' => 6, 'style' => 'I');
