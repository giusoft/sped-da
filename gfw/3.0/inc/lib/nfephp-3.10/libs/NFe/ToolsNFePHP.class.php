...
class ToolsNFePHP extends CommonNFePHP
{
    ...
    public function statusServico($siglaUF = '', $tpAmb = '', &$aRetorno = array())
    {
        try {
            ...
            //Se for BA não tem o 2 no final
            $dois=$cUF<>"29" ? '2' : '';
            $namespace = $this->URLPortal.'/wsdl/'.$servico.$dois;
     ...
     * @return mixed string XML do retorno do webservice, ou false se ocorreu algum erro
     */
    public function autoriza($sxml, $idLote, &$aRetorno = array(), $indSinc = null)
    {
        if ($indSinc == null) {
            $indSinc = $this->indSinc;
        }
        $indSinc = (int) $indSinc;
        ...
        try {
            ...
            if ($chave != '') {
                ...
                if (in_array($aRetorno['cStat'], array('100', '101', '110', '301', '302'))) {
                    $protNfe = $doc->getElementsByTagName('protNFe')->item(0);
    ...
    public function envCCe($chNFe = '', $xCorrecao = '', $nSeqEvento = '1', $tpAmb = '', &$aResp = array())
    {
        ...
        try {
            ...
            if (!$retorno) {
                //não houve retorno
                $msg = "Nao houve retorno Soap verifique a mensagem de erro e o debug!!";
                throw new nfephpException("Retorno de ERRO: " . $retorno);
            }
            //tratar dados de retorno
            /*
            ...
            */
            ...
            $retorno = preg_replace('/^\xEF\xBB\xBF/', '', trim($retorno));

            $doc = new DOMDocument('1.0', 'utf-8');
            $ok = $doc->loadXML($retorno, LIBXML_NOBLANKS);

            if (!$ok) {
                foreach (libxml_get_errors() as $e) {
                    error_log("Retorno de ERRO: " . trim($e->message));
                }
                $msg = 'XML inválido';
                throw new nfephpException("Retorno de ERRO: XML invalido");
            }
            $xp = new DOMXPath($doc);
            $xp->registerNamespace('soap', 'http://www.w3.org/2003/05/soap-envelope');
            $xp->registerNamespace('wsdl', 'http://www.portalfiscal.inf.br/nfe/wsdl/NFeRecepcaoEvento4');
            $xp->registerNamespace('nfe',  'http://www.portalfiscal.inf.br/nfe');

            $cStat   = '';
            $xMotivo = '';

            $n = $xp->query('//nfe:retEnvEvento/nfe:retEvento/nfe:infEvento')->item(0);
            if ($n) {
                $cStat   = $xp->evaluate('string(nfe:cStat)', $n);
                $xMotivo = $xp->evaluate('string(nfe:xMotivo)', $n);
            } else {
                $n = $xp->query('//nfe:retEnvEvento/nfe:infEvento')->item(0);
                if ($n) {
                    $cStat   = $xp->evaluate('string(nfe:cStat)', $n);
                    $xMotivo = $xp->evaluate('string(nfe:xMotivo)', $n);
                }
            ...
            $xmlenvCCe->loadXML($Ev, LIBXML_NOBLANKS);
	...
	global $EMPRESA;
		...
		if($EMPRESA=='logicpe')
            curl_setopt($oCurl, CURLOPT_SSLVERSION, 6); // Sefaz-BA 4 , RJ 3
else
...
file_put_contents('/tmp/nfe.log', $data."\n------------------------------\n", FILE_APPEND);
foreach ($info as $k=>$v)
{
    file_put_contents('/tmp/nfe.log', $k." = ".$v."\n", FILE_APPEND);
}
file_put_contents('/tmp/nfe.log', "\n------------------------------\n", FILE_APPEND);
file_put_contents('/tmp/nfe.log', $xml."\n------------------------------\n", FILE_APPEND);
file_put_contents('/tmp/nfe.log', "\n------------------------------\n\n", FILE_APPEND);
file_put_contents('/tmp/nfe.log', "\n HEADER------------------------------\n", FILE_APPEND);
file_put_contents('/tmp/nfe.log', json_encode($parametros)."\n------------------------------\n", FILE_APPEND);
file_put_contents('/tmp/nfe.log', "\n------------------------------\n\n", FILE_APPEND);
            $txtInfo ="";
