...
class PowerGraphic {
	...
	function start()
	{
		...
		foreach ($_GET as $parameter => $value)
		{
				if (preg_match("/^x\d+$/i", $parameter))
				{
					...
					if (strtolower($parameter) == 'fixed_width') $this->fixed_width=$value;
					if (strtolower($parameter) == 'width') $this->width=$value;
		...
		if ((!$this->fixed_size) || ($this->fixed_size==""))
		{
	...
	function ttftext($img, $size, $angle, $x, $y, $color, $font, $string)
	{
		imagettftext($img, $size, $angle, $x, $y+11, $color, $font, $string);
	}
	...
	function create_graphic()
	{
		...
		$this->gradient_region($this->img, 0, 0, $this->width, $this->height,$this->color['background'], $this->color['background_end']);

		// Draw title
		if (!empty($this->title))
		{
				$center = ($this->width / 2) - ($this->string_width($this->title, 10) / 2);
				//imagestring($this->img, 5, $center, 10, $this->title, $this->color['title']);
				$this->ttftext($this->img, 10, 0, $center, 20, $this->color['title'], $this->fontfile,$this->title);
		}
		...
		if (ereg("^(1|3|4)$", $this->type))
		{
				...
				$less= $this->string_width("0",8) * strlen($this->higher_value_str);
				...
				$this->ttftext($this->img, 8, 0, ($this->graphic_area_x1-$less-7), ($higher_value_y-7), $this->color['axis_values'], $this->fontfile,$this->higher_value_str);
				for ($i = 1; $i < 10; $i++)
				{
					...
					if ($i % 2 == 0) {
						$value = $this->number_formated($this->higher_value * $i / 10);
							$less= $this->string_width("0",8) * strlen($value);
						//$less = 7 * strlen($value);
						
						//imagestring($this->img, 3, ($x1-$less-7), ($y2-7), $value, $this->color['axis_values']);
						$this->ttftext($this->img, 8, 0, ($x1-$less-7), ($y2-7), $this->color['axis_values'], $this->fontfile,$value);
					}
				}

				// Axis X
				//imagestring($this->img, 3, $this->graphic_area_x2+10, $this->graphic_area_y2+3, $this->axis_x, $this->color['title']);
				$this->ttftext($this->img, 8, 0, $this->graphic_area_x2+10, $this->graphic_area_y2+3, $this->color['title'], $this->fontfile,$this->axis_x);
				...
				$this->ttftext($this->img, 8, 0, 20, $this->graphic_area_y1-20, $this->color['title'], $this->fontfile,$this->axis_y);
		...
		else if ($this->type == 2)
		{
				...
				$this->ttftext($this->img, 8, 0, (($this->graphic_area_x1+$higher_value_size) - ($this->string_width($this->higher_value, 3)/2)), ($this->graphic_area_y2+2), $this->color['axis_values'],$this->higher_value_str);
				...
				for ($i = 1, $alt = 15; $i < 10; $i++)
				{
					...
					if ($i % 2 == 0) {
						...
						$this->ttftext($this->img, 8, 0, (($this->graphic_area_x1+$dec_x) - ($this->string_width($this->higher_value, 3)/2)), ($this->graphic_area_y2+$alt), $this->color['axis_values'], $this->fontfile,$value);
				...
				$this->ttftext($this->img, 8, 0, ($this->graphic_area_x2+10), ($this->graphic_area_y2+3), $this->color['title'], $this->fontfile,$this->axis_y);
				imageline($this->img, $this->graphic_area_x1, $this->graphic_area_y2, $this->graphic_area_x2, $this->graphic_area_y2, $this->color['axis_line']);
				// Axis Y
				//imagestring($this->img, 3, 20, ($this->graphic_area_y1-20), $this->axis_x, $this->color['title']);
				$this->ttftext($this->img, 8, 0, ($this->graphic_area_y1-20), $this->axis_x, $this->color['title'], $this->fontfile,$this->axis_x);
				imageline($this->img, $this->graphic_area_x1, $this->graphic_area_y1, $this->graphic_area_x1, $this->graphic_area_y2, $this->color['axis_line']);
		}
		...
		*/
		if ($this->type == 1)
		{
				...
				foreach ($this->x as $i => $parameter)
				{
					...
					$this->ttftext($this->img, 8, 0, ((($x1+$x2)/2) - (strlen($parameter)*7/2)),($y2+$alt+2),$this->color['axis_values'], $this->fontfile,$parameter);
				}
		}
		...
		else if ($this->type == 2)
		{
				...
				foreach ($this->x as $i => $parameter)
				{
					if (isset($this->z[$i])) {
						...
						$this->ttftext($this->img, 8, 0, ($x2+7), ($y1+7), $this->color['bars_2_shadow'], $this->fontfile,$this->number_formated($this->z[$i], 2));
					}
					...
					$this->ttftext($this->img, 8, 0, ($x2+7), ($y1+2), $this->color['bars_shadow'], $this->fontfile, $this->number_formated($this->y[$i], 2));

					//imagestring($this->img, 3, ($x1 - ((strlen($parameter)*7)+7)), ($y1+2), $parameter, $this->color['axis_values']);
					$this->ttftext($this->img, 8, 0,($x1 - ((strlen($parameter)*7)+7)), ($y1+2), $this->color['axis_values'], $this->fontfile,$parameter);
				}
		}
		...
		*/
		else if (ereg("^(3|4)$", $this->type))
		{
				...
				foreach ($this->x as $i => $parameter)
				{
					...
					$this->ttftext($this->img, 8, 0, ($x[$i] - (strlen($parameter)*7/2 )), ($this->graphic_area_y2+$alt+2), $this->color['axis_values'], $this->fontfile, $parameter);
	...
	function calculate_width()
	{
		switch ($this->type)
		{
				// Vertical bars
				case 1:
					$this->legend_box_width   = ($this->legend_exists == true) ? ($this->string_width($this->biggest_graphic_name, 3) + 75) : 0;
				...
				// Pie
				case 5:
					$this->legend_box_width   = $this->string_width($this->biggest_x, 3) + 205;
		...
		$this->graphic_area_x2 = $this->graphic_area_x1 + $this->graphic_area_width;
		$this->legend_box_x1   = $this->graphic_area_x2 + 15;
	...
	function draw_legend()
	{
		...
		if (ereg("^(1|2|3|4)$", $this->type))
		{
				...
				$this->ttftext($this->img, 8, 0, ($x+15), ($y-2), $this->color['axis_values'], $this->fontfile, $this->graphic_1);
				$y += 20;
				imagefilledrectangle($this->img, $x, $y, ($x+10), ($y+10), $color_2);
				imagerectangle($this->img, $x, $y, ($x+10), ($y+10), $this->color['title']);
				//imagestring($this->img, 3, ($x+15), ($y-2), $this->graphic_2, $this->color['axis_values']);
				$this->ttftext($this->img, 8, 0, ($x+15), ($y-2), $this->color['axis_values'], $this->fontfile, $this->graphic_2);
		}
		...
		else if (ereg("^(5|6)$", $this->type))
		{
				if (!empty($this->axis_x))
				{
					...
					$this->ttftext($this->img, 8, 0, ((($x1+$x2)/2) - (strlen($this->axis_x)*7/2)), $y, $this->color['title'], $this->fontfile, $this->axis_x);
					$y += 25;
				}
				...
				foreach ($this->x as $i => $parameter)
				{
					...
					$less= $this->string_width("0",8) * strlen($this->higher_value_str)+28;
					...
					$this->ttftext($this->img, 8, 0, ($x+15), ($y-2), $this->color['axis_values'], $this->fontfile, $parameter);
					//imagestring($this->img, 2, ($x2-$less), ($y-2), $percent, $this->color['axis_values']);
					$this->ttftext($this->img, 8, 0, ($x2-$less), ($y-2), $this->color['axis_values'], $this->fontfile, $percent);
					$y += 14;
					$num++;
				}
		}
	...
	function string_width($string, $size) {
		...
		$single_width =imagettfbbox($size,0,$this->fontfile,$string);
	...
	function number_formated($number, $dec_size = 0)
	{
	...
	function load_color_palette()
	{
		switch ($this->skin)
		{
				...
				case 1:
					...
					$this->color['background_end']  = imagecolorallocate($this->img, 240, 240, 240);
				...
				case 2:
					...
					$this->color['background_end']  = imagecolorallocate($this->img, 40, 40, 40);
				...
				case 3:
					...
					$this->color['background']  = imagecolorallocate($this->img, 200, 200, 240);
					$this->color['background_end']  = imagecolorallocate($this->img, 240, 240, 255);
					...
					$this->color['bg_legend']   = imagecolorallocate($this->img, 240, 240, 240);
...
<input type="submit" value="Create" style="cursor: pointer; margin: 10px 0px 0px 60px;" />
	...
	function create_query_string() {
		...
		$graphic['fixed_width']= $this->fixed_width;
