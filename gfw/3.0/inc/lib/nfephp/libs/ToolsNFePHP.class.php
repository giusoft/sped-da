...
class ToolsNFePHP
{
    ...
    */
	...
	/**
     * enableSVC
     * Habilita contingência ao serviço SVC ao invés do webservice estadual
     * @var boolean
     */
	...
	/**
	 * SCVlist
	 * Lista do ambiente SVC para cada estado
	 */
	 private $SVClist = array('AC'=>'SVCAN',
                               'AL'=>'SVCAN',
                               'AM'=>'SVCRS',
                               'AP'=>'SVCAN',
                               'BA'=>'SVCRS',
    ...
    function __construct($aConfig = '', $mododebug = 2, $exceptions = false)
    {
        ...
        if (is_array($aConfig)) {
			...
			$this->enableSVC = $aConfig['enableSVC'];
    ...
    */
    ...
    public function statusServico($UF = '', $tpAmb = '', $modSOAP = '2', &$aRetorno = '')
    {
        try {
            ...
            } elseif($this->enableSVC) {
            	$aURL = $this->loadSEFAZ( $this->raizDir . 'config' . DIRECTORY_SEPARATOR . $this->xmlURLfile, $tpAmb, $this->SVClist[$UF]);
            } 
            else {
    ...
    public function sendLot($mNFe,$idLote,$modSOAP='2')
    {
        ...
        //verifica se o SCAN esta habilitado
        if ((!$this->enableSCAN) && (!$this->enableSVC) ) {
            $aURL = $this->aURL;
        } elseif($this->enableSVC) {
        	$aURL = $this->loadSEFAZ( $this->raizDir . 'config' . DIRECTORY_SEPARATOR . $this->xmlURLfile, $this->tpAmb, $this->SVClist[$this->UF]);
        }
		else {
    ...
    public function cancelEvent($chNFe = '', $nProt = '', $xJust = '', $tpAmb = '', $modSOAP = '2', &$aRetorno = array()){
        try {
            ...
            if ((!$this->enableSCAN) && (!$this->enableSVC)) {
            	...
            	$aURL = $this->loadSEFAZ( $this->raizDir . 'config' . DIRECTORY_SEPARATOR . $this->xmlURLfile,$tpAmb,$this->SVClist[$this->UF]);
            } 
            else {
                $aURL = $this->loadSEFAZ( $this->raizDir . 'config' . DIRECTORY_SEPARATOR . $this->xmlURLfile,$tpAmb,'SCAN');
            }
