...
//		$mtz['labelWidth']="'0'";
	...
	function get()
	{
		...
		$this->name=$name;
		// 1 coluna
		$defaults="{bodyStyle:'padding:5px 5px 5px 5px', style: 'text-align: left', frame:true, defaultType:'textfield', collapsible: false, submitEmptyText: true, standardSubmit: true, modal:true, width: '100%', height: '100%'}";
		$defaults="{fieldset:}";
	...
	function render()
	{
		...
		if ($this->hasGps<>'')
		{
				$gpsTxt="onActivate: function(){Ext.Msg.alert('Hello!')},";
		}
		if ($title<>'')
			$titulo=$title;
		// ======================= formulário
