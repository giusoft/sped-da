<?php
if((gVar("nfe.version")=="2.0") && (!strpos($_SERVER['SCRIPT_NAME'],"nfse") > 0))
{
	include 'gNfe_NEW.php';
}
else
{
	define("NL", "\n");
	define('NFEPHP', "nfephp-2.0");
	define('M', '-          ');
	...
	if ($gPathLib <> "") {
	    ...
	    include $gPathLib . NFEPHP . "/libs/ToolsNFePHP_.php";
	    include $gPathLib . NFEPHP . "/libs/DacceNFePHP.class.php";
	} else {
	    //include $gPathDefault.NFEPHP."/libs/ToolsNFePHP.class.php";
	    include $gPathDefault . NFEPHP . "/libs/ToolsNFePHP_.php";
	    include $gPathDefault . NFEPHP . "/libs/DacceNFePHP.class.php";
	}
	...
	class gNFe extends ConvertNFePHP
	{
	    ...
	    public $c, $i, $s;
	    public $nfetxt, $nfexml, $nfeid, $recibo, $dataRecibo, $protocolo, $numeroNota, $numeroLote;
	    public $nfsexml;
	    public $erros;
	    ...
	    private $versao, $schemes, $nfeTools, $protfile;

	    function __construct($tipo = "nfe", $verificaStatus = true)
	    {
	        ...
	        if ($verificaStatus && $tipo == "nfe") {
	            ...
	            $rs = gQuery($sql);
	            if ($rs->fields['modo_operacao'] == "3") { // SCAN
	                $this->c['tipoEmissao'] = "3"; // 1) Normal, 2 em diante 3) Contingencia SCAN
	                $this->c['serie'] = $rs->fields['serie'];
	                $this->c['dhCont'] = str_replace(" ", "T", $rs->fields['data']);
	    ...
	    function config()
	    {
	        ...
	        $cfg['baseurl'] = $http_lib . "nfephp-2.0/";
	        $cfg['schemes'] = $this->schemes;
	        $cfg['arquivoURLxml'] = 'def_ws2.xml';
	        if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') {
	            $cfg['arquivosDir'] = '/';
	        } else {
	            $cfg['arquivosDir'] = '/tmp';
	        ...
	        $cfg['enableSCAN'] = $this->enableSCAN;
	    ...
	    function geraTxt()
	    {
	        ...
	        $s.="E|" . $xNome . "|" . $this->c['inscricaoEstadual'] . "||" . NL;
	        ...
	        $m[] = $this->tiraEstranhos($this->c['enderecoMunicipio']);
	        $m[] = $this->c['enderecoUf'];
	        ...
	        foreach ($this->i as $item) {
	            ...
	            if ($item['tipo'] == "importacao") {
	                ...
	                if ($item['icms'] > 0) {
		                ...
		                elseif(intval(substr($this->c['cst'], 0, 2)) == 20) // 20 - Com redução de base de cálculo
						{
							...
							"|" . number_format(($item['quantidade'] * $item['icms']), 2, ".", "") . NL /*vICMS*/;

						}
	                    else
	                    {
	                        $s.="N02|$origem|00|0|" . number_format(($item['quantidade'] * $item['baseICMS']), 2, ".", "") . "|" . number_format($item['aliquotaICMS'], 2, ".", "") . "|" . number_format(($item['quantidade'] * $item['icms']), 2, ".", "") . NL;
	                    }
	            ...
	            if ($item['ipi'] > 0) {
	                //IPI
	                $s.="O|0||||999|" . NL;
	                $s.="O07|50|" . number_format($item['ipi'] * $item['quantidade'], 2, ".", "") . NL;
	        ...
	        if ($t_desconto > 0) {
	            $tt_itens = ($total_itens + $t_ipi + $outrasDespesas + $valor_frete + $valor_seguro) - $t_desconto;
	            $t_desconto = number_format($t_desconto, 2, ".", "");
	        } else {
	            $tt_itens = ($total_itens + $t_ipi + $outrasDespesas + $valor_frete + $valor_seguro);
	            $t_desconto = "0.00";
	        }

	        $s.="W" . NL;
	        $s.="W02|" . number_format($base_calculo, 2, ".", "") . "|" . number_format($t_icms, 2, ".", "") . "|" . number_format($total_base_icms_st, 2, ".", "") . "|" . number_format($total_icms_st, 2, ".", "") . "|" . number_format($total_itens, 2, ".", "") . "|" . number_format($valor_frete, 2, ".", "") . "|" . number_format($valor_seguro, 2, ".", "") . "|$t_desconto|0.00|" . number_format($t_ipi, 2, ".", "") . "|" . number_format($t_pis, 2, ".", "") . "|" . number_format($t_cofins, 2, ".", "") . "|" . number_format($outrasDespesas, 2, ".", "") . "|" . number_format($tt_itens, 2, ".", "") . NL;
	        // Transportadora
	        if (($this->c['transportadoraCnpj'] <> "") || ($this->c['transportadoraCpf'] <> "")) {
	            ...
	            $s.="X03|" . $razao . "|" . $this->soNumeros($this->c['transportadoraInscricaoEstadual']) . "|" . $this->c['transportadoraEndereco'] . "|" . $this->c['transportadoraEnderecoUf'] . "|" . $this->c['transportadoraEnderecoMunicipio'] . NL;
	            ...
	            if ($this->c['veiculoPlaca'] <> "") {
	                ...
	                $s.="X18|" . strtoupper($this->c['veiculoPlaca']) . "|" . $placaUf . "|" .$this->c['codigoAntt'] . NL;
	            }
	            ...
	            if (intval($this->c['quantidadeEspecie']) > 0) {
	                $s.="X26|" . intval($this->c['quantidadeEspecie']) . "|" . $this->c['especieTransportada'] . "|".$this->c['especieMarca']."||" . number_format($total_pl, 3, ".", "") . "|" . number_format($total_pb, 3, ".", "") . NL;
	            } else {
	    ...
	    function geraNfse()
	    {
	        ...
	        foreach ($this->i as $item) {
				...
				$CodigoTributacaoCnae = $dom->createElement("CodigoCnae", trim($item['codigoCnae']));
				$CodigoTributacaoMunicipio = $dom->createElement("CodigoTributacaoMunicipio", '2001001');
				$descTrib = "Conforme lei 12.741/2012 o valor aproximado dos Tributos Federais, Municipais e Estadual é de R$".number_format(($item['valor']*0.1425), 2, ',', '');
	            $Discriminacao = $dom->createElement("Discriminacao", $this->tiraAcentos($item['discriminacao'].' '.$descTrib));
	            $CodigoMunicipio = $dom->createElement("CodigoMunicipio", '2927408');

	            $Servico->appendChild($Valores);
				$Servico->appendChild($ItemListaServico);
				$Servico->appendChild($CodigoTributacaoCnae);
			...
			if($this->soNumeros(trim($this->c['inscricaoMunicipal'])) <> '')
			{
				$TomadorIscricaoMunicipal = $dom->createElement("InscricaoMunicipal", $this->soNumeros(trim($this->c['inscricaoMunicipal'])) );
				$IdentificacaoTomador->appendChild($TomadorIscricaoMunicipal);
			}
	    ...
	    function protocolo($recibo = "", $chave = "")
	    {
	        ...
	        if ($aRet = $this->nfeTools->getProtocol($rc, $ch)) {
	    ...
	    function cartaCorrecao($id_nfe, $correcao)
	    {
	        ...
	        if (!$errStatus) {
	            ...
	            if ($aRet['cStat'] == "135") {
	                ...
	                $sql.="values (" . intval($usr_id) . ",$id_nfe,'$datahora','$correcao','" . $aRet['xml'] . "',1,$nSeqEvento)";
	                gQuery($sql);
	                $aRet['true'] = true;
	                return $aRet;
	            } else {
	                return $this->nfeTools->errMsg;
	            }
	        } else {
	            return $errMsg;
	        }
	    ...
	    function cce($gId, $emp, $dest = "")
	    {
	        ...
	        if (!$rs->EOF) {
	            ...
	            $cce = new DacceNFePHP($docxml, 'P', 'A4', $img, $sairPara, $aEnd, '', 'Times', 0);
	            $cce->printCCe($nomeArq, $sairPara);
	        }
	    }
	    ...
	    function cancela($chave, $protocolo, $justificativa = "Dados incorretos")
	    {
	        ...
	        if (trim($aRet['cStat']) <> '135') {
	            $sai = false;
	        } else {
	            $sql = "update nfe set xml_cancelamento='" . $aRet['xml'] . "' where chave='$chave'";
	            gQuery($sql);
	        }
	...
	function importaNfe($xml, $lc = true)
	{
		...
		if(floatval( $dados['versao']) >= 3.10)
		{
			$dados['dataEmissao'] = tagValue($doc, "dhEmi");
			$dados['dataMovimento'] = tagValue($doc, "dhSaiEnt");
		}
		else
		{
