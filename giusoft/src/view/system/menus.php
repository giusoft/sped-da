...
switch($gPage) {
	...
	case 0: // Listagem de páginas e opção pra criar uma nova
		...
		if (count($rs) > 0) 
		{
			...
			foreach ($rs as $field) 
			{
				...
				foreach ($locales as $key => $value) 
				{
					$sql = "SELECT * FROM gfw_menus where keyword='" . $field['keyword'] . "' and locale='$key'";
					$rsi = dbFastQuery($sql);
					if (count($rsi) > 0) 
					{
						foreach ($rsi as $fld) 
	...
	case 1: // Nova página
		...
		$frm->add("{name: show_at; fieldLabel: show_at; type: combo; value: " . $show_at . "; items: ".json_encode(array(gT("both"),gT("offline"),gT("online")))."}");
	...
	case 2: // Tunnel (salvar/editar)
		...
		if ($gId == 0) {
			...
			if ($type == "submenuLink") {
				...
				dbFastQuery("UPDATE gfw_menus set order2=order2+1 WHERE order1=$maxOrder1 and order2>=$maxOrder2");
			}
			...
			$tem = dbFastQuery($sql);
			if (count($tem) > 0) {
				$flds['keyword']=$flds['keyword'].rand(10000,99999);
			}
			$flds['idd'] = 0;
	...
	case 4: // Apagar (desativar)
		...
		$keyword = (gCleanField($_REQUEST['keyword']));
		$sql = "SELECT * FROM gfw_menus where keyword='$keyword'";
		$rs = dbFastQuery($sql);
		if ($rs[0]['order2'] == 0) {
			$sql = "UPDATE gfw_menus set active=1-active WHERE order1=" . $rs[0]['order1'];
		} else {
			$sql = "UPDATE gfw_menus set active=1-active WHERE keyword='$keyword'";
		}
		dbFastQuery($sql);
		redirect($o->page);
		break;
	...
	case 7: // Apagar
		$_SESSION['gMenu']='';
		$keyword = (gCleanField($_REQUEST['keyword']));
		$sql = "DELETE from gfw_menus WHERE keyword='$keyword'";
		dbFastQuery($sql);
		redirect($o->page);
		break;
}
