...
switch ($gPage)
{
	...
	case 0: // Listagem de páginas e opção pra criar uma nova
		...
		if ($usrId == 1)
			...
			$sql="SELECT u.* FROM gfw_users u 
			LEFT JOIN pessoas ON pessoas.id = u.id
			WHERE u.id>1 AND pessoas.motorista = 0 ORDER BY u.name";
	...
	case 1: // Novo registro
		...
		//$frm->add("{name: id_sales; fieldLabel: sales; type: combo; value: " . $id_sales . "; items: " . $sp['combo_representantes'] . "}");
		...
		//$frm->add("{name: modules; type: text; value: " . $modules . "}");
	...
	case 2: // Tunnel (salvar/editar)
		...
		//$flds['modules']=gCleanField($_REQUEST['modules']);
		...
		if (!is_array($errors))
		{
			...
			} else
			{
				...
				if (($gId>1) || ($usrId==1)) // Só o root altera seus dados
	...
	case 3: // Apagar (desativar)
		$sql="UPDATE pessoas set ativo=1-ativo WHERE id=$gId and id>1";
		dbFastQuery($sql);
		redirect($o->page);
		break;
	//-----------------------------------------------------------------------------------------------
	case 4: // Admin ou não?
		$sql="UPDATE pessoas set admin=1-admin WHERE id=$gId and id>1";
		dbQuery($sql);
		redirect($o->page);
		break;
}
