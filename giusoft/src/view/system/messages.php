...
switch ($gPage) {
	case 0:
		...
		$where='';
		if ($_REQUEST['de']>0 || $_REQUEST['para']>0 || $_REQUEST['mensagem']<>'' || $_REQUEST['data_de']<>'' || $_REQUEST['data_ate']<>'')
		{
			$flt='';
			if ($de>0)
			{
		...
		if (count($rs)>0)
		{
			...
			foreach ($rs as $row)
			{
				...
				$mtz[]='<-'.nl2br($row['mensagem']);
				$mtz[]='<>'.gDateTime($row['data']).$o->small('<br>'.$diasSemana[date('w', strtotime($row['data']))]);
				if ($caixa)
					$mtz[]='<-'.$row['para'];
				else
					$mtz[]='<-'.$row['de'];
			...
			switch ($rs[0]['tipo'])
			{
				case 1:
					...
					if (count($rsa)>0)
					{
						...
						$frm->add("{name: mensagem; type: show; value: ".nl2br($rs[0]['mensagem'])."}");
		...
		if (count($rs)>0)
		{
			...
			if ($valor_solicitado>0)
			{
				...
				if (!is_array($erros))
				{
					...
					$flds['mensagem'] = "Desconto aprovado para $aluno. Valor: ".gFloat($valor_solicitado).". \n\n".$mensagem;
