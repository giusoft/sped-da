...
switch ($gPage) {
	case 0:
		...
		//$sql = "SELECT * FROM " . gVar('database.i18n') . " where keyword='" . $field['keyword'] . "' and locale='$key'";
		...
		if (count($rs) > 0) {
			...
			foreach ($rs as $field) {
				...
				$mtz[]="<-".$field['source'];
	//			$mtz[] = '<>' . $o->button("{style: danger; size: tiny; icon: trash; hint: Delete; url: " . $o->page . "&keyword=" . $field['keyword'] . "&tnl=delete}");
				$tab.=$o->tableRow($mtz);
			}
			$tab.=$o->tableEnd();
		} else {
	...
	case 2: // ---------------------------------------------- Tunnel
		...
		if (!is_array($erros)) {
			...
			if ($id) {
				...
				//$redirect .= '&locale='.$locale.'&id='.(isset($_POST['salvar_proxima']) && !empty($_POST['next_id'])? $_POST['next_id'] : $id.'&update=1').'&gPage=1';
				...
				foreach ($locales as $key => $value) {
					...
					$flds['text'] = $text;
					$flds['revised'] = 1;
					$sql = "SELECT * FROM ".gVar('database.i18n')." WHERE locale='$locale' and keyword='$keyword'";
					$rs = dbFastQuery($sql);
					if (count($rs) == 0) {
	...
	case 3: // ---------------------------------------------- Delete
		$keyword = gCleanField($_REQUEST['keyword']);
		$sql = "DELETE FROM gfw_i18n WHERE keyword = '$keyword'";
		$rs = dbFastQuery($sql);
		redirect("index.php?g=translations");
		break;
