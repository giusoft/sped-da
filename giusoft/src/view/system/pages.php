...
switch ($gPage) {
	...
	case 1: // Nova página
		...
		if ($gId > 0) {
			...
			$content = base64_decode((string) $rs[0]['content']);
	...
	case 2: // Tunnel (salvar/editar)
		...
		$flds['content'] = base64_encode(addslashes((string) $_REQUEST['content']));
		$flds['locale'] = $locale;
		$flds['active'] = 1;
		...
		if ($gId == 0) {
			...
			$sql = sprintf("SELECT * FROM gfw_pages where keyword = '%s'", $keyword);
			$tem = dbFastQuery($sql);
	...
	case 4: // Apagar (desativar)
		$keyword = gCleanField($_REQUEST['keyword']);
		$sql = sprintf("UPDATE gfw_pages set active = 1-active WHERE keyword = '%s'", $keyword);
		dbFastQuery($sql);
		redirect($o->page);
		break;
}
