...
switch($gPage)
{
	...
	case PESQUISAR:
		...
		if ($_REQUEST["data_importacao_de"] && $_REQUEST["data_importacao_ate"])
		{
			...
			$where[]="(N.data_criou >= '{$data_de}' AND N.data_criou <= '{$data_ate}')";
		}
		if ($_REQUEST["numero"])
		{
		...
		} else
		{
			...
			{	
				...
				foreach ($rs as $row)
				{
					...
					{
						if ((intval($sTotalValorNota)>0) && $_REQUEST['exibirTotais'])
						{
							$mtz=array();
							$mtz[]="~2->Sub-total NF: ".$nf_numero;
							$mtz[]="->".gFloat($sTotalQuantidadeNota);
					...
					$mtz=array();
					$mtz[]="<>".date("d-m-Y H:i", strtotime($row["data_criou"]));
			        $mtz[]="<-".$sku;
