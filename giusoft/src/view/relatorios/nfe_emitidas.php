...
switch($gPage)
{
	case INICIO:
		...
		$frm->add('{name: id_armazens; type: combo; fieldLabel: Armazém;  value: 1; allowBlank: false; items:' . $sp['combo_armazens'] . ';}');
		$frm->row(
			...
			$frm->add('{type:checkbox; name:exibir_itens; fieldLabel:Exibir Itens ?;}'),
	...
	case PESQUISAR:
		...
		if ($_REQUEST["id_cfop"])
		{
			$flt[]="CFOP = ".gFieldById("cfops", $_REQUEST["id_cfop"], "codigo"."descricao");
		...
		if ($_REQUEST["data_emissao_de"] && $_REQUEST["data_emissao_ate"])
		{
			...
			$data_de=date('Y-m-d', strtotime(gDBDate($_REQUEST["data_emissao_de"])));
			$data_ate=date('Y-m-d', strtotime("+1 day", strtotime(gDBDate($_REQUEST["data_emissao_ate"]))));
			$where[]="(N.data_emissao>='{$data_de}' AND N.data_emissao <'{$data_ate}')";
		}
		if ($_REQUEST["numero"])
		{
			...
			if (count($rs)==0) {
				...
				foreach ($rs as $row)
				{
					if ($idProprietario<>$row["id_pessoas_proprietario"]) {	
						if ((intval($sTotalValor)>0) && gDBCheck($_REQUEST['exibirTotais'])) {
							$mtz=array();
							$mtz[]="~8->Sub-total";
					...
					if ($idProprietario<>$row["id_pessoas_proprietario"])
					{
						...
						$mtz=array();
						$mtz[]="~{$colspan}".strtoupper($row["nome_completo_proprietario"]) . " (". $row["proprietario"] .") ";
			        ...
			        //echo $_REQUEST['exibir_itens'];exit;
